...
**Prefer MCP over shell launchers.** Use **`wave_validate`** for docs lint results, **`wave_garden`** for metadata gardening (follow the tool’s `mode` contract), and **`wave_audit`** when you need wave state + validation + index health in one structured response. Treat **`.wavefoundry/bin/docs-lint`** and **`.wavefoundry/bin/docs-gardener`** as **CLI fallbacks** for hooks, CI, terminals, or any host where MCP is not attached — not the default path for agent instructions.

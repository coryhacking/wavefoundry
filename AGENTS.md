...
3. The wave is **readied** — a successful **Prepare wave** / **Ready wave** pass with `wave-council-readiness` recorded. A successful prepare requires that signoff: `wf_prepare_wave` returns `status: "ready_for_council_review"` when technical checks pass but the council review has not yet been run; run the review immediately, then call `wf_prepare_wave` again (`mode='ready'` to ready-without-opening, or `mode='create'` to prepare-and-open).

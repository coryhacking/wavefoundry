...
It owns the canonical framework source and will provide local tooling for framework-aware work across target repositories. The initial MCP tool surface should focus on inspection, validation, seed resolution, prompt-surface audit, and exact code search.
...
Planned Python package shape once MCP implementation starts:

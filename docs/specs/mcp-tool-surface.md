...
index database (`index.sqlite`, holding separate docs/code chunk, vector and FTS tables
alongside the code graph, its extraction and merge state, communities and analysis, and the
codebase-map receipt; wave 1xny6 retired the separate graph database and the `graph/` directory). Framework seeds and the top-level
...
| `index_health`  | Check semantic index health and surface stale/missing layers; also returns a `size` object (`total_bytes`, `total_human`, and a per-component `components` map keyed by the real on-disk entries — `index.sqlite`/`memory-state.sqlite`/…) for the on-disk index, so growth/bloat is visible without `du`, plus a `state_store` object (wave 1rsh9: `present`, `schema_version`, `integrity` — `ok`/`structural-fail`/`stale-fingerprint` from the two-layer probe, `size_bytes`; wave 1sbfk adds `chunk_index` — per-table `{vector_rows, registry_rows, covered}` coverage of the derived FTS/registry vs SQLite vectors, with a `chunk_index_undercovered` diagnostic when a table is materially behind, since structural `integrity: ok` says nothing about coverage) for the index database; wave `1x6ti` (`1x551`) adds a `reap` block (`deferred` per-table `{would_reap, table_paths}`, `preserved` per-table row-path counts, `recorded_generation`, `recorded_at`) read from the index database's persisted reap record whenever one exists (omitted otherwise), and raises `stranded_reap_deferred` while the deferred map is non-empty (the absent rows stay searchable until newly indexed files dilute the fraction under the breaker; if the paths really are gone, rebuild once every directory and volume is readable again: `index_build(content='all', mode='rebuild')`) and `stranded_reap_preserved` while the preserved map is non-empty (the subtree is served as of the last readable build; the first readable build reconciles it: `index_build(content='all', mode='update')`), both with `recovery_tools` `index_build` and `index_build_status` and neither advisory-flagged |
| `index_build_status` | Poll a detached background index refresh; also returns a `lock` object (`held`, `present`, `owner_pid`, `owner_cmdline`, `started_at`, `ended_at`, `note`) — the **authoritative** "is a build running" signal, where `held` is determined by **testing the real OS lock** (POSIX `fcntl` `F_GETLK` / Windows momentary `msvcrt`), not the file's presence. `ended_at` distinguishes a clean finish from an interrupted build. Read `lock.held`, never the file. Wave `1x6ti` (`1x551`): every state (`finished`, `idle`, `running`, `interrupted`) also carries the same `reap` block `index_health` reads (`deferred`, `preserved`, `recorded_generation`, `recorded_at`) whenever the index database holds a reap record, read from the store and never from the build log, and omits it when none exists |
| `index_build` | Run a synchronous index build: `mode='update'` (incremental) or `mode='rebuild'` (full); `content='fts'` rebuilds only the derived lexical layer (FTS5 + registry) from canonical SQLite chunks without regenerating embeddings, providing the `chunk_index_undercovered` recovery. |
| `index_optimize` | Maintain the shared semantic and graph SQLite index once under the index-build lock without regenerating embeddings: preserve reusable space, checkpoint passively, reclaim free pages in bounded batches, and report reclaimed bytes and integrity results. |
...
Gate classification is **per emit
...
healthy). Modes: `semantic`/`hybrid` (normal, including direct file/path artifact
questions), `exact` (`code_ask`'s weak generated-symbol/config/tool artifact exact-first
pass — healthy), `lexical_fallback` (semantic path unavailable over a
...
on a store whose model bookkeeping lacks provenance for an available semantic layer, ANY scoped build — including graph — escalates to all-layer
...
- The exact public `question_type` values, in order, are `navigational`, `explanatory`,
  `instructional`, `artifact_anchored`, and `assessment`. Artifact/path anchoring is evaluated
  before phrase signals. Direct file/path questions keep broad explanatory-like hybrid retrieval
  and stable-pin the strongest eligible exact path/basename owner at rank one after selection;
  the named path's published owner rows (`DIRECT_ARTIFACT_OWNER_ROWS` per table) are injected
  into the pool before reranking so the pin never depends on vector recall of a small file, and
  the seven ignore-file names are indexed as line-window code units (walker 16) so a direct
  ignore-file question can pin the file itself; a question that asks about the mechanism around
  the named file ("how does the framework restore the `.gitignore` block", "which function
  renders `.aiignore`") keeps the injection and the artifact type but does not pin the file over
  reranked evidence, while a named file that is the question's subject, with or without a leading
  article ("how does the `.aiignore` file exclude paths"), still pins; an explanatory lead ("how
  does", "why", "explain") keeps a question explanatory even when an assessment noun appears later;
  weak generated-symbol/config/tool questions retain the exact-first short circuit and use broad
  retrieval only when it is thin. Instructional phrases such as `how do I` and `how should I`
  take precedence over assessment nouns. Assessment retains its distinct response value but follows the
  explanatory-like gates: `VECTOR_TOP_K_EXPLANATORY = 50` candidates per index, documentation
  demotion, infrastructure partitioning, and `validation_required` when the top citation is
  documentation. Within assessment, every `docs/reports/` record recovers the generic report
  down-weight as a path class (no currentness or freshness predicate is evaluated), while
  historical `docs/waves/` records receive an additional bounded down-weight unless the query
  names that path. This is score-only, never exclusion, and preserves
  the per-source floors. Assessment alone also runs one bounded derived docs semantic/lexical query
  using generic audit/findings/gaps/weaknesses/opportunities/current-implementation vocabulary;
  deduplicated hits rejoin the same reranker and selection caps with their true reranker scores;
  no path class is injected and no synthetic score is written (the cycle-2 council of wave `1seaw`
  removed a report-class injector for exactly that reason). No other question type takes this path.
...
- **`confidence_basis` (wave `1wscp`):** names WHY the reported `confidence` has the value it does, so a caller can tell a strong answer from a merely long candidate list. One of five values: `semantic_lead` (the confidence follows the LEAD citation's reranked score), `exact_owner` (an exact owner row answered the question directly), `no_citations` (nothing was retrieved), `unranked_similarity` (the cross-encoder did not run, so ordering is vector or coverage based), and `lexical_fallback` (the semantic store was not servable and BM25 answered). The basis reads the LEAD citation, never the maximum over the set: a high-scoring citation buried at rank 7 does not raise confidence in the answer the response actually leads with.
...
- These lifecycle-focus diagnostics are observational because they are appended after the lifecycle result is derived; they do not gain an `advisory` payload key. The explicit `advisory: true` diagnostic contract is separate. Recorded credits are never re-attributed. Upgrade replaces the packaged server surface and requires the normal MCP reload before this response contract is active; no data migration, compatibility alias, or fallback is added.
...
- `artifact_or_test_id_ephemeral` is advisory in finding/approval `dry_run` and `create`: it warns when at least one path-like citation is present and all such citations are temporary. Recognized roots include `/tmp`, `/private/tmp`, `/var/tmp`, `/var/folders`, the runtime temporary directory and `TEMP`/`TMP`; any `scratchpad` path segment also qualifies. A durable repository path suppresses the warning; a bare test ID beside a temporary path does not. Zero path tokens produce no warning. This lexical heuristic neither checks artifact existence nor refuses a valid event.
...
- With `mode='run'`, regenerates agent/platform surfaces after required prompt and role sources exist. This includes renderer-owned upgrade-policy regions in local prompt docs; empty marker pairs alone are incomplete. The CLI fallback is `wf render-surfaces`. Do not hand-copy the renderer's block or overwrite project-authored prose outside its markers.
- The default `dry_run` skips rendering and reports `skipped: true`; it does not preview changes. Run mode reports files changed.
...
`index.sqlite` (wave 1sed7 — there is no `meta.json`); "metadata present" means the store
...
- Maintains the shared index database under the index-build lock, without re-embedding. Canonical text, vector, FTS and graph state all live in that one file, so wave `1xny6` collapsed maintenance to exactly ONE pass and ONE storage/reclamation entry.
- `content` accepts `docs`, `code`, or `all` and selects per-layer statistics; the one shared database is maintained once per invocation. A retired graph database still on disk before the upgrade's cleanup removes it is ignored entirely — neither maintained nor reported as live index storage.
- Maintenance uses native integrity/FTS checks, FTS optimization, query-planner optimization, a passive WAL checkpoint and incremental reclamation of at most 5,000 free pages per store. Routine passes do not run full `VACUUM` or force a truncating checkpoint, and the graph-only auto-vacuum conversion arm was retired with its parameter, stage and result key in wave `1xny6`. Reusable free pages may remain allocated; reclamation converges at exactly the per-pass bound, so draining a large freelist takes freelist-over-bound passes. Each pass reports page, freelist and WAL counts before and after, retained on the failure path.
- Response `data` retains per-layer `{tier, rows, size_before, size_after, reclaimed}`, per-store sizes/integrity under `stores`, `total_reclaimed`, `needs_rebuild` and `rebuild_spawned`. Shared-file bytes are counted under `stores`, not duplicated across layer rows. A lock-busy call returns `build_skipped_lock_busy` with `index_build_status` recovery.
- A failed integrity or publication check leaves the epoch unavailable and returns an error with `index_health` recovery guidance and the store failure details. Structural corruption of the shared semantic file is retained for explicit recovery, not automatically discarded or rewritten through the retired Lance repair ladder. Preserve the database and WAL/SHM companions; pending migration follows its receipt. `rebuild_if_needed` remains accepted for call compatibility.
- Close-time maintenance runs opportunistically when the shared index database has at least 40% free pages, subject to `indexing.close_optimize_enabled` and the build lock. Retired graph files do not trigger maintenance. Close reports maintenance failure as `index_optimize.ran=false` with `skipped="optimize_error"`, store details and recovery guidance; it does not turn a successfully closed wave back into an open wave. Successful maintenance retains per-store checkpoint and reclamation telemetry.
- Canonical synchronous setup also runs this bounded maintenance after publication, including upgrade setup.
...
- **Scanner coverage advisory (wave `1x5tr`):** normal gate-error and successful close responses include `data.scanner_skips`, a list of `{file, reason, detail}` rows for outstanding file-size, line-length, binary-extension and NUL-sniff guard omissions. Findings on ordinary lines still survive a partial file scan. These rows are advisory and do not change finding classification or close status. `data.scanner_skips_error` reports unavailable coverage when the ledger is malformed/unreadable; missing/empty history emits neither field and means **no recorded skips**, not complete coverage.
- Guard history is stored at `.wavefoundry/index/scan/guard-skips.json` relative to the target root, including builds using a custom index directory. The scanner publishes path/reason observations under a short lock with atomic replacement. A complete subsequent scan or confirmed path removal retires a record; re-skips, cache hits, unrelated scans, allowlisting and I/O failures retain it. Persistence failures emit nonblocking `WARNING` output and preserve existing history. The added close reader never scans or writes; existing docs validation still runs the record-only scanner before reading the advisory. Deleting the index or ledger loses history, which later scans must repopulate; the scanner-version change forces the first indexed scan after upgrade to be full. A path the ledger cannot name (a backslash on POSIX) is warned once and left out without vetoing the rest of the delta; a nested or otherwise undecodable ledger counts as malformed; and when a run's observations do not reach the ledger none of that run's files is cached as scanned (a path the ledger cannot name is likewise left uncached), on the indexer scan and the `wf_scan_secrets` subprocess alike, so the next run re-observes them.
- Without Git, scanner candidates exclude the existing semantic walk's machine-authority paths (canonical/legacy index, logs/locks, guard/purge state, canonical wave ledgers, memory archive/pointers and scan findings). Ordinary `.env`, generated and lock files remain eligible. Git candidate selection stays unchanged. This prevents new cache pollution in fresh caches; it does not purge historical machine rows or add exclusions for arbitrary custom index directories.
...
`wf_upgrade(phase: str = "preflight_to_docs_gate", confirm_hosts_stopped: bool = false, rebuild_storage: bool = false)`
...
token and bounded layer summary in the shared index database; an upgrade-lock copy
...
| `wavefoundry://graph/communities` | `text/markdown` | Catalog of code-graph communities — id, label, node count, boundary count, top-3 members by degree, ordered by size. Architectural communities first; Evidence/Data communities last under their own heading, each carrying `Type: Evidence/Data`, its evidence share, and the classification reason. Ids stay discoverable for both halves. The majority rule is `graph_query.is_evidence_community`, the same rule `wf_graph_report` applies, so the two surfaces cannot disagree. Read first to discover available `community_id` values | `code_graph_community(community_id=…)` |
...
## Index compatibility refusals

Index preparation/publication and query-triggered repair preserve newer or
unproven populated index contracts. `index_version_newer` identifies a newer
ordered revision; `index_compatibility_unproven` identifies malformed or missing
required provenance; `index_runtime_stale` identifies changed installed producer
source. Recovery names the affected component and persisted/supported identity
and requests reload/restart before the ordinary command resumes. These refusals
must not become healthy zero hits, an absent index or a destructive fallback
rebuild. Compatible read behavior remains governed by the existing reader
contract; a successful read does not authorize an incompatible writer.

For an upgrade from unprotected loaded code, `index_guard_restart_required`
returns the exact fresh-CLI handoff before publication, including same-schema
upgrades. `index_guard_handoff` persists in the ordinary upgrade checkpoint;
retain its original package and execute the returned `command_argv` with
`--confirm-hosts-stopped` only after repository hosts stop. Host discovery is
best effort. This obligation is separate from storage migration and cannot be
satisfied by in-process reload or editing a receipt.

A validated, invocation-bound guard pause has outer `status: "action_required"`,
no true `isError`, `data.code: "index_guard_restart_required"`, and
`data.failed_phase: null`; its exact `command_argv` is preserved. The pre-guard
ppjy wrapper receives the same envelope when it newly loads the incoming
restart-action reader after extraction. A host with an already-cached older
reader may still emit an outer error: recovery follows the validated guard
checkpoint and captured CLI command. This does not fabricate a storage receipt
or relax host confirmation. Plain exit code 3, stale or invalid checkpoints,
and dry-run failures remain errors; real storage pauses retain their contract.

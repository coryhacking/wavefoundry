...
semantic index (`index-state.sqlite` with separate docs/code chunk, vector and FTS tables). Framework seeds and the top-level
...
| `index_optimize` | Maintain the shared local SQLite index once under the index-build lock without regenerating embeddings: preserve reusable space, checkpoint passively, reclaim free pages in bounded batches, and report reclaimed bytes and integrity results. Graph storage remains independently maintained. |
...
- Maintains the shared semantic SQLite database and separate graph state under the index-build lock, without re-embedding. Canonical text, vector and FTS state stay in the same semantic file.
- `content` accepts `docs`, `code`, or `all` and selects per-layer statistics; the shared semantic and graph stores are maintained once per invocation.
- Maintenance uses native integrity/FTS checks, FTS optimization, query-planner optimization, a passive WAL checkpoint and incremental reclamation of at most 5,000 free pages per store. New graph stores enable incremental auto-vacuum; controlled maintenance converts older graph stores once after integrity validation, using one full `VACUUM` when the old mode was `NONE`. Routine passes thereafter do not run full `VACUUM` or force a truncating checkpoint. Reusable free pages may remain allocated.
...
- Close-time maintenance runs opportunistically when either the semantic or graph database has at least 40% free pages, subject to `indexing.close_optimize_enabled` and the build lock. An older graph auto-vacuum mode alone does not trigger close-time conversion. Close reports maintenance failure as `index_optimize.ran=false` with `skipped="optimize_error"`, store details and recovery guidance; it does not turn a successfully closed wave back into an open wave. Successful maintenance retains per-store checkpoint and reclamation telemetry.

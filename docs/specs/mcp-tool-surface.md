...
`wf_prepare_wave(wave_id: str, mode: str = "dry_run")` — modes: `dry_run` / `ready` / `create`
...
- Review-policy receipts use evaluator version `2`. Their admitted-change digest normalizes exactly one canonical top-level `Last verified: YYYY-MM-DD` line to a stable sentinel; every other byte remains significant. Missing, duplicated, malformed, or body-level lookalike lines are left byte-significant rather than guessed. A non-closed wave carrying a version-1 receipt therefore needs one deterministic re-Prepare to publish version 2, after which repeated Prepare is idempotent. Closed `wave.md` files and their event ledgers remain byte-immutable during this transition.
- Executed approvals and findings require the exact caller-authored `integrity_checks` object: five true booleans (`test_ran_without_unintended_skip`, `public_path_reached`, `boundary_values_realistic`, `assertions_non_vacuous`, `known_bad_detected`) and non-empty `known_bad_detection_method`. Approval calls require `approval_phase: readiness | delivery`; readiness and delivery currency are distinct, and server-derived readiness approvals bind the current policy receipt. Caller-supplied receipt IDs are rejected.
...
- Drives the framework upgrade flow phase-by-phase (subprocess over `upgrade_wavefoundry.py`). Valid phases:
  ...
  - `cleanup` — phase 5: remove the upgrade lock, **print the full human operator-summary prose**, and reload the server (non-cutover runs only; on a cutover-active run the reload is suppressed, see the cutover rule below). Also re-emits `data.summary` (same builder as the primary phase).
...
- Response `data` contains:
  ...
  - `doc_drift` — the doc-code drift worklist (wave 1ro44): `{available, flagged_count, entries}`. **This is the stable consumer contract for the future Verify-docs review loop** — build against these formats, not the implementation:
    ...
    - Drift never blocks `ready` — it is a proposal for deliberate review, not a gate.

...
8. Independent docs and code selectors both name `Snowflake/snowflake-arctic-embed-s`; equal IDs reuse one embedder instance (FP16 GPU / INT8 CPU, batch 32), both layers run in the foreground by default, and the code pass skips framework internal tests plus non-source/test/generated files unless `--include-tests` or `--include-generated` is passed. These models ship as model set 3 (the same weights as set 2); at upgrade cleanup, retired model-cache components are removed only after the freshly loaded cleanup process reads the semantic authority in `index-state.sqlite`, a one-way inspection boundary per `layering-rules.md`
...
10. Project index written to `.wavefoundry/index/index-state.sqlite`: canonical docs/code chunks, float32 vectors, external-content FTS and indexing state share one local file. Graph and memory keep separate stores; framework seeds still fold into the docs layer.
...
12. **Build epoch:** the existing FULL-durable `building` fence precedes mutation, and attempt-ID compare-and-set finalization advances the generation only after all mandatory residents reconcile. Readers validate the complete token before and after retrieval; a true no-op leaves the generation unchanged. The shared semantic transaction does not collapse the separate graph authority into the same SQLite transaction.
...
**State read:** repository inputs, `.wavefoundry/index/index-state.sqlite`

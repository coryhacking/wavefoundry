...
9. A failed transaction durably writes `.wavefoundry/logs/context-efficiency.gap`, suppressing all positive publication. A busy or locked SQLite error is first retried as a whole attempt (event commit and general-row transfer) within 10 seconds per write; only other errors or a lock that outlasts the budget poison. The gap file records the first cause (time, operation, exception type, and the message only for SQLite errors, so no call content), which health reports. Exceptions before the normal event commit route through the same poison barrier. Only failure to persist both event and poison changes the public call, returning `telemetry_persistence_failed`. The operator clears the gap with `context_efficiency.py --root <repo> --clear-gap`, which marks every unsealed or not-closed wave `accounting_gap` in the same transaction that removes the store-wide barrier, so only waves created after the clear report healthy totals.

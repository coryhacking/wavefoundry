...
**Update — `voyageai/voyage-4-nano` (2026-06-19, evaluated only, NOT tested against our corpus):** Open-weights (Apache-2.0; loadable via Transformers / Sentence-Transformers / vLLM, so local-only-compatible — unlike API-only Voyage models), ~340M params (≈3× `bge-base`), 32k context, Matryoshka dims 256–2048, int8/binary output precision. RTEB-code 80.66 (+5.5pp over `voyage-3.5-lite`'s 75.13) — but RTEB is VoyageAI's own benchmark and the sheet had no `bge`/OpenAI/Cohere rows for an apples-to-apples comparison. **Parked for the local path** for the reasons this section already documents: it's ~3× the size (the shipped FP16/CoreML speed win is tuned for `bge-base` and would not transfer for free), and the embedder is not the code-retrieval lever here — deep-rank misses are an NL→code vocabulary gap fixed by hybrid-lexical + reranker + chunking, not a base-model swap (ADR `1p52q`, `1p50s`). The one genuinely differentiated axis is the 32k context (a capability, not quality vs `bge`'s 512). **However, the size objection inverts for a future remote embedding *compute* backend** (a planned, opt-in direction — *only the embed call is offloaded* to dedicated/enterprise GPU hardware via API; the SQLite vectors and index-state store, and all change-detection stay **local**): there the 3× params is not a local-throughput blocker, and the open weights mean it can be **self-hosted on the enterprise GPU** rather than consumed as VoyageAI's cloud API (fits a no-external-SaaS posture). Re-evaluate `voyage-4-nano` (and the larger `voyage-4` / `voyage-4-large` tiers) as a remote-compute candidate when that backend lands — gated on a `code_ask` test-pack head-to-head, not RTEB. Remote embedding *compute* must remain **opt-in**; the local index (unified SQLite index) and the local-compute default stay unchanged.

**Merkle tree for index change-detection — evaluated, low value (2026-06-19):** Considered a Merkle tree to answer (a) "did anything change?" (root-hash compare) and (b) "what changed?" (subtree diff) for semantic + graph indexing. **Not worth adding** for this architecture. Change-detection is already content-hash-based and entirely **local**: per-file SHA-256 in the index-state store's build bookkeeping (`_detect_changes`) + per-chunk `chunk_hash` in canonical SQLite rows for semantic, and per-file `source_hash` in the graph state store (`project-graph-state.sqlite`, wave `1p9q2`) for the graph — so "what changed" is already answered precisely at file/chunk granularity (a Merkle tree is just the hierarchical generalization of this flat map; it adds no precision). A Merkle tree's only real wins are (1) an O(1) root-compare to skip the no-op directory walk and (2) subtree pruning — both *optimizations of the walk*, both requiring an incrementally-maintained tree fed by a **trusted, complete change feed** (the post-edit hook misses `git pull`/`checkout` and external edits, so a fallback walk is still needed for correctness), and both marginal at the current corpus scale (~1.1k files; the no-op path already skips file *reads* via the stat cache). The one case where Merkle trees genuinely shine — diffing **remote** index state to ship only changed leaves — **does not apply here**: the planned remote backend is stateless embedding *compute* only (SQLite vectors and state stay local), so there is no remote index state to diff. Merkle is also a **weak fit for the graph** specifically — edges cross file/subtree boundaries (a change in one file invalidates referrer edges elsewhere), so subtree-locality breaks and you cannot prune cross-file edge re-resolution by subtree (the graph merge pass handles this with symbol-name-scoped invalidation — wave `1p9q2` — not subtree locality). **Revisit only** if the corpus grows large enough that the per-refresh walk becomes a *measured* bottleneck — profile first.
...
4. Canonical chunks, FP32 vectors, external-content FTS and indexing metadata are persisted in
   `.wavefoundry/index/index-state.sqlite`. Docs and code use separate logical tables in the same file.
   Embeddings are prepared outside the writer transaction; related semantic changes commit together.

Subsequent builds reuse embeddings for unchanged chunk content and update metadata as needed.
...
2. Query embeddings use the layer's compatible model configuration with offline model loading.
   The native vector adapter validates dimensions and finite values before executing exact cosine
   search. Missing, invalid or unavailable storage follows the public tool's structured error path.
3. Metadata filters constrain candidates before top-K. Separate docs/code FTS5 results feed the
   existing hybrid fusion and reranker; conversion changes neither candidate counts nor final top-K.
4. Publication-epoch checks prevent a tool from accepting results spanning an index update.
...
| `test_embedding_dimension_matches_expected` | output dim == 384 | A dimension change invalidates stored SQLite vectors |
| `test_embedding_is_float32` | dtype is `float32` | The cosine search and SQLite vector BLOB format use float32; this pins the embedder output before serialization |
...
| `test_round_trip_search_returns_correct_chunk` | full embed → write SQLite → load → search pipeline | Exercises every link end-to-end; catches bugs in the index write or load path that unit tests miss |

...
**Finalize output-pass order** (the sequence `finalize()` applies to the assembled output edge map, each pass building on the previous one's edge set): inheritance-aware output passes (`_apply_inheritance_output_passes` — C# base-relation kind correction + single-definer inherited-method/`super.` binding) → config-key→reader bind (`reads_config`) → embedded-SQL bind (`1p9qf` — `reads`/`writes` from captured sink literals) → ORM entity→table bind (`1p9qg` — `maps_to`) → reverse-invalidation prune (drops edges into THIS merge's removed paths and symbols) → short-symbol prune → assembly-time dangling-endpoint filter (wave `1x6ti` / `1x5pc`: drops every edge whose endpoint is neither a node, nor an `external::` id, nor a current path of the build, nor (under a directory the walk reported unreadable this build) an endpoint whose edge the last published payload served, counted as `edges_dropped_dangling`; this, not the reverse-invalidation prune alone, is what keeps a stored fragment's re-emitted edge into a doc deleted on an earlier build or a symbol renamed while its doc was walk-shadowed out of the payload) → embedded-SQL capture reconcile → zero-edge doc prune (which therefore prunes a doc whose only edge dangled, exactly as a from-scratch build does); the analytics flags and the typed-call confidence downgrade (`_downgrade_unresolved_typed_calls`, which re-keys edges without adding or removing any) run after this chain, before the fingerprint. The same final-map inspection counts surviving non-callable call targets and removes malformed external call targets; callable-winning collision counts are aggregated from extraction artifacts. Every pass in this chain is recomputed FRESH from the merged per-file fragments/output maps on every build — none of them write back into fragments — so an incremental build's finalize output is identical to a full rebuild's (the fragments-stay-pristine invariant the `1p9qh` inheritance passes established and every later finalize-only pass in this wave follows).
...
5. Each node entry carries a `community` field populated from `_load_cluster_lookup(root)`. Outgoing `file`/`line` describe the callee definition; its top-level `snippet` is null. The nested `call_site` carries the caller’s `{file, line, snippet}`. Incoming entries retain their top-level caller call-site coordinates and expose the same nested shape. Missing definition lines and unavailable or external call sites are null. `_context_source_paths` credits outgoing content only to `call_site.file` when its snippet is present, never to the callee metadata.
...
To re-derive persisted call populations for a project without rebuilding, run:

```bash
python3 -B .wavefoundry/framework/scripts/graph_call_census.py --root /path/to/project
```

The helper defaults to `.wavefoundry/index/index.sqlite`; `--database` accepts another path beneath the supplied root. It opens an escaped read-only SQLite URI, uses one read snapshot, and closes the connection explicitly. Missing paths and paths escaping the root are refused. WAL databases require existing, root-confined regular WAL and SHM sidecars; otherwise use an already-open index. This avoids creating missing sidecars and retains visibility of committed WAL updates. Output separates variables/constants from callable targets and reports confidence, language, external targets, node kinds and `reads`/`defines`. It cannot reconstruct pre-dedup candidates or swallowed declarations; use extraction evidence and the builder collision counter for those.

Builder 52 invalidates earlier extracted artifacts even when source files are unchanged. Merge diagnostics include `non_callable_call_targets` (a count of surviving calls to variables/constants, without dropping them), `callable_wins_collisions` (callable-winning declaration collisions from extraction, retained with unchanged artifacts), and `malformed_external_call_targets_dropped` (malformed call targets removed by the final invariant). These appear in `merge_stats` and the build merge line. A no-change build retains the last materialized call-integrity summary; its malformed-drop count describes that publication, not a new removal on the no-op pass. The first counter exposes surviving exact-path calls to data-kind nodes instead of hiding them, including supported callable-valued constants; the callable-constant fixture is an expected positive. Compare confidence, `reads`, `defines`, target kinds and external targets alongside removed false calls.
...
2. A supported forward version change (`schema_version`, `builder_version`, `walker_version`, `chunker_version`) records reset intent in `GraphStateStore.ensure_current()`; scoped replacement occurs inside publication. A newer persisted revision or malformed/unproven populated identity refuses before reset, preserving the published graph.
3. The store's file listing is empty after loading — `update_graph_index()` expands `changed_set` to all files.

Additionally, when code files change, doc artifacts whose cached `mentioned_symbols` intersect the changed symbol ids are automatically re-scanned via the `impacted_docs` pass inside `finalize()`. Since wave `1x5tq` (`1x8e1`, graph builder 51), document artifacts and merge fragments also retain `unresolved_doc_targets`: normalized local Markdown/backtick paths that were absent when the document was extracted. This state survives pruning a zero-edge document from the served payload. Before the zero-change return, `finalize()` intersects each document's unresolved targets with current paths, including paths with no graph node. A matching document is read once and re-extracted; only successful extraction replaces its obligation. An unreadable directory or failed file read leaves the obligation for an unchanged retry. Never-present targets and already-resolved links do not trigger document reads.

The merge-state blob is loaded once and reused for eligibility and merge work; its decode cost scales with the whole blob, while unaffected artifact rows and document text remain unread. The outer `build_index` idle path uses `read_pending_doc_link_repairs()` to read the same state without constructing a mutating graph store. A real build forwards that snapshot to the graph session under its build lock and epoch, and the session checks the snapshot's metadata before reuse. An older supported builder/version is reported as pending re-extraction; a newer or unproven contract is a compatibility refusal, not pending repair. Dry runs report pending graph work and return before execution; a healthy idle build with no pending work opens no epoch. Graph recovery also performs graph orphan retirement, so a combined idle pass reconciles the other sidecars without running a second graph merge.
...
**Regeneration ownership + MCP surface + idempotence (wave `1p601`).** Ordinary **`indexer.py::build_index`** runs do not regenerate the map because writing into indexed `docs/references/` would create a self-referential write/reindex loop. Create-mode prepare-and-open and create-mode close refresh it fail-safe; ready-only and dry-run modes do not. Upgrade, forced **`index_build(content="map")`**, the direct change-only `wf codebase-map` / generator CLI, and the **missing-file** fallback in **`wavefoundry://codebase-map`** are the other owners. A resource read of an existing map performs no freshness check and writes nothing. The missing-file fallback may write the map, its graph fingerprint, and the marker-bounded modules block in `docs/repo-index.md`. Regeneration is otherwise **change-only / idempotent**: a matching input fingerprint skips the render, and matching rendered content (ignoring `Last verified`) skips the write. The forced map-only MCP path bypasses the input fingerprint so renderer changes materialize without a full rebuild. Newly registered MCP resources and tool options remain startup-bound; reconnect only if a fresh turn still has stale schemas, then restart the host last.

**Per-area `AGENTS.md` context (wave 1p5xc).** Vendor-neutral per-area context files live at major areas' representative paths. `gen_codebase_map.py --scaffold-area-contexts` is an **opt-in** command that scaffolds an empty **stub** `AGENTS.md` for each major area (idempotent; never overwrites an existing file; never auto-authors conventions — humans write the content). `render_markdown` names each existing file with an inline-code repo-relative prose path. Discovery is agent-agnostic: that map path, a standing convention line woven into the run-contract seed (`020`) and rendered into every host agent surface, and the doc index (a subdirectory `AGENTS.md` is a normal `.md` file, picked up by the index walk and surfaced in `docs_search` / `code_ask`). The only `@import` the framework adopts is the root `CLAUDE.md` → `@AGENTS.md` bridge (rendered by `render_agent_surfaces.py`); there are no per-folder `CLAUDE.md` bridge files and no nested `@import`.

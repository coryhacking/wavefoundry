...
> **Line citations** in older sections reference the `GRAPH_BUILDER_VERSION="29"`-era source (waves 1p4ls/1p4q4/1p4up); the current constant is `"50"` (wave `1x6ti`). Line numbers shift on builder version bumps — use function names as stable anchors when citing across versions.

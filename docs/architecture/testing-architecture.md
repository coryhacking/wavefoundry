...
| Eligibility reap absence guards (wave 1x54z) | Injection at the seams, never `chmod`: `os.scandir` denied for one directory (or the root) drives `os.walk`'s `onerror` collector, and `_orphan_path_stat` injected with EACCES drives the stat classification. The build-path seam is exercised with an unrelated edit during the outage (Lance rows, layer hashes, bookkeeping, freshness and graph rows all kept; recovery re-hashes and re-extracts nothing) and the zero-change seam with the 1p312 strand technique; the breaker's two legs are pinned at their boundaries with absent candidates (8 of 15 defers, 8 of 16 reaps, 7 of 13 reaps) and present scope departures are pinned to reap whatever their count, including an `include_tests` narrowing end to end; the build result's deferral and preservation fields are asserted at both seams. A fixture note: a non-git repository's secrets candidate walk can include index internals, so the sidecar reconcile is neutralised where an assertion is about the reap alone | `test_indexer.py` (`EligibilityReapAbsenceGuardTests`) | Same runner |

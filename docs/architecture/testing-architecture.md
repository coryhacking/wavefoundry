...
| Install reliability (wave 1yp0y) | Source opt-in for nine mandatory framework-internal constant claims; consumer fixture with packaged scripts remains exempt while structural checks stay active; invalid opt-in errors. Windows tests cover bounded `python3`/`python` diagnosis, strict early stop before setup writes, completed-log independent diagnosis, and policy-blocked IT guidance. Public sync fills an empty upgrade-policy region and preserves authored prose/permission boundaries; partial row 2.9 resumes by conditional catalog plus manifest and audits before completion. Mocked Windows branches do not qualify native execution; native diagnostic branches, standard-user repair, fresh-host MCP and hook evidence remain unverified and are operator-deferred to external testing after the next release (2026-09-22); see the wave post-release-windows-validation.md protocol. | Focused validator, setup, launcher, renderer and install-audit tests; native Windows runbook in `docs/references/native-windows-support.md` | Focused tests; native execution receipt pending |
...
| Retained repair-context independence (wave 1ypy6) | Producer-built fixtures and public event boundary tests cover cross-finding refusal, append non-mutation, replay, both close audit consumers, delivery advisories, current-authority supersession, receipt rotation and summary history; guard-removal probes fail the targeted assertions. Dependency fixtures exercise exact wave-record IDs, legacy unique prefixes and ignored prose/table mentions. | `test_review_evidence.py`; `test_server_tools_lifecycle.py` | canonical runner |
...
| Distribution extension tools (wave 1yv9l) | The real server in a scratch copy of the scripts tree: stock build reports empty extension provenance; a declared module's new tools and explicit overrides are served, tiered, allowlisted and wrapped exactly like the core tools they add to or replace; write-tier tools fail fast during an upgrade checkpoint; edited modules are served after reload with a new executed-bytes hash; twenty-seven refusal fixtures (including async handlers, a changed parameter type, reserved tool names, registrations outside `FastMCP.add_tool`, staging tampering, withdrawn registrations, direct served-table writes, resources and prompts) plus an incompatible-override fixture serve nothing; `build_server` refuses an invalid declaration and serves no tool; overrides, write-during-upgrade and unknown arguments are also driven through FastMCP `call_tool`, with the lifecycle lock observed held inside the override body; a failed reload serves only runner tools; an invalid tier declaration stops the allowlist renderer. Mutants removing the strip, attempt recording, compatibility check, extension guard, roster validation, location check, async refusal, unrecorded-registration check, withdrawn-registration check, served-table check, schema-preservation check or relative provenance path each fail the file; a changed default alone, and an added optional parameter with a custom title, stay call-compatible and serve through `call_tool`; a new `wf_`-prefixed extension tool is served, tiered, wrapped and allowlisted (wave 1yyoj); a census classifies each module-level assignment in `scripts/` and `wave_lint_lib/` whose literals include a served or retired tool name, or that is derived from a classified collection, as reserved, non-behavior or derived, and fails on an unclassified one; a membership test keeps the reserved collections to served core or retired names; a collection naming only unserved tools, or one built inside a function, is outside the census (a documented surviving mutant) | `test_extension_tool_modules.py` | Same runner |
| Differential equivalence harnesses (wave 1rsh9) | Optimized path vs authoritative path over identical inputs — the registry-backed incremental skip vs the canonical-row delta plan (`RegistryDifferentialTests`), and the secret-scan cache path vs a no-cache full scan through a six-mutation git fixture matrix with the REAL scanner (`DifferentialEquivalenceTests`). Any divergence fails; these are the adoption gates for skip-class optimizations | `test_fts_lexical_layer.py`, `test_secret_scan_cache.py` | Same runner |
...
| Memory-retrieval eval (waves 1sufo / 1tbt5) | Hermetic golden set over a synthetic memory corpus scoring the shipped search path and an evaluation-only BM25+semantic RRF candidate: recall@k / MRR, 11 policy/archive/adaptive-freshness invariants, deterministic single-stream controls, and an explicit adoption decision. An optional bounded self-summary diagnostic freezes its sample before scoring and emits aggregate metrics, kind/status counts, and a fingerprint only; it is explicitly nonqualifying. Wave 1yad2 adds strict frozen-manifest/adoption helpers, independent holdout and failure evidence, SQL group-before-limit controls, and production summary5/RRF query tests. Blind per-record support judgments complement query recall; explicit useful-answer acceptance keeps adjacent context distinct from direct support. Model/provider isolation, no unchecked refill, filters/history and lexical recovery require public-path tests. A missing docs layer, failed semantic query or incomplete measurement cannot earn qualification credit. The registered 1,000-record lexical budget guards its single-pass cost. See `docs/references/memory-retrieval-eval.md`. | `memory_eval.py` (shipped engine), `tests/eval/memory_golden.json` (test-only fixture), `test_memory_eval.py`, `tests/perf_budget_policy.py` | Same engine; the live pass is the `wf_memory_eval` MCP tool (CLI fallback `--curated-root <repo>`) |
...
| Eligibility reap absence guards (wave 1x54z) | Injection at the seams, never `chmod`: `os.scandir` denied for one directory (or the root) drives `os.walk`'s `onerror` collector, and `_orphan_path_stat` injected with EACCES drives the stat classification. The build-path seam is exercised with an unrelated edit during the outage (canonical chunk/vector rows, layer hashes, bookkeeping, freshness and graph rows all kept; recovery re-hashes and re-extracts nothing) and the zero-change seam with the 1p312 strand technique; the breaker's two legs are pinned at their boundaries with absent candidates (8 of 15 defers, 8 of 16 reaps, 7 of 13 reaps) and present scope departures are pinned to reap whatever their count, including an `include_tests` narrowing end to end; the build result's deferral and preservation fields are asserted at both seams. A fixture note: a non-git repository's secrets candidate walk can include index internals, so the sidecar reconcile is neutralised where an assertion is about the reap alone | `test_indexer.py` (`EligibilityReapAbsenceGuardTests`) | Same runner |

...
| Distribution extension tools (wave 1yv9l) | The real server in a scratch copy of the scripts tree: stock build reports empty extension provenance; a declared module's new tools and explicit overrides are served, tiered, allowlisted and wrapped exactly like the core tools they add to or replace; write-tier tools fail fast during an upgrade checkpoint; edited modules are served after reload with a new executed-bytes hash; twenty-eight refusal fixtures (including async handlers, a changed parameter type, registrations outside `FastMCP.add_tool`, staging tampering, withdrawn registrations, direct served-table writes, resources and prompts) plus an incompatible-override fixture serve nothing; `build_server` refuses an invalid declaration and serves no tool; overrides, write-during-upgrade and unknown arguments are also driven through FastMCP `call_tool`, with the lifecycle lock observed held inside the override body; a failed reload serves only runner tools; an invalid tier declaration stops the allowlist renderer. Mutants removing the strip, attempt recording, compatibility check, extension guard, roster validation, location check, async refusal, unrecorded-registration check, withdrawn-registration check, served-table check, schema-preservation check or relative provenance path each fail the file; a changed default alone, and an added optional parameter with a custom title, stay call-compatible and serve through `call_tool` | `test_extension_tool_modules.py` | Same runner |

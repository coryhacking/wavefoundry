...
| Offline model-set asset | Synthetic warmed-cache fixtures verify independent model-set selection, component/file hashes, traversal/link rejection, idempotence, corruption repair, and no-downgrade behavior without downloading model bytes | `test_model_bundle.py` | Same runner |

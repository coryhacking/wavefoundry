...
**Server package (wave 1yzd0, ADR 1yx4m):** `server_impl`, `mcp_tool_registry` and the ten `*_handlers` modules live in `scripts/wf_server/`, with an empty `__init__.py`. Each keeps a flat three-line alias file that replaces itself in `sys.modules` with the package module. `_FLAT_ALIASES` in `wf_server/server_impl.py` drives the eager aliases, registered after the last module-top import, and the dual-key reload purge, which evicts the flat and package keys of every moved module except `server_impl`. `SCRIPTS_DIR` is the scripts root, and every retained-script lookup goes through it; the package directory is never on `sys.path`. Upgrade-mandatory modules and every consumer outside the package import flat names, which is what installed 1.25 and 1.26 runners accept.

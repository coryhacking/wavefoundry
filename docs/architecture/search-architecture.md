...
2. **Native HNSW index above threshold.** When a table reaches `LANCEDB_INDEX_THRESHOLD = 1000` rows, an `IVF_HNSW_SQ` index is built automatically. Below that threshold, LanceDB performs a flat scan (comparable to numpy) with no index overhead. **ANN query tuning runs at the library defaults (wave 1wpif, `1wpah`).** `_lance_search` submits every query as `search(vec).metric("cosine").limit(n)[.where(predicate, prefilter=True)]` and sets neither `nprobes` nor `refine_factor`; the query object it submits carries `minimum_nprobes = maximum_nprobes = refine_factor = None`, so the installed engine's defaults apply. On lancedb 0.33.0 the executed plan for an indexed table reads `ANNIvfPartition: minimum_nprobes=20, maximum_nprobes=Some(20)` with no refine stage (asserted by `test_retrieval_candidate_generation.LanceQueryBuilderDefaultsTests` through `explain_plan` on a real `IVF_HNSW_SQ` table). The former `LANCEDB_NPROBES = 20` / `LANCEDB_REFINE_FACTOR = 10` constants declared in `indexer.py` and `server_impl.py` were applied nowhere and were retired (the probe count happened to equal the engine default; no refinement ever ran). Measured ANN tuning, with its exact-search reference and overlap metric, is owned by wave `1wpih`.
...
accounting ledger; each Lance vector search, each FTS5 MATCH per table (recorded inside the
...
**Mechanics.** The index-state store (`index-state.sqlite`, see Index Format) carries one contentful FTS5 table per Lance content table (`fts_docs`, `fts_code`), keyed by chunk id and maintained by the build under an ordered-consistency model: Lance is authoritative for chunk existence, the store's rows commit in one SQLite transaction ordered after the Lance writes, and an end-of-build reconciliation pass (chunk-id set comparison → derived-only rebuild from Lance) repairs any crash window. Since wave 1wpif (`1wpag`) that reconcile is honest about FTS-only damage: with registry ids in sync it still verifies FTS liveness (a real MATCH), FTS-versus-registry row parity, shadow-table (`_docsize` / `_content`) parity, and the keyed payload digest before taking the in-sync return; any failure heals through the same derived-only rebuild. The digest is a per-row sha256 over the FTS-served payload fields (id, path, kind, language, tags, lines, text), XOR-aggregated so it is order-independent and maintained in O(delta) at the incremental seam (XOR-out the payloads a delta removes, XOR-in the rows it adds, so a corrupted pre-state is never re-blessed by a later delta); it is computed and written only by the derived-state writers inside their own transaction under the build lock (the publication boundary) and only ever compared by readers. Its threat model is corruption and partial-write detection under the local single-writer model; it is not tamper evidence (anything that can write the store can rewrite the digest). Tokenizer: `unicode61 tokenchars '_'` — compound identifiers stay whole tokens, because exact-identifier queries are this layer's reason to exist (concept queries are the dense layer's job).

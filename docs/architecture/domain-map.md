...
| **Operational Telemetry** | `.wavefoundry/logs/context-efficiency.sqlite` | Ignored host-local write-through event/source/evaluation ledger, phase state, replay protection, store identity, and pending checkpoint state; numeric authority for live telemetry | Eligible retrieval/lifecycle calls and typed paired-evaluation attachment | Marker-owned `## Context Efficiency` snapshots in wave records; `wave_current` / `wave_audit` durable-state reads |

...
|--------|------|----------------------|-------------|---------------|
| **Framework Seeds** | `framework/seeds/` | Canonical prompt source; numbered seed prompts; overview docs; reference appendices | None — canonical source | Consumed by target repositories via install/upgrade |
| **Framework Scripts** | `framework/scripts/` | Lifecycle ID generation; docs linting; docs gardening; platform surface rendering; packaging; test running | `docs/workflow-config.json` (config read); `docs/` tree (lint/gardener read/write) | framework/VERSION (write); zip archives (write); .claude/, .cursor/, .github/hooks/ (render writes) |
...
| **Wave Framework Distribution** | Root zip archives | Packaged distribution for target repositories | `framework/` tree | Target repository `.wavefoundry/framework/` after unpack |
...
1. `framework/seeds/` is source of truth for generic framework behavior — no domain modifies it except Wavefoundry maintainers through an explicit wave.
2. `framework/scripts/` reads `docs/` but does not own it; the docs tree is owned by the Wave Framework seeding process.
...
|------|------|-----------|-------|
| `build_pack.py` → `framework/VERSION` | file write | stable | Engineering (packaging) |

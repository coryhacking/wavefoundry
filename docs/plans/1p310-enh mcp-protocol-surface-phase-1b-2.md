...
2. `index_build(mode='rebuild')` inline path emits `ctx.report_progress` checkpoints during the embedding-rebuild phase.
...
- `.wavefoundry/framework/scripts/server_impl.py` — wire `ctx.report_progress` into the `index_build` rebuild path; thread `Context` through the affected tool handlers.
...
- [ ] AC-3 (parent): `index_build(mode='rebuild')` inline path emits `ctx.report_progress` checkpoints during the embedding-rebuild phase.
...
- [ ] **Phase 1b (b):** thread `Context` through the rebuild-triggering tool handlers (graph_query auto-rebuild + server_impl `index_build`)
...
- Patterned after: `1p2q3/131bu` (`wf_reload_mcp` description-refresh via `send_tool_list_changed`) — same "adopt an existing MCP protocol primitive we weren't using" template.

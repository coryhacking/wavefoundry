...
1. The model warm must run with `HF_HUB_DOWNLOAD_TIMEOUT` and `HF_HUB_ETAG_TIMEOUT` set to bounded, conservative values in the environment visible to the fastembed/HF download, so a stalled socket raises a timeout on the worker thread inside the wall-clock deadline rather than only being caught by the abandoning wall-clock abort.
...
3. The env must be applied **scoped** to the warm (set-then-restore, mirroring `_offline_env` at `setup_index.py:567`), so the setting does not silently leak into unrelated later process behavior; an operator-set value in the inherited environment must be respected (do not clobber an explicit operator override).
...
- Set `HF_HUB_DOWNLOAD_TIMEOUT` and `HF_HUB_ETAG_TIMEOUT` in the environment around the model warm (`_warm_model_inner` / the `_warm_model` deadline-bounded attempt), scoped via a set-then-restore context manager mirroring `_offline_env`.
...
- [ ] AC-1: During the model warm, `HF_HUB_DOWNLOAD_TIMEOUT` and `HF_HUB_ETAG_TIMEOUT` are present in `os.environ` at the configured (or default) values; a unit test asserts both are set to the expected values while the warm body runs.
...
- [ ] AC-3: The warm env is applied scoped: after the warm returns (success or failure), `HF_HUB_DOWNLOAD_TIMEOUT`/`HF_HUB_ETAG_TIMEOUT` are restored to their prior state (unset stays unset; an operator-set value is preserved and not clobbered during the warm). Verified by a unit test asserting pre/post env state and operator-value preservation.
...
- [ ] Add a `_hf_hub_timeout_env()` scoped context manager in `setup_index.py` (mirror `_offline_env`, `:567`): set `HF_HUB_DOWNLOAD_TIMEOUT`/`HF_HUB_ETAG_TIMEOUT` from config/defaults on enter, restore on exit; do not clobber an operator-set inherited value.
...
- [ ] Wrap the model warm (`_warm_model_inner` body / the `_warm_model` deadline attempt) in `_hf_hub_timeout_env()` so the fetch runs under the socket timeouts.
- [ ] Document the new keys in `docs/workflow-config.json` (notes-only) and add an honest inline comment that these complement, not replace, the DF-1 wall-clock deadline.
- [ ] Add unit tests: env-present-during-warm, scoped-restore + operator-value-preservation, config override/default/malformed, healthy-warm-unchanged, default-ordering (`socket < model_warm_deadline`).

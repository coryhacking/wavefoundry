...
3. The five lifecycle gates (`wave_validate`, `wave_prepare`, `wave_review`, `wave_close`, `wave_install_audit`) must be **unchanged** — they continue to run the full-corpus `run_validate` scan; only the post-write attachment path is rescoped.
...
5. Empty or non-git changed sets must be treated as a clean no-op (mirroring `_lint_changed`'s `([], [])` contract), so a write tool that touched no `docs/` markdown (or a non-git checkout) reports clean rather than erroring — the full gates remain the backstop that catches anything the incremental path skips.
...
- [ ] AC-3: The five lifecycle callers (`wave_validate`, `wave_prepare`, `wave_review`, `wave_close`, `wave_install_audit`) still call the full-corpus `run_validate` (no `--changed`); a unit test asserts at least one lifecycle path's argv omits `--changed` (regression guard that the rescope did not leak into the gates).
...
| ---- | ------ | -------- |
| 2026-07-03 | Scoped from the first `1p9j0` delivery council's rotating-seat (performance) alternative and `1p9iu`'s accepted-but-flagged 30s→300s post-write latency risk. Verified: `_run_post_write_lint` (`server_impl.py:3019`) → `run_validate` (`:3071`) full scan, no `--changed` (`:3095`), 300s bound (`docs_lint_full_scan_timeout_seconds`, `:2005`). Incremental machinery present and proven: `docs_lint.py --changed` via the post-edit hook (`render_platform_surfaces.py:350`), flag + `_lint_changed` in `wave_lint_lib/cli.py:62,171` (safe `([], [])` no-op; config-change → full-lint fallback), hook bound `docs_lint_hook_timeout_seconds` (default 120s, `indexer.py:218`). | `server_impl.py:2005,3019,3071,3095`; `wave_lint_lib/cli.py:62,171`; `render_platform_surfaces.py:350`; `indexer.py:218`. |

...
**Brief.** Goal: begin shrinking `server_impl.py` by moving the two largest self-contained handler families, code navigation and graph query, into sibling modules that declare their own tools through the registry from `1y0be-ref tool-registry-and-wrapper-chain`. Audience: framework maintainers and downstream integrators who merge around the server file. Approach: move the `*_response` functions and their private helpers; keep the thin registration in the composition root or have it composed from each module's `TOOLS` list. Constraints: public surface identical; reload safe; no change to retrieval behavior. Success: neither family's response functions are defined in `server_impl.py`, both modules are unit-tested without the transport, and the golden fixture is unchanged.
...
2. Each module exposes `TOOLS: list[ToolSpec]` built with the registry types, and `register_mcp_surface` composes those specs instead of defining the nineteen closures inline. Names, tiers, parameters, docstrings, and annotations are unchanged.
...
6. An AST test asserts that none of the nineteen response functions is defined in `server_impl.py` after the move, and the registry-based parity test from `1y0be` covers the relocated tools so the AST census can be retired in this change.
...
- The two modules, their `TOOLS` lists, purge-list entries, and tests.
- Composition of the registry from module `TOOLS` lists.
- Retiring the AST roster census in favor of the registry parity test.
...
- [ ] AC-5: The registry parity test covers the relocated tools, and the AST census is removed without loss of coverage.
...
- [ ] Move the graph family first with its `TOOLS` list; run golden and retrieval corpus.
- [ ] Move the code-navigation family; run golden and retrieval corpus.
- [ ] Compose `register_mcp_surface` from the two `TOOLS` lists.
- [ ] Add purge-list entries, the no-import test, the AST absence test, and the reload test.
- [ ] Retire the AST census after the registry parity test covers relocated tools.
...
| compose-guards | qa          | codenav      | composition, tests, census retirement, packaging |
...
`docs/architecture/current-state.md` and `docs/architecture/domain-map.md` list the two handler modules and state the rule that a handler family is a sibling module plus a `TOOLS` list on the purge list. `docs/architecture/search-architecture.md` updates file references for the graph query entry points. The decision record from `1y0be` gains the exemplar outcome.
...
| AC-5 | important | Replaces a brittle census with a runtime one |
...
| 2026-09-14 | Retire the AST census in this change | Once relocated tools are covered by the registry test, the census would need module-aware parsing to stay correct | Keep both: redundant and the census becomes wrong as soon as handlers move |

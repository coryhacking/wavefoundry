...
- Promoted to agent memory (typed record, via memory_propose/memory_validate rewrite): "Reload-survivor MCP tool changes are a process-restart boundary" (`1t38p-mem reload-survivor-mcp-tool-changes-are-a-process-restart-bound`).

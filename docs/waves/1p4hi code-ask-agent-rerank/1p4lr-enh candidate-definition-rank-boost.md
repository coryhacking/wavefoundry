...
- [ ] AC-4: **Multi-language detection.** Declaration detection + name extraction works for Python (const/def/class), JS/TS (const/function/class/interface/type/enum), and Go (const/func/type). Verified by per-language unit tests.
...
- [ ] AC-6: Code comments document the boost + its tunable; `docs/specs/mcp-tool-surface.md` `code_ask` entry notes the definition-boost behavior only if observable in the response contract (else N/A with rationale).

## Tasks

- [ ] Add `CONST_DEFINITION_BOOST` constant + `_definition_match_boost(query, candidate)` helper (declaration detect → name extract → tokenize → strict overlap → factor) in `server_impl.py`.
- [ ] Apply the boost on candidate `score` in `search_combined`'s agent branch (before `_agent_candidate_select`, alongside the navigational weight / demotion logic).
- [ ] Per-language declaration patterns (Python/JS-TS/Go) with fail-open behavior.
- [ ] Unit tests: AC-1–AC-4.
- [ ] AC-5 value eval (reuse the `1p4hj` AC-10 recall harness): before/after ranks across the known-answer set + the `RERANKER_MODEL` query; record and set `CONST_DEFINITION_BOOST`.
- [ ] Run framework tests + docs-lint; reload MCP; live smoke on the `RERANKER_MODEL` query.

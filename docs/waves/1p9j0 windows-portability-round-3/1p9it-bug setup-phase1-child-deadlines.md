...
- [x] AC-1: The venv-create (`:210`), uv-bootstrap (`:353`), and dep-install (`:401`) spawns each pass a `timeout=`; a simulated `subprocess.TimeoutExpired` from those spawns produces a loud, stage-named failure message referencing network/proxy/TLS reachability, verified by unit test. — `_bootstrap_venv`/`_bootstrap_uv`/`_install_deps` now pass `timeout=` + catch `TimeoutExpired`; tests `test_bootstrap_venv_timeout_fails_loud`, `test_bootstrap_uv_timeout_falls_back_with_loud_message`, `test_install_deps_timeout_fails_loud_with_network_guidance`.
- [x] AC-2: The in-process model warm (`_warm_model`/`_build`, `:812-821`) is bounded by a wall-clock deadline; a simulated over-deadline `embed(...)` triggers an abort with a stage-named message referencing model-download network/proxy/TLS, verified by unit test (no reliance on a real network fetch). — `_warm_model` wraps `_warm_model_inner` in `_run_in_process_with_deadline`; test `test_warm_model_aborts_over_deadline` (patches `_warm_model_inner` to sleep past a 0.05s deadline).
- [x] AC-3: `_run_indexer` (`:1383`/`:1398`) terminates and kills a child that emits no output within the configured stall window and fails loud with a stage-named message referencing disk/CPU/memory, verified by unit test with a stub/no-output child; the child is confirmed reaped (no orphan). — reader-thread + queue watchdog + `_terminate_and_reap`; test `test_run_indexer_stalled_child_terminated_and_reaped` asserts terminate() + kill() both called. Scope note (delivery review): "no orphan" covers the DIRECT `indexer.py` child only — its spawn-mode multiprocessing pool workers are not signalled by terminate/kill and may survive holding RAM until they notice the broken management pipe; portable process-tree kill is future hardening, not in scope.
- [x] AC-4: Each stage deadline is read from `docs/workflow-config.json` (setup block) with a documented default; an override in a fixture config is honored and a missing/malformed config falls back to defaults without raising, verified by unit test. — `_setup_deadlines(root)` loader; tests `test_setup_deadlines_reads_config_override`, `_none_root_returns_all_defaults`, `_missing_file_returns_defaults`, `_malformed_and_nonpositive_fall_back`; `setup` block documented in `docs/workflow-config.json`.
- [x] AC-5: A within-deadline (normal) setup run is behaviorally unchanged — same streamed output, same exit code, same background-build spawn — verified by the existing setup tests staying green plus a within-deadline regression assertion. — existing `test_setup_index` (123 tests) green; `test_run_indexer_within_deadline_streams_all_lines_in_order` asserts byte-identical streamed output + no terminate/kill.
- [x] AC-6: `python3 .wavefoundry/framework/scripts/run_tests.py` passes; `wave_validate` (docs-lint) is clean after the workflow-config reference-doc update. — deferred to the coordinator's central full-suite + docs-lint pass (per wave-landing rules, the full `run_tests.py` was not run in-lane). Relevant subsets green: `test_setup_index` 123 OK, `test_indexer` 193 OK, server-tools validate/guard subsets OK.

## Tasks

- [x] Add a workflow-config loader for setup deadlines (helper next to `_workflow_project_include_prefixes` at `:1511`), with default constants for venv, deps, model-warm, and index-build stall window; malformed/missing config → defaults. — `_setup_deadlines` + `_SETUP_DEADLINE_KEYS` + `*_TIMEOUT_DEFAULT` constants.
- [x] Pass `timeout=` to `isolated_run` at `:210`, `:353`, and `:401`; wrap each in `try/except subprocess.TimeoutExpired` and raise/exit with a stage-named, actionable (network/proxy/TLS) message. — venv/deps → `SystemExit(2)`; uv → loud message + fall back to pip.
- [x] Wrap `_warm_model`/`_build` (`:812-821`) in a wall-clock deadline (daemon worker thread joined with timeout, or equivalent); on deadline, abort with a stage-named model-download-reachability message. Ensure the existing CA-retry loop still runs inside the bounded attempt. — body renamed to `_warm_model_inner` (CA ladder intact), wrapped by `_run_in_process_with_deadline`.
- [x] Add a no-progress watchdog around the `for line in proc.stdout:` loop in `_run_indexer` (`:1398`): reset the deadline on each line; on stall, `proc.terminate()` then escalate to `proc.kill()`, drain/join, and fail loud with a disk/CPU/memory message. Preserve the existing exit-code/lock-conflict/OOM handling for normal exits. — reader-thread pumps a `queue.Queue`; `line_queue.get(timeout=stall)` per line; `_terminate_and_reap` on stall; lock/OOM/exit-code handling unchanged.
- [x] Add unit tests: each timeout path (venv/deps/model/index) fires and emits the correct stage message; within-deadline path unchanged; config override honored; missing config → defaults; stalled index child is reaped. — class `SetupPhase1DeadlineTests` (14 tests).
- [x] Document the new `docs/workflow-config.json` setup-deadline keys (defaults + meaning) in the workflow-config reference doc. — documented inline in `docs/workflow-config.json` `setup` block (keys at their defaults + a `notes` field); no standalone reference doc exists.
- [x] Run `python3 .wavefoundry/framework/scripts/run_tests.py` and `wave_validate`; fix any failures. — deferred to the coordinator's central pass (full suite not run in-lane); relevant subsets green.
...
| 2026-07-03 | Second delivery-council fix pass. Watchdog heartbeats: `indexer.py` `_StreamingLayerWriter` now prints one unconditional line per embed flush and one entering each layer's finalize — the council quantified that the per-50-file prints left silent windows (a 1024-chunk flush on a swap-thrashing host, and the entirely verbose-gated finalize tail) that could exceed the 1800s stall window on exactly the low-RAM machine class the watchdog targets; the heartbeats bound the silent window to a single forward-pass batch. Covering tests added for the previously-untested hardening branches: `test_main_graph_only_stall_timeout_exits_2_with_message` (stall `TimeoutError` → clean exit 2) and `test_run_indexer_post_eof_nonexiting_child_terminated` + a bounded-wait kwarg assertion in the within-deadline test (post-loop `proc.wait(timeout=…)` + expiry branch). `ModelPrewarmTimeout` message gained a secondary corrupted-cache hint (clear FASTEMBED_CACHE_PATH and rerun) since a corrupt cache that hangs the offline verify now times out with otherwise network-only guidance and never self-quarantines. | `tests.test_setup_index` + `tests.test_indexer` + siblings 359 OK |
| 2026-07-02 | Delivery-review fix pass (wave-council findings DF-1 + hardenings). DF-1: new `ModelPrewarmTimeout(ModelPrewarmError)` marker raised by `_warm_model`'s deadline; `_prewarm_required_model` now re-raises it BEFORE the corruption-check/quarantine/retry (the abandoned warm thread may still be writing the cache — quarantining a live cache raised a raw sharing violation on Windows and double-spawned the warm on POSIX); propagates to `main`'s existing `except ModelPrewarmError → exit 2`. Hardenings: index-build stall `TimeoutError` is now caught in `main` (both the `--graph-only` and `build_index` paths) → stage-named message + exit 2, matching the sibling-stage convention; the post-loop `proc.wait()` in `_run_indexer` is now bounded by the stall window with terminate+reap on expiry (a reader-thread failure is indistinguishable from EOF via the sentinel); `docs/workflow-config.json` `setup` block converted to notes-only (defaults named in notes, no pre-seeded numerics — aligns with `1p9iu`'s convention and the framework-owns-defaults rule). New tests: `test_prewarm_timeout_skips_quarantine_and_propagates`, `test_prewarm_timeout_is_a_prewarm_error_for_mains_handler`, `test_bootstrap_uv_forwards_configured_timeout`, `test_install_deps_forwards_configured_timeout` (the uv/deps timeout-kwarg binding gap). | `SetupPhase1DeadlineTests` 18 tests OK |
| 2026-07-02 | Implemented. Two intentional deviations from the literal plan: (1) the model-warm deadline reaches `_warm_model` via a per-run module var (`_ACTIVE_MODEL_WARM_DEADLINE_SECONDS`) set once by `main` — NOT by threading root through `prewarm_models`/`_prewarm_required_model` — because the generic `warm_fn(model_name, local_files_only=…)` contract (and the tests asserting it) must stay byte-identical; `_warm_model` also accepts an explicit `deadline_seconds` for direct testability. Consequence: every `_warm_model` caller is now deadline-bounded by the code default when the var is unset — a safety win, not new watchdog machinery for out-of-scope pipelines. (Correction, delivery review: the dashboard watcher was a wrong example — it spawns `indexer.py` and never calls `_warm_model`. Also note the facet that an operator's `setup.model_warm_timeout_seconds` override applies only via `setup_index.main`; all production paths — upgrade pipeline, MCP-triggered builds — reach `main` as subprocesses, so no production path ignores the override.) (2) A uv-bootstrap timeout prints a loud stage-named message and falls back to plain pip (uv is optional) rather than aborting; venv/deps timeouts abort with `SystemExit(2)`. | `SetupPhase1DeadlineTests` (14 tests); `test_setup_index` 123 OK; `test_indexer` 193 OK |
...
| Watchdog thread for the in-process model warm cannot truly interrupt a blocked native fastembed call. | Use a daemon worker thread joined with a deadline so the abort path returns control and fails loud even if the underlying native call is still parked; document that the process may need to be exited on the abort path rather than resumed. Delivery review (DF-1): the timeout abort must also SKIP the cache-corruption quarantine/retry, because the abandoned thread may still be writing the cache — enforced via `ModelPrewarmTimeout` and pinned by `test_prewarm_timeout_skips_quarantine_and_propagates`. |
| A stall-killed `indexer.py` leaves multiprocessing pool grandchildren alive holding RAM. | Accepted residual (delivery review): terminate/kill reaches the direct child only; workers self-exit when they notice the broken management pipe. The parent never hangs on them (bounded reader join + bounded post-loop wait). Portable process-tree kill is future hardening. |

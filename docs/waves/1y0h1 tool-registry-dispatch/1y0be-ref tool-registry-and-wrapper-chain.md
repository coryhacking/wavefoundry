...
Verification established the shape of today's code. `register_mcp_surface` in `server_impl.py` holds the implementation-owned decorated handlers as closures that call `_ensure_no_extra_args` and delegate to module-level `*_response` functions. The implementation-owned set excludes runner survivors; derive it from actual registration and the roster, not textual decorator occurrences. The three calls at the tail of `register_mcp_surface`, `_wrap_first_party_tool_costs(mcp, get_handler)`, `_wrap_lifecycle_mutation_lock(mcp, get_handler)` and `_wrap_upgrade_publication_guard(mcp, get_handler)`, apply the wrappers in that order, so cost is innermost and the guard is outermost, and a test already proves the guard never waits on the lock. The roster parity check just before those calls loads the roster with `_load_script("mcp_tool_roster")`, compares it with the runtime tool table, and is warning-only by a recorded decision that drift must never deny the whole server; `mcp_tool_roster.py` itself is untouched. Hot reload clears `_script_cache` and purges a named list of sibling modules before re-importing `server_impl`, so a new module is reload-safe when it is on that list. The RFC and kickoff propose a `server/` package; this change keeps flat sibling modules to preserve that reload model and the pack's tree-walk packaging.

Re-verified 2026-09-17, one new finding: `1y3og setup-local-reconciliation` added `setup_readiness.py`, statically imported at the top of both `server.py` and `server_impl.py`, and `runtime_advisory.py` (from the later `1y6hg` wave), statically imported only in `server.py`. Neither is in the module purge list, neither is loaded via `_load_script`, and a plain top-level `import` does not re-execute an already-cached module on `importlib.reload(server_impl)` — so both are stale across `wf_reload_mcp` today, the same gap already open for `repo_root`, `subprocess_util`, and `venv_bootstrap`. Upstream worked around the symptom rather than the gap: `_record_runner_identity` in `server.py` now explicitly re-stamps three `setup_readiness`-owned identity attributes onto the freshly-reloaded `server_impl` module after every reload, rather than making `setup_readiness.py` itself reload-safe. This does not change this change's design — `mcp_tool_registry.py` is stateless by Requirement 4 below, so it belongs on the purge list outright, no re-stamp trick needed — but it widens the case for the deferred reload-staleness follow-up in the wave watchpoints from three modules to five.

Re-verified 2026-09-19 at `f4063a85`, after waves `1y0h0 typed-phase-gates` and `1yd97 phase-gate-follow-ups` landed. Every symbol and test this change depends on still holds, and the design is unchanged. Three landed mechanisms now constrain how Requirement 3 and Requirement 4 are built, and the requirements below state that shape rather than leave it to the implementer. First, `1yd97`'s `test_provenance_rule_is_derived_and_documented_at_every_site` derives the key-less refusal codes from the functions in `server_impl.py` whose body contains `tool.fn =`, and the published tool-surface specification states that a response produced by a registration wrapper carries no `configured_gates` key; both stay true only if that rebinding stays inside the three `_wrap_*` functions. Second, the `1y0do` wrapper-order test permutes the chain by rebinding the three module-level wrapper names, so the chain must look each wrapper up by name when it is applied, and `test_lock_order_is_lifecycle_then_publication_structurally` requires the literal call `_wrap_lifecycle_mutation_lock(mcp, get_handler)` in `server_impl.py`. Third, `1y0h0`'s `test_reload_picks_up_every_added_module` requires every purge-block entry after `record_paths` to be re-imported under its public name on reload, so a module loaded through `_load_script` or imported lazily fails it. The purge block now also names `operator_identity`, `record_paths`, `lifecycle_gate_support`, `lifecycle_gates` and `sensor_runner`.
...
2. A function `build_registry(mcp, roster) -> ToolRegistry` runs once at the end of `register_mcp_surface`, after the existing decorator-based registration and after the middleware chain in Requirement 3, so each spec's callable is the callable FastMCP actually serves. `register_mcp_surface` keeps `_load_script("mcp_tool_roster")` and passes that module in as `roster`; a public-name `import mcp_tool_roster` is not used, because that module is not on the purge list and would go stale across reload. `build_registry` reads entries with the same `getattr` guards the wrappers use today, so a stub `mcp` without `_tool_manager`, or a table whose entries are bare functions, yields an empty or partial registry rather than raising. It derives one `ToolSpec` per implementation-owned entry in FastMCP's tool table (the same `mcp._tool_manager._tools` attribute the golden-snapshot test in `1xzsl-enh tool-surface-golden-snapshot` reads), taking the tier from `mcp_tool_roster.TOOL_TIERS` and the annotations and callable from the table entry. A registered tool with no roster tier, or a roster tier (outside `RUNNER_TOOLS`) with no registered tool, is recorded as a parity defect on the registry rather than raising. None of the existing `@mcp.tool` sites change: no edit to handler bodies, names, parameters, docstrings, decorators, or annotations. Runner boundary: initial `build_server` registers survivors after the implementation surface, while reload retains survivor entries. `build_registry` must explicitly exclude `RUNNER_TOOLS` on both paths rather than assume they are absent from the FastMCP table. Parity tests cover both an initial table without survivors and a reload table containing them. A tool registered directly in `server.py` must be declared in `RUNNER_TOOLS`; full-runner parity in `1xzsl` detects an undeclared extra tool. Reject no server startup solely because of a parity defect.
3. The three post-registration wrappers become a single ordered tuple `MIDDLEWARE`, declared in `server_impl.py` beside the wrappers, and applied by one call to a generic `mcp_tool_registry.apply_middleware(mcp, get_handler, MIDDLEWARE)`, composing cost innermost, lifecycle lock middle, upgrade-publication guard outermost. Each entry pairs a label with a late-binding call that spells out its wrapper call, for example `("lock", lambda mcp, get_handler: _wrap_lifecycle_mutation_lock(mcp, get_handler))`, so rebinding a module-level wrapper name takes effect and the literal lock call stays in `server_impl.py`. The three `_wrap_*` functions keep their names, `(mcp, get_handler)` signatures and bodies in `server_impl.py`, and they remain the only place `tool.fn` is rebound: `apply_middleware` never rebinds `tool.fn`, and no `tool.fn =` appears in `register_mcp_surface`. After each wrapper runs, `apply_middleware` stamps its label onto the `__wf_middleware__` tuple of every callable that wrapper replaced, detected by a change of `tool.fn` identity, so a cost-exempt tool carries only the labels of the wrappers that applied to it. `_ensure_no_extra_args` remains the first statement of each handler and is not part of the chain.
4. The registry is rebuilt on every `register_mcp_surface` call and holds no handler instance: each spec's callable resolves `get_handler()` per invocation exactly as today. `mcp_tool_registry` is appended as the last entry of the module purge list at the top of `server_impl.py` and imported by its public name at the top of `server_impl.py`, beside the lifecycle-gate imports, so `1y0h0`'s `test_reload_picks_up_every_added_module` covers its reload freshness. The module imports nothing from `server_impl` at any scope; everything it needs arrives as arguments.
5. The runtime roster check stays warning-only, but it now compares the registry against `TOOL_TIERS` from the roster module `register_mcp_surface` loads, instead of reading the runtime tool table directly, and so runs after `build_registry`; and the test suite gains a registry-based parity test beside the existing AST census. The AST census is retained in this wave and in `1y0bf`: decorated closures remain in the composition root even when response functions move.
...
- [x] AC-1: The golden tool-surface fixture is unchanged, and a test compares an AST-derived digest of every actual `@mcp.tool` handler definition in `register_mcp_surface` (decorator, signature, docstring, body) against a digest captured before this change, with identical name sets and every corresponding digest equal.
- [x] AC-2: `registry.tools()` returns specs whose names exactly equal `set(TOOL_TIERS) - RUNNER_TOOLS` and whose tiers equal the corresponding `TOOL_TIERS` values, and a test that registers one extra tool on a stub `mcp` (absent from the roster) and removes one roster entry sees both defects reported by the registry parity test.
- [x] AC-3: The wrapper-order test from `1xzsl` passes unchanged, and a new test reads `__wf_middleware__` on `wf_add_change`, the tool every one of the three wrappers applies to, through `registry.get("wf_add_change")`, and finds the tuple in the order cost, lock, guard; the same test finds only lock and guard on a cost-exempt lifecycle tool such as `wf_prepare_wave`.
- [x] AC-4: After reload, no spec is an instance of the pre-reload `mcp_tool_registry.ToolSpec` class and no spec's callable is a pre-reload closure, and a tool description change made before reload is reflected in the rebuilt registry. The description change is made in a scratch copy of the scripts tree or by altering the tool table, never by editing the live `server_impl.py`, which parallel test workers read. An identity check against the `server_impl` module object itself is not used, because reload reuses that object and the check could never fail.
- [x] AC-5: These existing tests pass unchanged: `test_upgrade_guard_is_outermost_and_never_waits_on_lifecycle_lock`, `test_roster_matches_registration_census`, `test_register_mcp_surface_consumes_the_roster`, the whole `WaveMcpReloadTests` class, `test_lock_order_is_lifecycle_then_publication_structurally`, `1yd97`'s `test_provenance_rule_is_derived_and_documented_at_every_site`, and `1y0h0`'s `test_reload_picks_up_every_added_module`. The registry-freshness test is a new class rather than an extension of `WaveMcpReloadTests`.
...
- [x] Write `mcp_tool_registry.py` with `ToolSpec`, `ToolRegistry`, `build_registry`, and the generic `apply_middleware`, plus unit tests against a stub `mcp` with a hand-built tool table.
- [x] Append `mcp_tool_registry` to the purge list and add its public-name import at the top of `server_impl.py`.
- [x] Declare `MIDDLEWARE` in `server_impl.py` and replace the three wrapper calls with one `apply_middleware` call, stamping the `__wf_middleware__` marker.
- [x] Call `build_registry(mcp, roster)` at the end of `register_mcp_surface`, after the middleware pass, and run the golden test to confirm the surface is unchanged.
- [x] Point the runtime roster warning at the registry and add the registry-based parity test.
- [x] Add the registry-freshness test as a new test class.
...
| 2026-09-19 | Readiness drift check at `f4063a85` after `1y0h0` and `1yd97` landed, run as one scoped round under the rule that readiness takes at most one round once the design is settled. Code-reviewer, qa-reviewer and red-team each approved this wave with no blocking finding. Their notes are applied here in one pass: Requirement 3 now states the only chain shape that keeps the `1y0do` permutation test, the `1yd97` provenance test and the lock-order structural test green unchanged, which three lanes each confirmed by scratch-copy mutation; Requirements 2 and 4 name the roster source, the build order and the purge placement; AC-3, AC-4 and AC-5 name their subjects and tests; line anchors are replaced with symbols | lane reports 2026-09-19 |
| 2026-09-19 | Wave opened. Task 1 done: `mcp_tool_registry.py` with `ToolSpec`, `ToolRegistry`, `build_registry` and a generic `apply_middleware`, and `tests/test_mcp_tool_registry.py` against a stub tool table; four scratch mutants (no identity check, no runner exclusion, no stub guard, labels not accumulated) each fail a test. The AC-1 baseline was captured before any `server_impl.py` edit: `tests/fixtures/register-surface-handler-digests.json` holds a SHA-256 of each of the 89 `@mcp.tool` handlers' exact source, hashed as text rather than `ast.dump` because `ast.dump` output varies across the supported Python versions. `server_impl.py` edits are held until the concurrent `1yfzu` readiness lanes, which read that file, report | `tests/test_mcp_tool_registry.py` (14 tests) |
| 2026-09-19 | Delivery round 1: code-reviewer approved; qa-reviewer blocked on two false sentences in `1ye5y-adr` (`QA-DEL-1`: the wrapper order was already stated in `build-and-verification.md` and pinned by the `1y0do` tests; `QA-DEL-2`: modules loaded through `_load_script` or lazily are re-imported on reload, and what fails is the reload test's public-name lookup). Both corrected in the record. The non-blocking gaps both lanes found were closed in the same pass: tests now pin the runtime drift warning, the registry-build fallback, `tools()` ordering, rebinding by assignment or by a literal `setattr` of `fn`, and the import placement and purge entry, and seven mutants that had survived now each fail a test; an unusable roster now yields an empty registry instead of 89 false defects; `MIDDLEWARE` moved to directly follow the wrappers; the reload probe now guards its scratch path and checks code objects. Two wording imprecisions in this document are recorded here rather than edited, since editing digested text would rotate the receipt: Requirement 3's "every callable that wrapper replaced" means the callable the wrapper installed, the 2026-09-19 Rationale paragraph's claim that a lazily imported module fails the reload test holds only for a lazy import outside re-registration, and the Scope problem statement's "documented only in a comment" and "parity is checked by parsing source" omit the pre-existing `build-and-verification.md` statement and the `1y0do` runtime tests, as `QA-DEL-1` corrected in the decision record | `events.jsonl` findings `QA-DEL-1`, `QA-DEL-2`; `tests/test_mcp_tool_registry.py` (27 tests) |
| 2026-09-19 | Delivery scoped reverification: qa-reviewer replayed `QA-DEL-1` and `QA-DEL-2` against their original reproductions and both are closed; code-reviewer read the repair diff and confirmed its round-1 approval holds, finding every tool's served wrapper chain and the reload probe byte-identical across HEAD, round 1 and the repair. Delivery converged in the one scoped round. Notes carried, not repaired after approval: the rebinding test does not catch `object.__setattr__` or a `for`-loop target; the late binding of a single `MIDDLEWARE` entry, an empty `TOOL_TIERS` mapping, and a roster without `RUNNER_TOOLS` are not individually pinned | `events.jsonl` reverifications of `QA-DEL-1` and `QA-DEL-2` |

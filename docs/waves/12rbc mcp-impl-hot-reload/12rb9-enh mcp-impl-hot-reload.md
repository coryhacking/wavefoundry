...
**Plan only.** Do not edit `.wavefoundry/framework/scripts/` for this change until **Implement wave** on `12rbc` with `framework_edit_allowed` open. Requirements, ACs, and tasks below are the specification; no code is shipped until that gate.
...
The fix is to split `server.py` into a thin runner that owns the stdio transport and fixed tool registrations, and a separate `server_impl.py` that contains all business logic. The runner can `importlib.reload(server_impl)` to swap in new implementation without closing the MCP connection. A `wave_hot_reload` tool performs the reload and returns version fields so operators can confirm the running impl matches disk; `wave_upgrade` cleanup triggers the same in-process reload path.
...
4. A `wave_hot_reload` MCP tool reloads `server_impl` via `importlib.reload`, closes the old index/cache state, and instantiates fresh state from the new module code. It returns structured version fields (see requirement 9).
5. After `wave_upgrade(phase="cleanup")` completes successfully, the **in-process** server path invokes hot reload (same semantics as `wave_hot_reload`). `upgrade_wavefoundry.py` updates the operator summary to reflect reload instead of "restart MCP server" — it does **not** spawn an MCP client (the cleanup subprocess is not attached to the live stdio server).
6. On reload, clear `_script_cache` (see `server.py` `_load_script`) so dynamically loaded sibling scripts do not stay stale.
7. New tools added in an upgrade are not visible to an existing client session until reconnect — this is an accepted limitation; `wave_hot_reload` handles logic/index changes only.
...
9. **Dual module versions** — queryable over MCP to confirm an upgrade was applied to the **running** process:
   ...
   - `wave_hot_reload` returns the same version fields on success so operators and agents can verify reload without a separate call:
...
- `wave_hot_reload` tool implementation in `server.py` (mutating annotations; version payload on success)
...
- `test_server_tools.py` updated to assert `wave_hot_reload` in the registered-tool list and version fields in responses
...
- AC-2: `wave_hot_reload` returns `ok: true` plus `framework_version`, `server_runner_version`, `server_impl_version`, and `impl_matches_disk`; the next tool call executes code from the freshly loaded `server_impl.py`.
- AC-3: After `wave_upgrade(phase="cleanup")` succeeds, the in-process server invokes hot reload (same as `wave_hot_reload`); `wave_server_info` then shows `impl_matches_disk: true` when the upgraded pack is loaded (no manual MCP restart).
- AC-4: All existing `test_server_tools.py` and `test_upgrade_wavefoundry.py` tests pass.
- AC-5: `wave_hot_reload` is present in the registered-tool assertion in `test_server_tools.py`.
...
- Implement `wave_hot_reload` in `server.py` (`_MUTATING_TOOL`; reload lock; clear `_script_cache`; return version payload per requirement 9)
...
- Add `wave_hot_reload` to registered-tool assertion in `test_server_tools.py`
- Test `wave_hot_reload` response includes version fields and `impl_matches_disk` after reload; test `wave_server_info` version fields
...
- Clear `_script_cache` inside `wave_hot_reload` before `importlib.reload(server_impl)`
...
| wave_hot_reload     | wave-coordinator | server split  | Tool impl + close/reload logic |
| upgrade integration | wave-coordinator | wave_hot_reload | in-process reload from `wave_upgrade_response`; summary text in upgrade script |
...
- Version helpers (`_read_framework_pack_version`, `_read_framework_version_at_root`) must exist before `wave_hot_reload` / `wave_server_info` version ACs
...
| 2026-05-19 | Dual versions via `wave_hot_reload` + `wave_server_info` | Operator verifies upgrade without restart; runner vs impl distinguishes hot-reload scope | Single `version` string only (rejected: ambiguous after split) |
...
| 2026-05-19 | Version fields on `wave_hot_reload` and `wave_server_info` | Primary upgrade verification path; reload response avoids extra round-trip | Disk-only VERSION in upgrade log (rejected: does not prove in-memory reload) |
...
| `_script_cache` survives `importlib.reload(server_impl)` | Clear cache at start of `wave_hot_reload` | |
...
| `impl_matches_disk` false after upgrade | Operator calls `wave_hot_reload` or completes `wave_upgrade` cleanup path; check `wave_hot_reload` return payload | |
| Reload mid-request | Serialize reload on a module lock inside `wave_hot_reload` | |

...
3. The full-corpus `run_validate` callers must be **unchanged** — they continue to run the full scan; only the post-write attachment path is rescoped. Besides `_run_post_write_lint`, there are **six** `run_validate` callers (readiness re-review census, confirmed at `server_impl.py:6251,6432,6501,9042,9267,9725`): the five lifecycle gates (`wave_validate`, `wave_prepare`, `wave_review`, `wave_close`, `wave_install_audit`) **plus `wave_audit`** (the combined-readout tool, `:6251` — omitted from the original plan's census). All six stay full-corpus.

...
Two ordered changes: the standing retrieval eval suite (golden corpus over nine query classes including verbatim misranked queries, abstention controls, and locally curated agentic fix-localization cases; one-command local run; recorded baseline), then eval-gated classifier/ranking improvements whose merge evidence IS the suite's before/after report. The model remains the current approximately 33.2M-parameter local embedder; hosted inference and 0.3B-class model adoption are not part of this wave.

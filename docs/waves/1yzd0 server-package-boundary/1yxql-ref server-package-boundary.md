...
- [x] Prepare and independently review the admitted wave before any repository code edit.
- [x] E0: make the retrieval evaluator layout-independent; focused independent review; operator commit of E0.
- [x] E1: refresh the index and record the approved baseline receipt on the E0 commit.
- [x] Create the package and alias table; move the 12 modules with body-equivalence evidence; apply scripts-root (with the harness-coherence carve-out), purge, eager-alias and `SOURCE_FILES` adaptations.
- [x] Adapt test seams: hollow loaders, the `framework_source_files()` helper and census, scratch copies, redirect patches, the source-path helper and census, the rewritten reload census; add discriminating tests for each silent degradation.
- [x] Retarget memory records that name moved flat paths.
- [x] Verify a scratch-built pack, fresh-install and 1.25/1.26 tag-sourced upgrade fixtures (including dashboard running), public MCP contracts, extension behavior and failure paths.
- [x] M then E2: incremental index update and the after receipt; graph ownership and call-edge verification.
- [x] Update ADR 1ye5y (or a successor), architecture/spec/build/evaluation guidance, the doc-citation census and changelog; regenerate maps through their producer; open the `upgrade_handlers` second-instance follow-up plan.
- [x] Run required delivery lanes and final framework and docs gates; hand off for operator closure.
...
| 2026-09-25 | E0 committed by the operator (`2ce2e953`). E1 recorded: `docs/reports/retrieval-quality-1yzd0-e1c.json`, verdict `baseline`, no invalidation or operator-review reasons, stable complete generation 2098, production identity bound to HEAD `2ce2e953` with a clean scripts tree, flat module paths, empty relevance resolution, fixture digest unchanged. Two earlier attempts were invalid and are not evidence: `retrieval-quality-1yzd0-before.json` (`index_not_ready`, a background refresh started mid-run) and a preflight `stale_index` refusal. Cause: the context-efficiency projection rewrites the open wave's `wave.md` after each MCP call, which stales the index and schedules a refresh. Recovery that worked: an index update and the evaluator run back to back as CLIs with the tool venv and no MCP calls in between. The evaluator workflow doc (Requirement 11) will state this, and E2 uses the same recipe | docs/reports/retrieval-quality-1yzd0-e1c.json |
| 2026-09-25 | M implemented in the working tree: 12 modules `git mv`-ed into `wf_server/` (empty `__init__.py`), exact three-line flat aliases, `_FLAT_ALIASES` table with derived dual-key `_PACKAGE_PURGE_KEYS`, guarded eager registration after the last module-top import, `SCRIPTS_DIR` at the scripts root with the harness-coherence carve-out, handler imports canonical (`from wf_server import server_impl`), `SOURCE_FILES` adds only `wf_server/server_impl.py`, `wf_server` reserved for extensions (RENAME-1). Censuses landed red against the moved tree first: the move turned 26 test files red, and the new flat-glob and source-read censuses (`test_server_package`) flagged 7 flat globs and 13 computed or literal reads, including silent passes (`assertNotIn` against the alias in `test_build_pack`, `test_review_policy`, `test_server_tools_retrieval`, `test_runtime_lock`, `test_retrieval_candidate_generation`, `test_graph_query`) and a lost redirect in `test_install_resume_integration`. Defect found by the existing reload test and fixed: `from wf_server import mcp_tool_registry` reuses the stale module bound on the never-evicted parent package after the purge; the import is now `import wf_server.mcp_tool_registry as mcp_tool_registry`, pinned by the placement test and a mutant. Reload census rewritten (`test_lifecycle_gates_structure`), hollow loaders repaired, full suite 9655 tests OK. Nineteen mechanism mutants each killed by an assertion | evidence/m/mutants.out |
| 2026-09-25 | Move/body equivalence: every top-level definition of the 12 moved modules is AST-identical to `2ce2e953` after the documented mechanical rewrites (canonical imports, `server_impl.SCRIPTS_DIR`), except the nine inventoried `server_impl` functions and its module statements (alias table, derived purge keys, `SCRIPTS_DIR`, canonical imports, eager registration); zero unexpected differences. Harness-coherence live tools 113 before and after (reading the alias would give 1) | evidence/m/equivalence.out; evidence/m/module_statements.diff; evidence/m/live_tools.out |
| 2026-09-25 | Pack and old runners: a pack built from a scratch copy carries `wf_server/` and every flat alias. Installed runners from `git archive v1.25.0` and `v1.26.0` in paths with a space accepted it, extracted it and (with `--confirm-hosts-stopped`, so the whole old-code window ran in the installed process) reached the ordinary historical-memory gate; each upgraded tree passes `server.py --dry-run`, answers a stdio `wf_server_info` with `1.27.0+m1yzd0`, and imports one module object per name. Dashboard-running case for both: the installed runner took its pause path, and the new flat `dashboard_handlers` stops and restarts it from the package. The 1.25 runner's 10 in-process memory-backfill failures (`index_runtime_stale: chunker`) reproduce identically with a pre-move pack, so they are pre-existing. Fresh install from the pack reaches a working server | evidence/m/old_runner.py and its .out files; evidence/m/fresh_install.out |
| 2026-09-25 | M then E2: the incremental update and graph rebuild resolve every moved definition to its package file (definition counts identical per module, e.g. 550 for `server_impl`), 248 handler-to-`server_impl` call edges before and after, no flat-path definition ownership left, alias modules kept; live `code_definition` after `wf_reload_mcp` answers package paths. E2 `docs/reports/retrieval-quality-1yzd0-e2b.json`: `pass` against E1, same evaluator identity and fixture digest, no violations, zero metric differences. A first E2 attempt ran while the scratch baseline graph build shared the machine and reported only latency (quality identical); it and the two E1 attempts are not evidence and were removed from `docs/reports` | evidence/m/graph_ownership.out; docs/reports/retrieval-quality-1yzd0-e2b.json |
| 2026-09-25 | Docs: successor ADR `1yx4m` (1ye5y superseded for placement), architecture docs (current-state, domain-map, layering-rules invariants), spec downstream guidance (merge procedure, reserved name, restart expectations), evaluator workflow doc (E2 reference, E0 resolution, CLI recipe), retention record, changelog, seed 160 (gate opened and closed), code map regenerated. Doc census re-derived: 82 living citations moved to package paths; remaining flat citations are historical ADR text and dated history. Memory: 25 active records with full flat targets retargeted; 7 bare-basename targets already match the package file. Follow-up plan `1yxnc-bug post-upgrade-reload-imports-second-runner` opened for readiness finding A4. Reload probes now cover `codenav_handlers` too | docs/architecture/decisions/1yx4m-adr wf-server-package.md; docs/plans/1yxnc-bug post-upgrade-reload-imports-second-runner.md |
| 2026-09-25 | Tested platform for the pack, fresh-install, old-runner and E1/E2 evidence: macOS 27.2 on arm64 with CPython 3.13.5 (the tool venv), paths containing a space. Native Windows is not claimed; the alias and scripts-root mechanisms use only `importlib`, `sys.modules` and `pathlib`, and the readiness demonstration ran on CPython 3.11 and 3.13 | evidence/m/*.out |
| 2026-09-25 | Delivery review round 1: all six lanes and the red-team seat approve; no blocking finding. Non-blocking findings repaired in one round: RT-DEL-1 (a restored full flat copy ran a second `server_impl` and broke reload silently): the package now refuses at import when a flat alias file is not the exact alias, checked on the file with newlines normalized so an older runner's in-place reload still works; RT-DEL-2 / SEC-DEL-1: all twelve moved names reserved for extensions, pinned to `_FLAT_ALIASES`; CODE-DEL-1: structural test that no package module resolves paths through `server_impl.__file__` and `__file__` appears only in `SCRIPTS_DIR` and the carve-out; QA-DEL-1/2: censuses cover `joinpath`, reads through a variable, `tokenize.open`/`io.open`, `iterdir`, `os.listdir`/`os.scandir`, `glob.glob` and `parents[1]`/`SCRIPTS_PATH` roots, with the `__file__` exemption narrowed to loaded package modules; the stronger censuses found one more hollow loader (`test_graph_incremental_merge`) and an intended alias read, now spelled `shipped_path`; QA-DEL-3: five computed-name loaders return `sys.modules[name]`, plus a loader census; QA-DEL-4: `sqlite_event_eval` hashes through `source_path`; QA-DEL-5: `source_path` returns non-module names unchanged; DOCS-DEL-1: ADR 1u49j reference repointed. 28 mutants each killed by an assertion; full suite 9661 tests OK | evidence/m/mutants.out |
| 2026-09-25 | Delivery reverification: each original reviewer re-checked its own findings against the repaired tree and approves (code, security, QA, architecture, docs-contract, release, red-team seat). Upgrade matrix re-run on the repaired code: fresh install and v1.25.0/v1.26.0 installed runners (dashboard running) reach a working server as `wf_server.server_impl`; the new flat-file refusal does not fire on packed, upgraded or CRLF trees, nor on an older runner's in-place reload. Residual notes, not findings: the refusal fires at the first handler call rather than at startup (startup tolerates torn upgrade trees); a few static census shapes (`rglob` filtered to the root, literal-path spec loads, subprocess-embedded code) are not covered and do not occur in the tree | evidence/m/old_runner_after_repair.out |

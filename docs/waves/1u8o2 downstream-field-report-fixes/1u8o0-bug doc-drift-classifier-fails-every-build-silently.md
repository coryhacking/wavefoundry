...
- [ ] AC-1: The reproduced trigger is fixed red-first: the synthetic living-doc-deletion (and
  ...
  absorbed.
- [ ] AC-2: One representative injection per failure class (subprocess error, timeout, malformed
  output, deletion frame) produces a distinct stage-plus-reason log line; the static three-way
  parenthetical is gone.
- [ ] AC-3: From the FIRST failed evaluation, the store records and `wf_audit` reports the
...
- [ ] AC-4: The spec's `doc_drift` section is updated in the same change: additive shape, the
...
- [ ] AC-5: The five named `test_doc_drift.py` re-points land as re-points; the two shape pins
  stay green; full framework suite passes.

## Tasks

- [ ] Encode the red-first deletion-frame repro as a regression test (probe fixture exists)
- [ ] Fix deletion/rename frame handling in the parser
- [ ] Thread per-return-site reason codes to the log line; remove the static parenthetical
- [ ] Add the evaluation state to store meta and the additive `wf_audit` extension
- [ ] Update `docs/specs/mcp-tool-surface.md` (Required): shape, `available` relationship,
      drift-never-blocks-ready restated
- [ ] Re-point the five named detector-failure stubs; verify the two shape pins stay green
- [ ] Injection tests per failure class; staleness-surface tests; full suite

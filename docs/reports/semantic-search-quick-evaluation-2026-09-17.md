...
Compact candidate identities, raw scores, public top-five results, timings, queries and the production source hash are retained in [probe data](semantic-search-quick-evaluation-2026-09-17.json). Full scratch captures and the exploratory driver remain under `/private/tmp/wf-search-quick-*`; they are not maintained test infrastructure or a reproducible frozen corpus. This report does not alter the closed memory wave's qualification.

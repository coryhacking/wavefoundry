...
2. When `wave_review.enabled` is true, confirm both `wave-council-readiness` and `wave-council-delivery` are recorded (typed approval events on declared waves, projected into `## Review Evidence`; prose lines count only on legacy waves).

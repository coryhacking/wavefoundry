...
2. When `wave_council_policy.enabled` is true, confirm both `wave-council-readiness` and `wave-council-delivery` are present in `## Review Evidence`.
...
6. Promote durable memory if applicable.

...
- Upgrade never generates or rewrites the trio; it points operators here. The `wf-techdocs` skill renders wherever `docs/prompts/refresh-techdocs.prompt.md` exists (every target after seed-100 reconciliation); on the upgrade that first ships seed `178`, backfill that prompt and re-run `wf render-surfaces` so the skill renders in the same upgrade.
...
Framework source: `.wavefoundry/framework/seeds/178-refresh-techdocs.prompt.md` (public shortcut band). In this repository the dogfood run of this workflow is recorded in wave `1vj4e backstage-techdocs-baseline`, change `1vmpz`.

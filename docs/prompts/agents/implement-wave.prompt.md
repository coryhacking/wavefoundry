...
- **Docs gate:** routine `docs/` edits receive automatic incremental changed-set lint; fix its findings as they arise, but do not run a full gate after every edit. Before handoff or declaring implementation complete, prefer full MCP **`wf_validate_docs`** (and **`wf_garden_docs`** when metadata needs refresh). **CLI-only fallback:** `wf docs-lint` when MCP is not attached.

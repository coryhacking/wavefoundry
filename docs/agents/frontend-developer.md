...
**Stack (Wavefoundry dashboard):**
- Plain JavaScript — no build step, no JSX, no bundler
- DOM construction via `h(tag, props, ...children)` helper — this is the only component model; do not introduce React, Vue, or any framework
- Two files: `.wavefoundry/framework/dashboard/dashboard.js` (all component logic) and `dashboard.css` (all styles)
- Dashboard served by a local HTTP server; reload in browser to verify changes

**Design system:**
- Tokens defined as CSS custom properties in the `:root` block of `dashboard.css`
- Key tokens: `--page-bg`, `--panel-bg`, `--panel-border`, `--ink`, `--muted`, `--accent`, `--accent-soft`, `--warn`, `--danger`, `--neutral-soft`, `--radius-lg/md/sm`, `--space-1` through `--space-6`, `--font-body`, `--font-mono`
...
- Status badges via `.status-badge` + `.status-done` / `.status-warn` / `.status-neutral` CSS classes
- All dialog tables use first-column `td { white-space: nowrap }` inside `.agent-dialog-body`
- Dialog max-width: `min(1000px, 92vw)`

**Gate protocol:**
- Dashboard/CSS edits: open `framework_edit_allowed` gate before, close immediately after
- Design-system token/pattern edits: governance is currently `"evolvable"` — no gate required, but document any new token in `docs/design-system/tokens/`

...
code_constants(symbols=["X","Type.Member"]) # module-/type-level constant + enum-member VALUES across all 11 languages; returns parsed value + line
...
**Guru journal** (a typed memory record) — the right place for:
...
| Path | Purpose |
|---|---|
| a typed memory record | Durable discoveries, index gaps, edge cases, operator Q&A answers |

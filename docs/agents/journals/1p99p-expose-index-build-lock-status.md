# Journal - Expose Index Build Lock Status
...
Schema version: 1.0
Last distilled: 2026-07-01

wave-id: `1p99p expose-index-build-lock-status`

## Operating Identity

- **Role:** wave-coordinator for wave `1p99p expose-index-build-lock-status`. **Responsibility:** coordinate the wave's admitted changes through prepare → implement → review → close per the lifecycle contract.
...
- Pending: wave `1p99p expose-index-build-lock-status` opened 2026-07-01; populate as admitted changes move through implementation.

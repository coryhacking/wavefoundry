# Journal - Hardware Aware Embedding Providers
...
- **Role:** wave-coordinator for wave `1p4u5 hardware-aware-embedding-providers`. **Responsibility:** coordinate the wave's admitted changes through prepare → implement → review → close per the lifecycle contract.
...
- Pending: wave `1p4u5 hardware-aware-embedding-providers` opened 2026-06-11; populate as admitted changes move through implementation.

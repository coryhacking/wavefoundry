# Journal - Install Upgrade Root Cleanup
...
- **Role:** wave-coordinator for wave `1rycd install-upgrade-root-cleanup`. **Responsibility:** coordinate the wave's admitted changes through prepare → implement → review → close per the lifecycle contract.
...
- Pending: wave `1rycd install-upgrade-root-cleanup` opened 2026-07-06; populate as admitted changes move through implementation.

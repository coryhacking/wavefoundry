# Journal - Indexer Drift Skips Empty Files
...
Schema version: 1.0
Last distilled: 2026-06-05

wave-id: `1p3iv indexer-drift-skips-empty-files`

## Operating Identity

- **Role:** wave-coordinator for wave `1p3iv indexer-drift-skips-empty-files`. **Responsibility:** coordinate the wave's admitted changes through prepare → implement → review → close per the lifecycle contract.
...
- Pending: wave `1p3iv indexer-drift-skips-empty-files` opened 2026-06-05; populate as admitted changes move through implementation.

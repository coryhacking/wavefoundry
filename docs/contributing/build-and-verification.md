...
`wave_index_build` accepts: `content` (`docs` | `code` | `all` | `graph`), `mode` (`update` | `rebuild`). It targets the single project index/graph (the separate framework layer was removed in wave 1p4ww). Successful responses include structured `stats` confirming file count, chunk count, and whether the run was already up to date.

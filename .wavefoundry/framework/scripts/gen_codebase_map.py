...
class CodebaseArea:
    ...
    name them from the map. Stable, deterministic, derived purely from the
    persisted artifacts.
    """
...
def _area_context_rel_path(area: CodebaseArea) -> str:
    ...
    from. Map rendering uses :func:`_resolve_area_context_rel_path`, which walks
...
def _resolve_area_context_rel_path(root: Path | None, area: CodebaseArea) -> str | None:
    """The repo-root-relative ``AGENTS.md`` path that an area should name.
    ...
    reference robust to representative-path churn across rebuilds.
    ...
    area and ``wavefoundry://agents``, and naming it from every unrelated area is
...
def render_markdown(
    ...
    *,
    ...
    When ``root`` is supplied, each area names the nearest ancestor
    ...
    lines.append(
        ...
        "do not edit by hand; refresh through create-mode prepare-and-open, create-mode close, "
        "upgrade, forced on-demand `index_build(content='map')`, the direct change-only CLI, "
        "or the missing-file resource fallback. -->"
    )
    lines.append("")
    lines.append(
    ...
    lines.append(
        "- The map refreshes during create-mode prepare-and-open or close, upgrade, "
        "forced `index_build(content=\"map\")`, the change-only generator CLI, or a "
        "missing-file resource fallback. Ordinary index builds and reads of an existing "
        "map do not regenerate it."
    )
    lines.append("")
    ...
    for area in model.areas:
        ...
        # Per-area AGENTS.md path ONLY when a file actually exists on disk.
        # Resolution walks UP to the nearest ancestor
        # AGENTS.md (1p66d) so conventionally project-root-placed files are found.
        ctx_rel = _resolve_area_context_rel_path(root, area)
        if ctx_rel:
            lines.append(
                f"- Area context: `{ctx_rel}` — conventions/gotchas; "
...
def generate_safe(
    ...
    """Fail-safe wrapper for the callers that own map regeneration.

    A generator error must NEVER fail its caller, so all exceptions are
    ...
    try:
        ...
        if verbose:
            print(
                f"codebase-map: regenerated → {OUTPUT_REL_PATH}",
                flush=True,
            )
        return True
        ...
        if verbose:
            print(
                f"codebase-map: generation skipped ({exc})",
                file=sys.stderr,
                flush=True,
            )
        return False

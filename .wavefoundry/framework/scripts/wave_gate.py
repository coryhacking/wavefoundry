...
``<repo-root>/.wavefoundry/guard-overrides.json``. Invoked by the
``.wavefoundry/bin/wave-gate`` shell launcher (generated by
``render_bin_launchers()`` in ``render_platform_surfaces.py``) and usable
standalone with ``python3 -m`` or direct invocation.
...
Usage:
    wave-gate open <gate-name>     Enable a guard (error if already open)
    wave-gate close <gate-name>    Disable a guard (advisory if already closed)
    wave-gate status               Show the current state of all gates
    wave-gate --root <path> ...    Override the repo-root resolution

...
def _run_incremental_checks(root: Path):
    ...
    if changed_docs:
        ...
        # 1v4mt: the second rendered marker family, same disposition and same
        # rendered-side-only reasoning as the line above.
        failures.extend(check_review_protocol_carrier_parity(root, only=changed_docs))

...
_SQL_RELATION_KEYWORD_STOPLIST = frozenset({
    ...
    "json", "jsonb", "array",
    # Oracle + T-SQL builtin type keywords (wave 1rvjs/1rvdq dialect fold-in) —
    # the 1rvdq column-type suppression only skips type names already known here;
    # these are the common Oracle / SQL Server types that otherwise parse as
    # object_references and leak phantom `reads` on non-Postgres schemas. Same
    # accepted precision-over-recall trade-off as the base list (a table literally
    # named `money`/`image`/`number`/`long`/`raw`/`geography`/`geometry` is
    # suppressed — type usage dominates, as with the base `text`/`date`/`json`).
    # Oracle:
    "varchar2", "nvarchar2", "number", "nclob", "rowid", "urowid", "long",
    "raw", "bfile", "xmltype", "binary_float", "binary_double",
    # SQL Server / T-SQL:
    "nchar", "ntext", "image", "money", "smallmoney", "datetime2",
    "datetimeoffset", "smalldatetime", "uniqueidentifier", "sql_variant",
    "hierarchyid", "geography", "geometry", "bit",
    # pseudo-types (wave 1rvdq) — a routine signature's `anyelement`/`record`/… is
    # a type position, never a table read.
    "anyelement", "anyarray", "anynonarray", "anyenum", "anyrange",
    "anycompatible", "anycompatiblearray", "anycompatiblenonarray",
    "anycompatiblerange", "anycompatiblemultirange", "anymultirange",
    "record", "void", "cstring", "internal", "language_handler",
    "fdw_handler", "index_am_handler", "tsm_handler", "event_trigger",
    # row-locking clause keywords (wave 1rvdq) — `SELECT … FOR UPDATE SKIP LOCKED
    # / NOWAIT` shreds `skip`/`locked`/`nowait` out as bare tokens that are never
    # a table reference. (`share` is deliberately NOT listed — a plausible real
    # table name; `FOR SHARE` is left to the locking-clause region check.)
    "skip", "locked", "nowait",
    # Oracle `DUAL` dummy table (wave 1rvky) — `SELECT … FROM dual` is Oracle's
    # one-row scratch table, never a real user table; suppress the read.
    "dual",
})
...
def _resolve_doc_path_ref(href: str, rel_path: str, current_paths: set[str] | None) -> str | None:
    ...
    for part in raw.split("/"):
        if part == "..":
            ...
            parts.append(part)
    resolved = "/".join(parts)
    if not resolved or resolved == rel_path or (current_paths is not None and resolved not in current_paths):
        return None
    return resolved


def _extract_doc_backtick_paths(source_text: str, rel_path: str, current_paths: set[str] | None) -> list[str]:
...
def _extract_doc_links(source_text: str, rel_path: str, current_paths: set[str] | None) -> list[str]:
    """Return normalized links; ``None`` retains candidates absent from the corpus."""
    targets: list[str] = []
    seen: set[str] = set()
...
def _is_legacy_memory_pointer_path(rel_path: str) -> bool:
    """Retired per-record pointers are generated migration residue, not graph input."""
    return rel_path.replace("\\", "/").startswith(
...
def _kind_for_path(rel_path: str) -> str:
    ...
    if rel.startswith(".wavefoundry/framework/seeds/"):
        return "seed"
    if rel == "docs/agents/memory-archive.md":
        # The compact archive register is the searchable history pointer for
        # excluded archive bodies. Keep it as a typed memory node even when it
        # has no resolvable targets; ordinary zero-edge docs are pruned below.
        return "memory"
    name = Path(rel).name
    ...
    if suffix in _CODE_EXTENSIONS:
        return "code"
    if rel.startswith("docs/agents/memory/") and Path(rel).name != "README.md":
        # Agent memory records (wave 1ro44 / 1p8gy): typed nodes so consumers
        # can query memory attachment distinctly from generic docs.
        return "memory"
...
def _pending_doc_link_repairs(
    merge_state: Any, current_paths: set[str], unreadable_dirs: set[str] | None = None,
) -> set[str]:
    """Select retained document obligations without reading document rows/text."""
    if not isinstance(merge_state, dict) or not isinstance(merge_state.get("files"), dict):
        return set()
    return {
        rel for rel, entry in merge_state["files"].items()
        if rel in current_paths
        and not _path_under_unreadable(rel, unreadable_dirs)
        and isinstance(entry, dict)
        and entry.get("kind") in {"doc", "seed", "memory"}
        and current_paths.intersection(entry.get("unresolved_doc_targets") or [])
    }


def _valid_doc_link_merge_state(merge_state: Any) -> bool:
    """Whether this summary can prove pending-link eligibility or idleness."""
    return bool(
        isinstance(merge_state, dict)
        and merge_state.get("format") == _MERGE_STATE_FORMAT
        and not merge_state.get("locality_violation")
        and isinstance(merge_state.get("files"), dict)
        and merge_state.get("payload_fingerprint")
    )
...
def read_pending_doc_link_repairs(
    *, index_dir: Path, current_paths: set[str], walker_version: str,
    chunker_version: str, layer: str = "project", unreadable_dirs: set[str] | None = None,
) -> dict[str, Any] | None:
    """Read-only idle-build preflight; never opens/repairs a GraphStateStore.

    A version mismatch requests normal re-extraction. Missing/unreadable state
    returns None (no claim of pending link work). A valid snapshot can be passed
    ...
    return {
        ...
        "pending_docs": sorted(_pending_doc_link_repairs(merge_state, current_paths, unreadable_dirs)),
...
_SQL_RECOVERY_KIND_BY_WORD = {
    ...
    "domain": "domain",  # wave 1rvdq — CREATE DOMAIN is an ERROR region (grammar has no create_domain)
    "type": "type",      # wave 1rvdq — CREATE TYPE parses natively, but recover it if it ever lands in an ERROR
}
_SQL_RECOVERY_ROUTINE_KINDS = frozenset({"procedure", "function", "trigger"})
# Benign ERROR-region patterns (wave 1rvdq) that must NOT inflate the
# `unrecovered_regions` loudness count: `CREATE OPERATOR` (the grammar has no
# create_operator node — recognized, its `procedure =` fn is out of the
# reads/writes lineage model), and a bare row-locking clause (`FOR UPDATE [SKIP
# LOCKED|NOWAIT]` / `FOR SHARE` / `FOR NO KEY UPDATE` / `FOR KEY SHARE`) that
# tree-sitter-sql shreds off the tail of a SELECT into its own ERROR span.
_SQL_BENIGN_REGION_RE = re.compile(
    r"^\s*(?:"
    # CREATE OPERATOR … up to its own statement terminator ONLY (`[^;]*`, not
    # `.*` — a `.*` with DOTALL would swallow a following unrecoverable CREATE
    # that tree-sitter merged into the same ERROR node and wrongly mark it
    # benign, undercounting `unrecovered_regions`; code-review lane 2026-07-06):
    r"CREATE\s+OPERATOR\b[^;]*"
    # a row-locking clause TAIL — tree-sitter shreds it off a SELECT, and the
    # leading `FOR` is often consumed, so match with or without it:
    r"|(?:FOR\s+)?(?:UPDATE|SHARE|KEY\s+SHARE|NO\s+KEY\s+UPDATE)"
    r"(?:\s+OF\s+[\w.\",\s]+?)?(?:\s+(?:SKIP\s+LOCKED|NOWAIT))?"
    r"|SKIP\s+LOCKED|NOWAIT"
    # T-SQL `GO` batch separator (wave 1rvky) — its own ERROR node between
    # statements; recognized-benign so it does not inflate unrecovered loudness.
    r"|GO"
    r")[\s;]*$",
    re.IGNORECASE,
)
...
def _sql_region_is_benign(region_text: str) -> bool:
    """True when an ERROR region is a recognized-benign construct (CREATE OPERATOR
    or a bare row-locking clause) that should not count as `unrecovered` loudness."""
    return bool(_SQL_BENIGN_REGION_RE.match(region_text.strip()))
...
_SQL_RECOVERY_CREATE_RE = re.compile(
    ...
    r"(?P<kind>MATERIALIZED\s+VIEW|PROCEDURE|FUNCTION|TRIGGER|TABLE|VIEW|DOMAIN|TYPE|(?:UNIQUE\s+)?INDEX)\s+"
...
# wave 1rvky — a `keyword_references`-preceded name in an ERROR region: the
# column-list-less / parenthesized-dialect-type FK-drop case (`amt number(10,2),
# fk int REFERENCES parent (id)` desyncs the parse and the trailing columns spill
# into a top-level ERROR where `parent` parses as a bare identifier, so the node
# path never mints it). Text-regex over masked ERROR text recovers each FK read.
_SQL_RECOVERY_REFERENCES_RE = re.compile(
    r"\bREFERENCES\s+(?P<name>[A-Za-z_#@\[\"`][\w.$#@\]\"`]*)", re.IGNORECASE)
# wave 1rvky — T-SQL `MERGE t USING s …` WITHOUT `INTO`: the whole statement
# fails to parse and the tree drops every table name, so ONLY a raw-text sniff
# on the ERROR region can recover the target (write) + source (read).
_SQL_RECOVERY_MERGE_RE = re.compile(
    r"\bMERGE\s+(?:INTO\s+)?(?P<target>[A-Za-z_#@\[\"`][\w.$#@\]\"`]*)"
    r"\s+USING\s+(?P<source>[A-Za-z_#@\[\"`(][\w.$#@\]\"`]*)", re.IGNORECASE)
...
def _sql_recover_error_region(region_text: str, start_line: int) -> dict[str, Any]:
    ...
    # wave 1rvky — region-level (not line-anchored) recoveries over the masked
    # ERROR text. Run only if the line scan recovered nothing structural, so a
    # normal recovered CREATE is not double-processed by a stray REFERENCES.
    if not result["definitions"]:
        # R4: FK targets orphaned into this ERROR by a parenthesized-dialect-type
        # desync (or a column-list-less REFERENCES that landed at top level).
        for m in _SQL_RECOVERY_REFERENCES_RE.finditer(masked):
            fk_name, fk_temp = _sql_recovery_clean_name(m.group("name"))
            if fk_name and not fk_temp and fk_name.casefold() not in _SQL_RELATION_KEYWORD_STOPLIST:
                result["references"].append({
                    "name": fk_name, "schema": _schema_of(fk_name),
                    "direction": "read", "clause": "references",
                    "statement": "create_table", "owner": None,
                    "extraction": _SQL_RECOVERY_MARKER,
                })
        # R6: T-SQL MERGE without INTO — target is a write, source a read.
        mg = _SQL_RECOVERY_MERGE_RE.search(masked)
        if mg is not None:
            tgt, tgt_temp = _sql_recovery_clean_name(mg.group("target"))
            src, src_temp = _sql_recovery_clean_name(mg.group("source"))
            if tgt and not tgt_temp and tgt.casefold() not in _SQL_RELATION_KEYWORD_STOPLIST:
                result["references"].append({
                    "name": tgt, "schema": _schema_of(tgt), "direction": "write",
                    "clause": "merge_into", "statement": "merge", "owner": None,
                    "extraction": _SQL_RECOVERY_MARKER,
                })
            if src and not src_temp and src.casefold() not in _SQL_RELATION_KEYWORD_STOPLIST:
                result["references"].append({
                    "name": src, "schema": _schema_of(src), "direction": "read",
                    "clause": "merge_using", "statement": "merge", "owner": None,
                    "extraction": _SQL_RECOVERY_MARKER,
                })
    return result
...
def _sql_analyze_program(root_node, source_bytes: bytes, *, recover: bool = True) -> dict[str, Any]:
    ...
    def handle_create_table(create_node, statement_node) -> None:
        ...
        # Oracle `CREATE GLOBAL TEMPORARY TABLE` (wave 1rvky) is a PERMANENT schema
        # object — only its DATA is session-scoped, unlike a Postgres/T-SQL temp
        # table. tree-sitter shreds `GLOBAL` into a sibling ERROR node (there is no
        # keyword_global), so `keyword_temporary` alone would mis-drop the GTT as
        # ephemeral. When a `GLOBAL` token is present alongside the temp keyword,
        # treat the table as permanent so it mints a real definition node.
        if is_temp and any(
            c is not name_node
            and _ts_node_text(c, source_bytes).strip().casefold() == "global"
            for c in children
        ):
            # (excludes name_node so a temp table literally named `global` is not
            # mis-flipped to permanent — adversarial-faithfulness lane 2026-07-06)
            is_temp = False
        ...
        # Reference extraction (wave 1rvdq — precise, replacing the old blanket
        # "every object_reference is an FK read" stack walk that leaked column
        # TYPE names, DEFAULT/GENERATED expression tokens, and CHECK-expr tokens
        # as phantom `reads`). The blanket walk did TRIPLE duty; each strand is
        # now discriminated by its parse position:
        #   * FK targets  — an `object_reference` immediately preceded (among its
        #     siblings) by a `keyword_references`, in a `column_definition`
        #     (column-level `… REFERENCES orgs (id)`) or a table-level
        #     `constraint` (`… FOREIGN KEY (a) REFERENCES orgs (id)`). KEPT.
        #   * LIKE source — `CREATE TABLE t (LIKE parent …)`: a `column_definition`
        #     whose leading identifier is `LIKE` and whose `object_reference` is
        #     the copied table. A GENUINE read. KEPT.
        #   * CTAS        — `CREATE TABLE t AS SELECT … FROM src`: the reads live
        #     under a `create_query` subtree; route it through `walk_reads` (which
        #     also de-dups the old doubled reads). KEPT.
        #   * column TYPE names / DEFAULT + GENERATED expression tokens / CHECK
        #     tokens — an `object_reference` in a column-definition/constraint that
        #     is NOT `keyword_references`-preceded and NOT a LIKE source, OR one
        #     nested under an `invocation`/`parenthesized_expression`. NOT a data
        #     read — SUPPRESSED by not minting non-FK/LIKE positions and not
        #     recursing into expression subtrees. (readiness primer 2026-07-06)
        owner = table_name if not is_temp else None

        def _emit_fk_and_like(container, *, allow_like: bool) -> None:
            kids = _sql_node_children(container)
            is_like = False
            if allow_like:
                first_ident = next(
                    (k for k in kids if str(getattr(k, "type", "") or "") == "identifier"),
                    None,
                )
                is_like = (
                    first_ident is not None
                    and _ts_node_text(first_ident, source_bytes).strip().upper() == "LIKE"
                )
            saw_references = False
            for k in kids:
                kt = str(getattr(k, "type", "") or "")
                if kt == "keyword_references":
                    saw_references = True
                    continue
                if kt == "object_reference":
                    if saw_references:
                        make_ref(k, direction="read", clause="references",
                                 statement="create_table", owner=owner, cte_names=set())
                        saw_references = False
                    elif is_like:
                        make_ref(k, direction="read", clause="like",
                                 statement="create_table", owner=owner, cte_names=set())
                    # else: column TYPE name / default token — suppress.
                    continue
                # A nested `constraint` (column constraint) or an `ERROR` shard
                # still carries FK references — recurse for those only; do NOT
                # descend into invocation/expression subtrees. The ERROR case
                # covers the column-list-less `REFERENCES parent` shorthand
                # (no `(id)`), which tree-sitter-sql shreds into an `ERROR`
                # holding `keyword_references + object_reference` (delivery
                # adversarial-faithfulness lane 2026-07-06). Because this recurse
                # only mints `keyword_references`-preceded object_references, it
                # cannot resurrect a column-type/default phantom from an ERROR.
                if kt in ("constraint", "ERROR"):
                    # A FK target lives INSIDE the constraint/ERROR (handled by
                    # the recurse), never after it — so clear any pending latch
                    # so a stray `keyword_references` cannot carry across and
                    # mis-mint a later sibling (code-review lane 2026-07-06).
                    saw_references = False
                    _emit_fk_and_like(k, allow_like=False)

        for child in children:
            if child is name_node:
                continue
            ct = str(getattr(child, "type", "") or "")
            if ct == "create_query":
                walk_reads(child, "from", "create_table", owner, set())
            elif ct == "column_definitions":
                for sub in _sql_node_children(child):
                    st = str(getattr(sub, "type", "") or "")
                    if st == "column_definition":
                        _emit_fk_and_like(sub, allow_like=True)
                    elif st == "constraints":
                        for con in _sql_node_children(sub):
                            if str(getattr(con, "type", "") or "") in ("constraint", "ERROR"):
                                _emit_fk_and_like(con, allow_like=False)
                    elif st in ("constraint", "ERROR"):
                        # ERROR: the column-list-less `REFERENCES parent` shorthand
                        # shreds a bare `keyword_references + object_reference` here.
                        _emit_fk_and_like(sub, allow_like=False)
            elif ct == "constraint":
                _emit_fk_and_like(child, allow_like=False)
    ...
    def analyze_statement(stmt_node, owner: str | None, outer_cte_names: set[str] | None = None) -> None:
        ...
        # T-SQL `SELECT … INTO newtbl FROM src` (wave 1rvky) — a new-table write,
        # the T-SQL twin of CTAS. `keyword_into` AND `select` both as DIRECT
        # children of the statement distinguish it from `INSERT INTO t` (whose
        # `keyword_into` is nested inside an `insert` node, not at statement
        # level). GATED to top-level scope (owner is None): a PL/pgSQL
        # `SELECT col INTO myvar` (variable assignment) parses to a BYTE-IDENTICAL
        # tree, so minting a table for it would be a phantom — the assignment
        # form only occurs inside a routine body (owner set). Mint the target
        # table definition + a `write`; the FROM reads flow through the normal
        # walk below (the target `select_expression`'s `field` is skipped, so no
        # double-processing).
        if owner is None and "keyword_into" in child_types and "select" in child_types:
            into_idx = child_types.index("keyword_into")
            target_expr = next(
                (children[j] for j in range(into_idx + 1, len(children))
                 if str(getattr(children[j], "type", "") or "") == "select_expression"),
                None,
            )
            if target_expr is not None:
                field_stack = [target_expr]
                target_field = None
                while field_stack:
                    fnode = field_stack.pop(0)
                    if str(getattr(fnode, "type", "") or "") == "field":
                        target_field = fnode
                        break
                    field_stack.extend(_sql_node_children(fnode))
                if target_field is not None:
                    # A `#tmp` / `##g` / `@tv` T-SQL temp target is a SESSION temp
                    # object, never a permanent schema table — record it as a temp
                    # name (so it is excluded like `CREATE TEMP TABLE`) and mint no
                    # definition/write (adversarial-faithfulness lane 2026-07-06).
                    # `_sql_temp_sigil` also inspects the byte BEFORE the target,
                    # because tree-sitter splits the `#`/`@` off as its own token so
                    # the field text alone is just `tmp`. `_sql_recovery_clean_name`
                    # strips `[dbo].[x]` bracket quoting for the permanent case.
                    field_text = _ts_node_text(target_field, source_bytes).strip()
                    if _sql_temp_sigil(target_field, source_bytes):
                        temp_names.add(field_text.lstrip("#@").casefold())
                        into_clean = None
                    else:
                        # Use the FULL target select_expression text (not just the
                        # first `field`) so a bracket- or dot-qualified target like
                        # `[dbo].[rpt]` / `dbo.rpt` keeps its schema — a single
                        # `field` node captures only the trailing component
                        # (code-review lane 2026-07-06). `_sql_recovery_clean_name`
                        # strips `[]` quoting and validates the qualified shape.
                        into_clean, _ = _sql_recovery_clean_name(
                            _ts_node_text(target_expr, source_bytes).strip())
                    if into_clean and into_clean.casefold() not in _SQL_RELATION_KEYWORD_STOPLIST:
                        into_schema = into_clean.rsplit(".", 1)[0] if "." in into_clean else None
                        definitions.append({
                            "name": into_clean, "schema": into_schema,
                            "sql_kind": "table", "temporary": False, "node": stmt_node,
                        })
                        references.append({
                            "name": into_clean, "schema": into_schema,
                            "direction": "write", "clause": "select_into",
                            "statement": "select_into", "owner": owner,
                        })
        if is_merge:
        ...
        for child in children:
            ...
            elif ctype == "create_type":
                # CREATE TYPE <name> AS (…) | AS ENUM (…) (wave 1rvdq) — the type
                # NAME is a definition node (`sql_kind="type"`, node kind stays
                # class/function per the sql_kind precedent). Its composite field
                # types / enum labels are TYPE positions and enum literals — never
                # table reads — so do NOT walk the body. Previously CREATE TYPE
                # fell to the generic `else: walk_reads(...)` branch and minted a
                # phantom self-`read` of the type name with no definition node.
                for sub in _sql_node_children(child):
                    if str(getattr(sub, "type", "") or "") == "object_reference":
                        name, schema = _sql_object_reference_parts(sub, source_bytes)
                        if name:
                            definitions.append({
                                "name": name,
                                "schema": schema,
    ...
    def handle_error_region(err_node) -> str | None:
        ...
        if (not recovered["definitions"] and not recovered["references"]
                and not _sql_region_is_benign(region_text)):
...
class GraphIndexSession:
    ...
    def _extract_doc_artifact(
        ...
        node_kind = kind if kind in ("seed", "memory") else "doc"
        module_id = rel_path
        nodes = [
        ...
        # Keep absent candidates as a per-document retry obligation (1x8e1).
        # Candidate parsing/normalization is identical to known-path links.
        candidates = set(_extract_doc_links(source_text, rel_path, None))
        candidates.update(_extract_doc_backtick_paths(source_text, rel_path, None))
        linked_paths = candidates.intersection(self._current_paths)
        for linked_path in sorted(linked_paths):
            edges.append(_edge(module_id, linked_path, "doc_references_doc", confidence="EXTRACTED"))

        # Memory records (1p8gy Req 5): explicit `## Targets` refs become typed
        # memory_targets edges — file targets validated against the current
        # path set; symbol targets resolved through the same matcher terms the
        # prose pass uses. Wave/change refs are record FIELDS resolved at
        # query time, never graph edges (those docs are scan-excluded).
        if node_kind == "memory":
            targets_section = source_text.split("## Targets", 1)
            targets_text = targets_section[1].split("\n## ", 1)[0] if len(targets_section) > 1 else ""
            for ref in re.findall(r"`([^`]+)`", targets_text):
                ref = ref.strip()
                if ref.startswith("community:"):
                    continue  # resolved via the cluster artifact at query time
                if ref.startswith("symbol:"):
                    term = ref[len("symbol:"):].lower()
                    for target in sorted(simple_lower.get(term, set()) | complex_lower.get(term, set())):
                        edges.append(_edge(module_id, target, "memory_targets", confidence="EXTRACTED", evidence=ref))
                elif ref in self._current_paths:
                    edges.append(_edge(module_id, ref, "memory_targets", confidence="EXTRACTED", evidence=ref))
        ...
        return {
            ...
            "unresolved_doc_targets": sorted(candidates - self._current_paths),
    ...
    def _is_doc_path(self, rel_path: str) -> bool:
        return _kind_for_path(rel_path) in {"doc", "seed", "memory"}
    ...
    def finalize(self) -> dict[str, Any]:
        ...
        Served-endpoint invariant (wave 1x6ti, 1x5pc): every edge in the
        payload has endpoints that are a node, an ``external::`` id, a
        current path of the build (the widened set that counts walk-shadowed
        known paths as current), or, under a directory the walk reported
        unreadable this build, an endpoint whose edge the last published
        payload served (the subtree is served as of the last readable build
        until the directory is readable again). Stored fragments keep whatever edges they
        carried until their file is re-scanned, so a fragment can re-emit an
        edge into a doc path deleted on an earlier build or into a symbol
        renamed while the fragment's doc was unreadable; the assembly-time
        filter drops those edges before the zero-edge doc prune (so a doc
        whose only edge dangled is pruned as a from-scratch build prunes it)
        and reports the count as ``edges_dropped_dangling``. Node-less
        current-path endpoints (a link to a ``.gitignore``, to a
        ...
        excluded_docs = {
            ...
            and _kind_for_path(rel) in {"doc", "seed", "memory"}
            and self._is_doc_scan_excluded(rel)
        }
        drop_paths = removed_paths | excluded_docs
        stats["files_removed"] = len(removed_paths)

        # Read one summary blob before the idle return; pending targets belong
        # to each document and survive failed/skipped reads across builds.
        plan = self._doc_link_repair_plan
        self._doc_link_repair_plan = None
        ...
        valid_merge_state = _valid_doc_link_merge_state(merge_state)

        # --- Zero-change fast path (Req-1): no merge work, no artifact rewrite.
        if not self.pending_code and not self.pending_doc_text and not drop_paths and not pending_link_docs:
        ...
        # Unchanged docs retry available unresolved paths or changed symbols.
        # A full merge reconstructs summaries from stored artifacts, which
        # retain the same obligations when the merge blob was lost.
        if not incremental:
            pending_link_docs |= _pending_doc_link_repairs(
                {"files": recompute_artifacts}, current_paths, self.unreadable_dirs,
            )
        impacted_docs: list[str] = []
        if changed_code_symbols or pending_link_docs:
            for rel, entry in _artifacts_view().items():
                if entry.get("kind") not in {"doc", "seed", "memory"}:
                    continue
                if rel in self.pending_doc_text:
                    continue
                ...
                if rel in pending_link_docs or mentioned.intersection(changed_code_symbols):
        ...
        # Doc extraction contributes no code symbols, so the matcher remains
        # valid: each impacted doc is read/extracted at most once this build.
        ...
        # every affected fragment edge (it updates fragments); a scope-(b)
        # miss class (a doc fragment linking into a removed doc path, which
        # no symbol delta covers; a shadowed doc's mention of a symbol renamed
        # during the outage) is masked here for exactly one build and
        # re-emitted by the fragment on the next unrelated edit. Since wave
        # 1x6ti (1x5pc) the assembly-time endpoint filter after the
        # short-symbol prune catches that re-emission on every build and
        # counts it as `edges_dropped_dangling`; a non-zero count on a build
        # with no removals points at a fragment the symbol delta did not
        # reach, not at this block.
        ...
        # Wave 1x6ti (1x5pc): assembly-time dangling-endpoint filter. The
        # reverse-invalidation prune above only knows THIS merge's removals;
        # a stored fragment re-emits its edge into a doc path deleted on an
        # earlier build, or into a symbol renamed while the fragment's doc was
        # walk-shadowed, on every later merge. A from-scratch build resolves
        # links and mentions against the current tree and carries neither
        # edge, so drop every edge whose endpoint is not a node, not an
        # `external::` id (unresolved by design), not a current path of the
        # build, and not (under a directory the walk reported unreadable this
        # build) an endpoint whose edge the last published payload served.
        # `current_paths` is the widened local set (walk-shadowed known paths
        # included): a node-less current path (a link to a `.gitignore`, to a
        # scan-excluded doc, a memory target into `docs/waves/`) is evidence
        # both sides of the differential carry and must survive. Runs BEFORE
        # the embedded-SQL capture reconcile (so its
        # counters see the filtered map) and BEFORE the zero-edge doc prune
        # (so a doc whose only edge dangled is pruned as the oracle prunes it).
        def _endpoint_servable(node_id: str) -> bool:
            return (
                node_id in node_map
                or node_id.startswith("external::")
                or node_id in current_paths
            )

        # Delivery review CODE-DEL-1 / CODE-RV1-2: a file the graph never
        # nodes (a `.gitignore`, a `Makefile`) under a directory the walk
        # could not read this build has no `files` row to widen from, so it
        ...
        def _edge_servable(key: tuple) -> bool:
            ...
            if _endpoint_servable(src) and _endpoint_servable(tgt):
                return True
            if not _last_served_keys or (src, tgt, rel) not in _last_served_keys:
                return False
            for endpoint in (src, tgt):
                if _endpoint_servable(endpoint):
                    continue
                endpoint_file = endpoint.split("::")[0] if "::" in endpoint else endpoint
                if not _path_under_unreadable(endpoint_file, self.unreadable_dirs):
                    return False
            return True

        _dangling_keys = [k for k in edge_map if not _edge_servable(k)]
        for key in _dangling_keys:
            edge_map.pop(key, None)
        stats["edges_dropped_dangling"] = len(_dangling_keys)
        if _dangling_keys and self.verbose:
            print(
                "build_index: dangling-endpoint filter — "
                f"{len(_dangling_keys)} edge(s) dropped whose endpoint is no node, "
                "external id, current path, or last-served path under an unreadable directory",
                flush=True,
            )
        ...
        # and provide no graph navigation value. `memory` nodes are DELIBERATELY
        # exempt (wave 1ro44 / 1p8gy AC-4): a record whose target_refs fail to
        # resolve must stay queryable — the record store still serves it, and
        # pruning would silently hide the memory from graph consumers.
...
def retire_orphaned_graph_paths(
    *,
    root: Path,
    index_dir: Path,
    layer: str,
    ...
    The retirement API for the orphan-store reconciliation at the indexer's
    build-epoch reap seam. It routes through ``update_graph_index`` with an
    EMPTY changed set: the merge's known-minus-current diff prunes the
    orphaned file rows, re-resolves edges that referenced them, and rewrites
    the payload and merge state atomically. A raw row delete would desync the
    per-file store from the served payload and the merge-state fragments;
    ...
    Caller census (recorded in the 1u8nz change doc): one production callee

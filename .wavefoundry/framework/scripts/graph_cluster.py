...
CLUSTER_BUILDER_VERSION = "13"  # Wave 1wpie: deliberate invalidation bump (the artifact shape is unchanged; Evidence/Data partitioning happens at report time from flags on graph nodes) so a same-fingerprint artifact built at 12 is recomputed rather than reused. Bump this on every cluster-artifact shape change; earlier bump rationale lives in CHANGELOG.md under each wave.

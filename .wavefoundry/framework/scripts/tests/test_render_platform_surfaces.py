...
class CodexPlatformOwnershipTests(unittest.TestCase):
    ...
    def test_first_render_creates_codex_config_without_guru_or_codex_directory(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertFalse((root / "docs" / "agents" / "guru.md").exists())
            self.assertFalse((root / ".codex").exists())
            # An explicit non-Codex platform selection is the negative control:
            # the common writer must still run on the first public render.
            self.assertEqual(mod.main(["--repo-root", str(root), "--platform", "claude"]), 0)

            config = root / ".codex" / "config.toml"
            self.assertTrue(config.is_file())
            self.assertIn("generated by render_platform_surfaces.py", config.read_text(encoding="utf-8"))

    def test_guru_present_first_render_writes_codex_before_second_render(self):
        scripts_dir = str(SCRIPT_PATH.parent)
        if scripts_dir not in sys.path:
            sys.path.insert(0, scripts_dir)
        mod = _load_render_module()
    ...
    def test_unwired_agent_renderer_negative_control_leaves_stale_carrier(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            unwired = SimpleNamespace(
                ...
                # The pure TOML merge primitive remains shared by design;
                # only the agent-renderer orchestration is the negative control.
                upsert_codex_mcp_config=ras.upsert_codex_mcp_config,
            )
            with patch.object(venv_bootstrap, "ensure_python_resolves", return_value="ok"), \
...
class RenderPlatformSurfacesScriptTests(unittest.TestCase):
    def test_renders_python_hook_entrypoints_and_configs(self) -> None:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            expected_stop_command = mod.launcher_command(
                ".claude/hooks/session-capture", "CLAUDE_PROJECT_DIR"
            )
            self.assertEqual(
            ...
            self.assertEqual(
                copilot_hooks["hooks"]["preToolUse"][0]["powershell"],
                expected_copilot_command,
            )
            self.assertEqual(copilot_hooks["hooks"]["preToolUse"][0]["cwd"], ".")
...
class ClaudeHookSimulateParityTests(unittest.TestCase):
    ...
    def _settings_hook_names(self, settings: dict) -> set[str]:
        ...
        for entries in settings.get("hooks", {}).values():
            for entry in entries:
                for hook in entry.get("hooks", []):
                    ...
                    for declared in self.mod.CLAUDE_HOOKS:
                        name = str(declared["name"])
                        if f"'{name}.py'" in command or f'/{name}.py"' in command:
                            names.add(name)
        return names
...
class LauncherCommandTests(unittest.TestCase):
    """Owner-bound hosts resolve paths without shell-specific interpolation."""
    ...
    def test_launcher_command_nt_invokes_python_on_py_body(self):
        ...
        with patch.object(mod.os, "name", "nt"):
            ...
            self.assertIn("os.environ.get('CLAUDE_PROJECT_DIR')", anchored)
            self.assertIn("runpy.run_path", anchored)
    ...
    def test_launcher_command_interpreter_and_path_byte_identical_across_os(self):
        ...
        self.assertIn("runpy.run_path", posix)

    def test_claude_launcher_stays_bound_to_configured_owner_from_nested_cwd(self):
        with tempfile.TemporaryDirectory() as tmp:
            outer = Path(tmp) / "outer"
            nested = outer / "nested" / "work"
            hook = outer / ".claude" / "hooks" / "pre-edit.py"
            hook.parent.mkdir(parents=True)
            nested.mkdir(parents=True)
            hook.write_text("print('owner-bound')\n", encoding="utf-8")
            env = dict(os.environ)
            ...
            self.assertEqual(result.returncode, 0, result.stderr)
            self.assertEqual(result.stdout.strip(), "owner-bound")

    def test_claude_launcher_missing_owner_fails_with_named_diagnostic(self):
        command = self.mod.launcher_command(
            ".claude/hooks/pre-edit", "CLAUDE_PROJECT_DIR"
        )
        env = dict(os.environ)
...
class NoPathedLauncherScanTests(unittest.TestCase):
    ...
    launcher (the bash `.wavefoundry/bin/mcp-server`) — they all name the byte-identical `python3`
    command and use their host's verified owner/config anchor for server.py. The scan enumerates the ACTUAL on-disk config set,
    INCLUDING the separately managed `.air/mcp.json`; `.codex/config.toml` is
    now owned by `render_platform_surfaces.py` alongside the JSON configs."""
    ...
    def test_rendered_json_mcp_configs_name_python_byte_identical(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            on_disk = {
                repo_root / ".mcp.json": self.CLAUDE_ARGS,
                repo_root / ".cursor" / "mcp.json": self.EXPECTED_ARGS,
                repo_root / ".junie" / "mcp" / "mcp.json": ["../../.wavefoundry/framework/scripts/server.py"],
                repo_root / ".agents" / "mcp_config.json": self.EXPECTED_ARGS,
            }
            for path, expected_args in on_disk.items():
                ...
                self.assertEqual(wf["args"], expected_args, f"{path.name}: wrong args")
    ...
    def test_committed_on_disk_configs_have_no_pathed_launcher(self):
        ...
        for path in present_json:
            ...
            expected_args = (
                ["../../.wavefoundry/framework/scripts/server.py"]
                if path == repo / ".junie" / "mcp" / "mcp.json"
                else self.CLAUDE_ARGS
                if path == repo / ".mcp.json"
                else self.EXPECTED_ARGS
            )
            self.assertEqual(wf["args"], expected_args, f"{path}: wrong args")
...
class HookReindexDetachTests(unittest.TestCase):
    ...
    def test_every_rendered_mcp_command_stays_python3(self):
        # Wave 1p8pe AC-5: the MCP JSON-RPC stdio transport must NOT be pythonw-converted. The
        # interpreter stays byte-identical while each host uses its verified owner/config anchor.
        renderers = {
            ".mcp.json": (
                "render_mcp_json",
                lambda r: r / ".mcp.json",
            ...
            "cursor": (
                ...
                lambda r: r / ".cursor" / "mcp.json",
                lambda args: args == [".wavefoundry/framework/scripts/server.py"],
            ),
            "junie": (
                "render_junie_mcp_json",
                lambda r: r / ".junie" / "mcp" / "mcp.json",
                lambda args: args == ["../../.wavefoundry/framework/scripts/server.py"],
            ),
            "antigravity": (
                "render_antigravity_mcp_json",
                lambda r: r / ".agents" / "mcp_config.json",
                lambda args: args == [".wavefoundry/framework/scripts/server.py"],
            ),
        }
        for host, (fn_name, path_of, valid_args) in renderers.items():
            with self.subTest(host=host):
                ...
                self.assertTrue(valid_args(wf["args"]), f"unexpected {host} owner-bound args")

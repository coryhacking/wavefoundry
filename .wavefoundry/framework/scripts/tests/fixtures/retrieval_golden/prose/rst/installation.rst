...
Import the :mod:`driftbeam` package to confirm the interpreter can see it.
...
.. note::
   ...
   If the import fails with a ModuleNotFoundError, confirm that the interpreter on your PATH is the same one pip installed into.

.. image:: images/install-check.png
   :alt: Successful verification output

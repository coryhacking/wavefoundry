...
8a. Do **not** create `docs/index.md` here. The Backstage/TechDocs landing page is generated missing-only by `wf_techdocs_baseline` (CLI fallback `wf techdocs-baseline`, invoked by **Refresh TechDocs** at install row 2.13.5, and on demand afterwards) once `docs/references/project-overview.md`, `docs/ARCHITECTURE.md`, and `docs/prompts/index.md` exist, together with the root `catalog-info.yaml` and `mkdocs.yml`; each generated file carries a one-line generated-by stamp. A project-authored `docs/index.md` is legitimate and is preserved byte-for-byte by that command.
...
10a. Note `docs/references/codebase-map.md` as a generated orientation artifact (wave 1p5tl): a graceful-scaling, read-only map of the target project's own codebase, produced offline by `.wavefoundry/framework/scripts/gen_codebase_map.py` from persisted graph + community-cluster artifacts. It is the **index to the index** — it routes agents to the right area, then hands off to the `code_*` tools (`code_graph_community` via a stable `hub_node_id`, `code_outline` on key files) for depth. The seeded `docs/references/project-overview.md` orientation section and `AGENTS.md` **Start Here** should point at it so agents consult the map first. Do not hand-edit the generated file. It refreshes during create-mode prepare-and-open/close, upgrade, forced `index_build(content="map")`, the direct change-only `wf codebase-map` / generator CLI, or the map resource's missing-file fallback. Ordinary index builds, ready-only/dry-run lifecycle modes, and reads of an existing map do not regenerate it.
11. Verify `docs/plans/plan-template.md` is present; Phase 1 surface rendering materializes it missing-only from `.wavefoundry/framework/install/plan-template.md`. If it is absent, copy that shipped file verbatim and replace its `{{generated_at}}` placeholder with today's `YYYY-MM-DD` date — never author a thin template from the section list below. Existing templates are operator-owned and must not be overwritten; tailor only project-specific wording when a merge-safe update is needed. Any example path in the template must remain inside a fenced block so the scaffold itself declares zero review targets. After any edit, run the docs gate and verify `serialization_point_paths` reports no targets for the template. The shipped template and any project copy must use the consolidated single-document model:
    - `Change ID:` using the `<id-prefix>-change <slug>` format from the MCP `wf_new_change` tool when available, with `wf lifecycle-id --kind change --slug <slug>` as the CLI fallback
    ...
    - `## Serialization Points` — shared files and integration gates, AND the machine-readable review-target declaration, in one of two forms: a bullet whose content is entirely repo-relative paths, each backtick-quoted, for example `` - `src/app/handler.py`, `docs/specs/` ``; or an explicit `**Review targets (repo-relative paths):**` block whose backtick-quoted entries may contain spaces. Prepare selects automatic review lanes from those declared paths and from nothing else — never from Scope, Rationale, or other narrative, and never from prose. A bullet declares only when it is entirely targets: one stray English word makes the whole bullet prose, in either form, and a bullet whose text wraps onto a continuation line is prose in its entirety. A declared path token has at least one `/`, so a root-level file (a changelog, a readme) is never a token in either form, and a bullet declares all or nothing in either form, so one such token turns the whole bullet into prose and every other path in it goes undeclared with it. In a bullet a `*` disqualifies the token too; inside the explicit block a span is kept only when its last segment carries an extension or the span ends in `/`, so there a `*` span is accepted as a phantom (a `*.json`, a `dir/*/`) that matches no file and recruits a lane only through a trigger token it happens to carry (a directory prefix, an extension, or a trigger basename), a block holding only phantoms leaves the document declared with whatever roster those triggers recruit (empty when none is a trigger), and any other `*` span turns its bullet into prose. Put a root-level file in its own prose bullet, and declare the directory that holds globbed files. Adoption is decided per document, so a change doc that declares no path keeps legacy whole-document scoring and coverage is never silently lost, but it forfeits the precision. Path scoring is a floor: any lane may additionally be requested by judgment in the wave record's `Requested review lanes`, and architecture, security and performance risk are usually judgment calls that no path can express
...
13. Seed `docs/design-system/design-language.md` when `docs/repo-profile.json` `design_system.design_evidence.detected` is `true`. The file must use the following stack-neutral canonical structure:
   ...
   When seeding `docs/design-system/design-language.md`, also ensure `docs/workflow-config.json` includes a `design_review_triggers` list (with at least: `ui-surface-change`, `new-color-token`, `new-component-pattern`, `typography-change`, `hig-departure`) and a `require_design_review_for_ui_surface_changes: true` flag so the trigger contract is machine-readable without opening the design doc. The file already exists after Phase 1: setup Step 0 provisions `lifecycle_id_policy` and the seven framework-required sections. These two project-evidence design keys are intentionally not defaults; add them directly to `docs/workflow-config.json` in this task without changing existing operator values.
14. **Design-system extraction contract.** Seed the machine-readable extraction contract under `docs/design-system/` as part of any install or upgrade that touches `docs/design-system/`. This is distinct from the narrative `design-language.md` (task 13): that file is operator-owned narrative; the extraction contract is a machine-readable surface agents read during UI implementation. The extraction contract must coexist with `design-language.md` — never overwrite its body.
    ...
    **Core tree (create these paths during install/upgrade when absent; never overwrite if present):**
    ...
    │   │   └── dark.tokens.json
    │   └── README.md
    ├── build.config.json                # token-build pipeline contract (tool/version/targets)
    ├── bin/
    │   └── build-tokens                 # wrapper: reads build.config.json, runs the configured tool
    ├── exports/
    │   ├── README.md                    # generated; explains subdirs + links to token-build pipeline
    │   ├── css/                         # generated by token-build pipeline (tokens.css)
    │   ├── tailwind/                    # generated (theme.config.js)
    │   ├── ts/                          # generated (tokens.ts)
    ...
    **Token naming convention (mandatory; enforced by `design_system_validators._DOT_PATH_RE`).** All token names must follow the `{category}.{subcategory}.{scale}.{variant}` dot-path schema. Every segment is lowercase. The first segment starts with a letter and continues with `a-z0-9`; each later segment either starts with a letter and continues with `a-z0-9`, or is all digits. Thus `color.primary.500`, `color.action.primary.background`, `spacing.4`, and `radius.button` pass, while `size.2xl` and `size.4xl` fail. Hyphens and uppercase letters are never valid. Normalize source names by splitting camelCase and hyphen boundaries into additional dot segments (`fontSize` → `font.size`, `kpi-card` → `kpi.card`, `in-review` → `in.review`), and record the original source name in `source-map.json` under `normalizedFrom`.

    **`manifest.json` core schema.** Required fields: `schemaVersion` (semver string, e.g. `"1.0.0"`), `extractionVersion`, `extractedAt` (ISO-8601), `canonicalRoot` (must equal `"docs/design-system"` — **fixed for all modes**, including `external-reference`; see the three-mode model below), `sourceStrategy` (enum: `figma-extract`, `repo-evidence-only`, `visual-bootstrap`, `hybrid`, `external-reference`), `evidenceTypes` (array), `artifactCounts`, `modes` (must include `"light"` and `"dark"`), `validationSummary`. Reserved for Split C (accept but not require): `targetSurfaces`, `platformStandards`, `deprecations`. Required only under `sourceStrategy: external-reference`: `externalReference` (see the three-mode model below).

    **Three-mode model — bootstrap / extract-mirror / adopt (wave `1p799`).** Before emitting the tree, select the mode from `docs/repo-profile.json` `design_system.mode` (set by `seed-030` from `classify_design_system_mode(design_evidence)` — see `seed-031` § Design and UI surface for the evidence bar). Do **not** re-decide by judgment; consume the recorded verdict.

    - **bootstrap** — no design system found (no evidence) → emit the standard nulls skeleton (current behavior; the no-design-system path above).
    - **extract-mirror** — in-repo design *evidence* (CSS custom properties, stylesheet tokens, in-repo theme files) but **no** maintained external system with its own source-of-truth/build → extract the full DTCG tree into `docs/design-system/` (current behavior). **Self-hosting guard:** Wavefoundry's own dashboard (in-repo `dashboard.css`, no external token package) stays `extract-mirror`, never `adopt`.
    - **adopt / `external-reference`** — a **declared source of truth with its own build** (a published/packaged token package, a Style-Dictionary/DTCG source dir + build, or Figma library links — **not** merely CSS custom properties or a stray Tailwind theme) → emit the **thin reference index**; do **not** re-extract a parallel mirror.
    - **ambiguous** — genuinely weak / mixed signals → **ask the operator** before proceeding (`seed-010` install / `seed-160` upgrade handle the prompt). Never silently adopt *or* mirror on weak evidence.

    Never overwrite or duplicate an existing system: when the target already owns a mature design system, adopt-in-place defers to it rather than imposing the extracted shape.

    **`canonicalRoot` stays fixed for all modes (interrogation C1).** `canonicalRoot` always equals `"docs/design-system"` — it means *where the contract/index lives*, which consumers (the validator, the `AGENTS.md` docs-map pointer, the semantic index) rely on. The external source location is expressed **only** via `externalReference.tokenSource`; never relax `canonicalRoot` to name the external root.

    **Thin reference tree (decline the full mirror).** Under `external-reference` the contract degrades to a thin index — only `manifest.json` (with `externalReference`), `source-map.json` (pointers), `AGENTS.md` (consumption guidance pointing at the real source), `gaps.md`, and `README.md` are required. The full `tokens/` + `exports/` mirror (and `DESIGN.md` / `VALIDATION.md` / `version.json` / `proposed-additions.md`) are **not** required. The design-system validator treats absent `tokens/`/`exports/` as not-an-error **only when a resolvable `externalReference` is present** — an unresolvable/absent pointer keeps the full requirement set so `external-reference` cannot silence a genuinely-missing token tree.

    **`externalReference` schema (required under `sourceStrategy: external-reference`).** An object with:
    - `tokenSource` (**required**) — path or URI to the real source of truth. The validator requires it to be **resolvable**: a path must exist in the repo; a URI must be well-formed (`scheme://…`). Missing/empty/unresolvable → validation failure.
    - `buildCommand` (optional) — how the project builds its tokens (e.g. `npm run build:tokens`).
    - `namingConvention` / `varPrefix` — the project's real token/var convention agents must use (e.g. `kebab-case`, `--brand`).
    - `consumptionDoc` — where agents read usage guidance for the existing system.
    - `notes` — free-form context.

    Mirror the same pointer into `source-map.json`: under `external-reference`, `source-map.json` records reference entries pointing at the external source (`sourceType: "external"`, `sourceRef` = `externalReference.tokenSource`, with `confidence`) instead of extracted token entries — never a parallel extracted token tree.

    **Derived `AGENTS.md` guidance under reference mode (interrogation C7).** The seeded `docs/design-system/AGENTS.md` consumption guidance must be **templated from** `externalReference` (`varPrefix`, `consumptionDoc`, `namingConvention`, `buildCommand`) and point at the project's `consumptionDoc` — never prescribe a mechanism (CSS-in-JS / SCSS / ThemeProvider are all valid) and never the `--ds-*` namespace or the `exports/` rules. The "never hard-code; consume tokens" rule is preserved but retargeted to *their* system and *their* var prefix.

    **`hybrid` composition with `external-reference`.** `hybrid` remains the multi-source enum value: when a project adopts an external source for most of its system but also has in-repo evidence the framework extracts, record `sourceStrategy: "hybrid"` and list all active sources in `evidenceTypes` — then both the extracted tree *and* an `externalReference` pointer may be present. `external-reference` is the *pure adopt* case (point-only, thin tree); `hybrid` is adopt-plus-extract (full tree allowed, pointer optional). Choose `external-reference` only when the system is fully external.

    **extract→adopt migration (operator-initiated only).** When migrating an existing `extract-mirror` project to `external-reference`, never silently delete the extracted tree: move the orphaned `tokens/` (and other extracted-mirror subtrees) to `docs/design-system/.backup/<ISO-date>/`, write a `meta`-category `gaps.md` entry recording the move, then emit the thin reference index. Auto-migrating a mirror to adopt is out of scope — the operator initiates it.
    ...
    **`source-map.json` core schema.** Each entry: `id` (dot-path token name or `<category>/<item-id>`), `sourceType` (`figma`|`code`|`visual`|`inferred`|`external`), `confidence` (`high`|`medium`|`low`), `sourceRef` (URI, path, or null), `normalizedFrom` (omit if unchanged), `platformHint` (optional, for Split C). Under `sourceStrategy: external-reference` (wave `1p799`), entries use `sourceType: "external"` and set `sourceRef` to `externalReference.tokenSource` — the map *points at* the existing system rather than recording an extracted token tree.
    ...
    **`AGENTS.md` content contract.** The seeded `docs/design-system/AGENTS.md` must contain — not just exist:
    ...
    - **Referencing tokens in code (wave `12atj`):** consume the generated `exports/` — never hard-code values. CSS: `var(--ds-<token-path-dashed>)` (e.g. `var(--ds-color-action-primary)`). TypeScript: import `tokens` / `tokensByMode` from `exports/ts/tokens.ts`. Tailwind: extend from `exports/tailwind/theme.config.js` (`theme.extend` light, `theme.extendDark` dark variants). Flat JSON: `exports/json/tokens.json`. Run `docs/design-system/bin/build-tokens` after editing any `tokens/*.json` source file; `exports/` is generated and must not be hand-edited.
    ...
    **`exports/README.md` must state** that the subdirectory contents are generated by the token-build pipeline (`docs/design-system/bin/build-tokens`, wave `12atj-feat design-token-build-pipeline`) and must not be hand-edited. Subdirectories are stubs until `bin/build-tokens` is run.

    **Token-build pipeline setup (wave `12atj`).** Seed the pipeline contract so the operator can generate `exports/` from the DTCG token source:

    - **`docs/design-system/build.config.json`** — tool-agnostic contract. Detect an existing `build.config.json` first; **emit the default stub only when absent** (never overwrite an operator's config). The stub:
      ...
      ```json
      {
        ...
        "version": "1.0.0",
        "targets": [
          { "format": "css", "outputDir": "exports/css", "options": { "prefix": "ds", "selector": ":root" } },
          { "format": "tailwind", "outputDir": "exports/tailwind", "options": { "darkMode": "class" } },
          { "format": "ts", "outputDir": "exports/ts", "options": { "perMode": true } },
          { "format": "json", "outputDir": "exports/json", "options": { "resolveAliases": true } }
        ]
      }
      ```

      `tool` enum: `style-dictionary` (default; run `npm install -D style-dictionary`), `token-pipeline`, `custom` (requires a `command` field with the shell invocation), or `builtin` (the bundled pure-Python transform — no Node dependency, used when the target repo has no Node toolchain). `targets[]` entries are `{ format, outputDir, options }`.

    - **`docs/design-system/bin/build-tokens`** — wrapper script that reads `build.config.json` and invokes the configured tool. It must exit non-zero with actionable messages on a missing dependency (e.g. style-dictionary not installed) or a broken token reference at build time. Mark it executable.

    - **Default targets** emit (mode-aware, deterministic, idempotent, each with a `/* generated — do not edit directly */` header): CSS custom properties (`exports/css/tokens.css`, light base + dark override block), a Tailwind theme config (`exports/tailwind/theme.config.js`, with dark variants), typed TS constants (`exports/ts/tokens.ts`, per-mode maps), and a flat resolved JSON map (`exports/json/tokens.json`).

    - **`manifest.json` export parity** — after a successful build, `validationSummary` records `exportsGenerated: true`, `exportsAt` (ISO-8601), and `exportsStale` (true when the token source is newer than the exports). The design-system lint validator warns when `exportsStale` is `true`. Generating `exports/` for real is optional at install time (it needs the chosen tool installed); when the tool is absent, leave the `exports/` stubs and record `exportsGenerated: false`, then instruct the operator to run `bin/build-tokens` after installing the tool. Never hand-author export contents.
    ...
    **`sourceStrategy` full semantics (Split C; `external-reference` added in `1p799`):** `figma-extract` — primary evidence is Figma variables/styles/components. `repo-evidence-only` — primary evidence is checked-in code. `visual-bootstrap` — primary evidence is screenshots/reference URLs/decks (no-DS path; outputs are non-normative until explicit operator promotion). `hybrid` — combination; `evidenceTypes` lists all active sources. `external-reference` — the canonical system lives elsewhere (a declared external source-of-truth with its own build); the contract emits the thin reference index that points at it via `externalReference` rather than extracting a parallel mirror (see the three-mode model above).
    ...
    **Split C governance validators** (implemented in `wave_lint_lib/design_system_governance_validators.py`): `sourceStrategy` enum (incl. `external-reference`); `targetSurfaces` non-empty; `platformStandards[].referenceVersion` present per surface; visual-bootstrap proposal guard (no `proposed-from-best-practices` in `semantic.tokens.json`); deprecated component must have `supersededBy` or `sunset`; `platformStandards[].overrides` paths exist. The module also exposes the pure `classify_design_system_mode(design_evidence)` helper (wave `1p799`) the seeds consume to set `design_system.mode`. **Surface/manifest validator** (`design_system_validators.py`): under `external-reference` it accepts the thin tree (absent `tokens/`/`exports/`), requires the `externalReference` block, and rejects a missing/empty/unresolvable `externalReference.tokenSource` — `canonicalRoot == "docs/design-system"` stays enforced across all modes.
...
17. When creating or refreshing `docs/contributing/build-and-verification.md`, include a **Wave framework pack upgrade verification** section (use that exact heading or an equivalent clearly titled subsection under **Verification**). The section must be actionable for any repository that vendors `.wavefoundry/framework/` and must document, at minimum:
    - **Bring the pack in:** either (a) place a semver `wavefoundry-MAJOR.MINOR.PATCH.<build>.zip` at the **repository root**, in `~/.wavefoundry/`, or in `~/.wavefoundry/dist/` (from **Package Wavefoundry** — `docs/prompts/package-wavefoundry.prompt.md` when that prompt exists) and run **Upgrade Wavefoundry** (legacy: **Upgrade wave framework** / **Upgrade wave context**) so `seed-160` **step 0** adopts the highest semver zip, stages it under `.wavefoundry/framework/`, runs `wf render-surfaces`, and continues full reconciliation; or (b) merge or copy into `.wavefoundry/framework/` then run **Upgrade Wavefoundry** (legacy: **Upgrade wave framework** / **Upgrade wave context**) without relying on an adopted zip.
    ...
    - **Docs gate:** **Agents:** prefer MCP **`wf_garden_docs`** (when metadata needs refresh) then **`wf_validate_docs`**. **Operators / CI / no MCP:** `wf docs-gardener && wf docs-lint` (or the repo’s equivalent docs verification commands discovered from checked-in scripts).
    ...
    - **`scripts/build_pack.py` semantics** (must match the checked-in script; document in `docs/prompts/package-wavefoundry.prompt.md` when that prompt exists): `--version MAJOR.MINOR.PATCH` is required and packaging is blocked below `1.0.0`; the zip filename is `wavefoundry-MAJOR.MINOR.PATCH.<build>.zip` in `~/.wavefoundry/dist/` by default unless `--output` is passed; `<build>` is a standalone 4-character base36 pure-time build suffix (5-minute buckets on a pinned build epoch, independent of the lifecycle-ID policy); immediately before writing the archive, the script overwrites `framework/VERSION` with `MAJOR.MINOR.PATCH+<build>`, requires manifest `framework_revision` consistency unless `--skip-manifest-check` is used, then writes the source-only archive. No framework index is built or shipped — the framework's seeds and README fold into each project's docs index at setup/upgrade.

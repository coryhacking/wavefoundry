...
- `Change ID:` uses `<id-prefix>-<kind> <slug>` from the MCP `wave_new_*` change-creation tool for the selected kind, or from the CLI fallback `wf lifecycle-id --kind <kind> --slug <slug>` when MCP is unavailable (kinds: bug, feat, enh, change, doc, debt, ref, task, maint, ops)

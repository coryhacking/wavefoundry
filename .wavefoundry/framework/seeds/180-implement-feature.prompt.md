...
- **New feature analogue** (adding a feature analogous to an existing one): start at `wavefoundry://graph/communities` (resource read) to identify the analogue's community → `code_graph_community` drilldown to see the analogue's public-API surfaces (top-degree members) → `code_callhierarchy(direction="incoming")` on those top-API members to find the integration sites the new feature will plug into → `code_callhierarchy(direction="outgoing")` to find shared helpers the new feature will need → `wave_graph_report(sections=["fan_in", "chokepoints"])` for shared infrastructure to route through.
...
- `code_callgraph` at `depth > 1` on chokepoint symbols (top of `wave_graph_report.fan_out`) often blows the 25K token cap. Default to `depth=1` for chokepoints, or restrict by file glob.
...
1. Load the active execution plan, spec refs, and wave artifacts. Consult `docs/references/project-context-memory.md` and relevant role journals (`docs/agents/journals/`) for active cautions and known pitfalls that apply to the current wave's scope. When the MCP memory tools are attached, also call `wave_memory_brief(context='pre_implementation', targets=[...])` for the files in scope — typed agent memory records (fragile files, prior failed attempts, operator preferences) surface as capped, cited advisories, and a `needs_reverification` flag means re-check the concern against current code before editing. If memory records a past mistake in this area, treat it as a constraint on implementation — not a suggestion.

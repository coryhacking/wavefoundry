...
**Delegating code work to a subagent — the rule travels with the work.** When you spawn a subagent for code investigation or implementation, route it through a role-typed agent that carries this MCP-first contract (`implementer`, `guru`, a builder specialist) **or** include the MCP-first code-navigation directive in the subagent's prompt. Tool access follows the wrapper contract: subagents **without** an explicit `tools:` allowlist inherit the parent session's MCP tools, and role wrappers **with** an allowlist must name the retrieval tools in it (the framework's rendered wrappers do) — so "the subagent didn't have the MCP" is never a reason to fall back to shell; if a wrapper genuinely lacks the tools, that is a wrapper defect to fix, not a license to grep. A bare general-purpose subagent spawned for code work without the directive is the same shell-by-habit defect as in the main thread. When fanning out code-investigation lanes, **name the MCP tools in the delegated task** (and the schema-load step when the host defers them, e.g. ToolSearch) — an unnamed tool loses to the zero-setup shell path every time. Require the same `Gapfill:` note from spawned subagents when they fall back, and surface it. (Do not restate the exploration order above in the subagent prompt — point to it.)

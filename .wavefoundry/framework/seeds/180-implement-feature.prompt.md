...
**Delegating code work to a subagent — the rule travels with the work.** When you spawn a subagent for code investigation or implementation, route it through a role-typed agent that carries this MCP-first contract (`implementer`, `guru`, a builder specialist) **or** include the MCP-first code-navigation directive in the subagent's prompt. Task-spawned subagents **inherit the parent session's MCP tools by default**, so "the subagent didn't have the MCP" is never a reason to fall back to shell — a bare general-purpose subagent spawned for code work without the directive is the same shell-by-habit defect as in the main thread. Require the same `Gapfill:` note from spawned subagents when they fall back, and surface it. (Do not restate the exploration order above in the subagent prompt — point to it.)

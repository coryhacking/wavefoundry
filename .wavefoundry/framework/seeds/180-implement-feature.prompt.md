...
- `code_ask` for navigational questions is the wrong tool. Use `code_definition` + `code_callhierarchy` directly — they are both faster AND more precise than `code_ask`'s synthesized prose. Do not start with `code_ask` and bail on slow `rerank_ms`; the cost is already spent by then.

...
- readiness records which implementer, reviewer, and persona agents may be needed
...
- **Action** — invoke a single, scoped implementation or computational-verification action with defined inputs. A named reviewer checkpoint is permitted for the Level 2 boundary below or a planned per-change delivery review.
...
Finding classification — after a named checkpoint or delivery review, before deciding which loop level to activate:
...
- **Stop condition.** After the first delivery review cycle, an editorial-only finding (wording that is true but imprecise, drifted line citations, formatting) is repaired inline within the current cycle and recorded in `## Progress Log`; it does not by itself open another repair cycle. Every finding that needs verification, a boundary repair, or escalation retains its existing action-matrix route — including missing test coverage, behavior, security, architecture, scope, requirements, and acceptance criteria. **A re-Prepare depends on WHERE the repair lands, not on how the finding was classified:** a repair confined to a section the review-policy digest EXCLUDES needs no re-Prepare; a repair that edits any digested section, including a drifted line citation in `## Rationale` or formatting in `## Requirements`, still supersedes the receipt and needs one. **Escape hatch:** an editorial finding that makes a shipped claim FALSE is a correctness defect, not an editorial one, and it opens a cycle like any other correctness defect.

- **Cite by symbol, so the citation survives the cycle.** The stop condition above names "drifted line citations" as a recurring editorial finding class. The cheapest way to stop paying for it is not to create it: when writing a finding, a repair note, or `## Progress Log` evidence, cite a **resolvable anchor** — a function, class, method, constant, test name, or distinguishing expression — rather than a bare `file:line`. A symbol anchor resolves with `code_definition` and `code_read`, so a reviewer re-reading it after the file has moved still lands on the right code; a line anchor can only be re-checked, and re-checking is the cost. This matters most during implementation, because the files under review are the files being edited, so cited lines drift *within the same cycle* that cites them.

  A line number remains correct where no symbol contains the site — a module-level constant block, a data file, a specific line in a generated artifact, or prose in a hand-authored markdown document — and where the citation is deliberately historical. **Name the case inline when you rely on it.** Line numbers already written into `## Progress Log` and `## Decision Log` rows are records of what was verified when; repairing them to symbols falsifies the history and is not an editorial repair.
- **An excluded section narrates; it must not amend.** A scope, requirement, or acceptance-criterion change is recorded in the section that owns it (`## Scope`, `## Requirements`, `## Acceptance Criteria`, `## AC Priority`, `## Tasks`), and the Progress Log row points at that edit rather than substituting for it. This is what makes the stop condition safe: the review-policy digest treats an excluded section's content as narration carrying no reviewable claim, so an amendment announced only there would never be reviewed. Announcing a scope change in an excluded section alone is the defect, not the record of it.

  The rule is stated as a property of the CLASS rather than as a list of sections, so it does not need re-editing each time the exclusion set grows — enumerating regions here would reproduce exactly the drift the stop condition above was just reworded to avoid.

  **Editing the canonicalizer is an exclusion-set change, and it re-digests the whole repository once.** `canonical_review_policy_body` and its normalizers decide what the digest sees, so changing them changes every change document's digest at the same time, lapsing every readiness approval in every open wave — with no document edited. Treat it like any other one-time transition: disclose it in the change doc, expect a single re-Prepare per open wave, and do not make the edit while waves are readied but unclosed without saying so.
...
5. `code_callhierarchy` — direct callers and callees with exact line numbers and snippets; use when the question is about structure ("what calls X?" / "what does X call?"). For languages whose cross-file resolution is less mature (Swift, Java, Kotlin, C/C++/C#, ObjC, Ruby, PHP, Scala), if a `code_callhierarchy` result is empty for a symbol you can independently verify exists, fall back to `code_references` — it uses text-based search and finds the call sites regardless of graph-extractor coverage. **AOP/advice exception:** if the empty result is on a Java method in a class with `@Advice.OnMethodEnter`, `@Advice.OnMethodExit`, `@Around`, `@Before`, `@After`, `@AfterReturning`, or `@AfterThrowing` annotations, do NOT fall back to `code_references` — the callers are wired at weave time by ByteBuddy/AspectJ and have no Java call sites. Instead search for the advice registration via `code_keyword(queries=[<advice_class_name>], glob="**/*Instrumentation*.java")` to find the `TypeInstrumentation.transform()` / `@Aspect` pointcut declaration; that registration IS the caller. The `code_callhierarchy.external_outgoing_count` / `external_incoming_count` fields surface how many external (non-project) entries were suppressed; pass `include_external=true` to inline them. Inheritance gets the same treatment: `supertypes` (with always-on `external_implements_count` / `external_extends_count`) surfaces a class's declared supertypes even when they are external, and `code_impact(symbol=<external interface name>)` answers "what implements it?" across the project.
6. `code_impact` — transitive upstream callers up to N hops; use when the question is about safety ("what breaks if I change X?"). Run before modifying a shared symbol to size the full blast radius across the codebase before the first edit. `code_callhierarchy` and `code_impact` are NOT interchangeable: `code_callhierarchy` answers structural questions (depth 1, line numbers) and will underestimate the blast radius; `code_impact` answers safety questions (transitive, file-level affected set). For pre-edit safety checks, run `code_impact` first. The `path=` heuristic mode only parses imports in Python/JS/TS/Go/Rust; for other languages it returns `unsupported_language: true`. Use `symbol=` (graph mode) for impact analysis on Swift/Java/Kotlin/C/C++/C#/etc.
7. `code_keyword` — exact token or string matches on live files
8. `code_lexical` — BM25-ranked exact-token search over the indexed chunk corpus (compound identifiers, error strings, rare tokens; the FTS layer the hybrid retrieval fuses). Tokens keep `_` inside — query the full compound identifier, never a partial. Also the verification tool for "is X in the lexical layer?" (reports coverage; warns when the store is under-covered). Regex stays with `code_pattern`.
9. `code_outline` — before a broad `code_read` on a large file
10. `code_callgraph` — call tree beyond 1 hop (depth > 1) when broader call structure is needed
11. `rg` / `grep` / broad file reads — fallback only
...
This rule is measured, not just stated: `wf_implement_wave`'s activation response carries it as a `retrieval_posture` field, and a `retrieval_posture_gap` advisory fires at implementation review and close when implement-stage retrieval telemetry is near zero against a non-trivial code diff. Legitimate fallback (bulk-mechanical edits, docs-only work) is fine — record the `Gapfill:` note and the advisory clears; the gate is explain-or-fix, never a hard block.

**Delegating code work to a subagent — the rule travels with the work.** When you spawn a subagent for code investigation or implementation, route it through a role-typed agent that carries this MCP-first contract (`implementer`, `guru`, a builder specialist) **or** include the MCP-first code-navigation directive in the subagent's prompt. Check the receiving worker's actual MCP tools and any explicit `tools:` allowlist; parent access alone does not establish inheritance. Use the capability fallback in **Host-neutral orchestration** when access is unavailable, and keep MCP-first when it is available. A bare general-purpose subagent spawned for code work without the directive is the same shell-by-habit defect as in the main thread. When fanning out code-investigation lanes, **name the MCP tools in the delegated task** (and the schema-load step when the host defers them, e.g. ToolSearch) — an unnamed tool loses to the zero-setup shell path every time. Require the same `Gapfill:` note from spawned subagents when they fall back, and surface it. (Do not restate the exploration order above in the subagent prompt — point to it.)

`code_ask` is the wrong tool for a direct navigational lookup of a **known** symbol: use `code_definition` + `code_callhierarchy` directly — they answer more precisely (exact line numbers and call sites vs synthesized prose). `code_ask` now has a single agent-mode ranking path: it scores the retrieved docs+code pool with a cross-encoder when available (FP16 on GPU, INT8 on CPU), then applies the agent coverage floor and text budget; `rerank="local"` is only a deprecated alias for the same path. So the *latency* reason to avoid `code_ask` no longer applies — the *precision* reason for going direct on a known symbol still does. When a broader `code_ask` question names one symbol that resolves unambiguously in the current published graph, the exact declaration is defensively placed first while broader citations remain. Agent mode also returns a **`graph_related`** section — structural `callers` / `readers` / `importers` grouped by relationship, separate from the textual `citations` — a real "what calls/reads X" answer when the symbol isn't known yet. Use `code_ask` for synthesis across unknown files and layers; for a known symbol, go direct — not "try `code_ask` then bail." `vector_ms` / `rerank_ms` are diagnostic fields for evaluation reports, not a runtime routing signal.

**Constant values are retrievable** (wave `1p4hi`, all languages): module-/type-level constants are now their own chunks, so `code_ask` / `code_search` surface a constant's **value** (e.g. "what is the value of `X`"), `code_definition` resolves it, and `code_references` lists its reader functions in a distinct **`reads`** bucket — separate from call sites.

Investigation patterns — chain tools rather than pick one:

- **Bug investigation** (defect's symptom function is known): `code_callhierarchy(direction="incoming")` on the symptom function to identify the conditional that selects the buggy branch → `code_impact(symbol=..., max_hops=3)` for the full transitive entry-point set → `code_keyword(queries=[<bug_conditional>, <inverse_conditional>], glob="**/*.<lang>")` for an exhaustive catalog of all sites flipping the conditional → optionally `code_graph_path(direction="forward")` to confirm a specific entry point reaches the symptom.
- **Refactor / API change** (changing a shared symbol's signature, return type, or contract): `code_callhierarchy(direction="incoming")` for direct callers with line numbers → `code_impact(max_hops=3)` for transitive callers → **read the `community:` field on each affected node**. If all callers are in one community, the change is contained. If callers span multiple communities, the change is cross-cutting and likely needs architecture-reviewer or a Wave Council readiness pass per seed 214. Before moving or renaming a definition, also search tests and fixtures with the exact-token tool for the old owner named in strings (mock/patch targets, path literals, source-text censuses and import allowlists), which the call graph misses; classify every hit in the plan before the first move as an update or intentionally retained with a reason.
- **New feature analogue** (adding a feature analogous to an existing one): start at `wavefoundry://graph/communities` (resource read) to identify the analogue's community → `code_graph_community` drilldown to see the analogue's public-API surfaces (top-degree members) → `code_callhierarchy(direction="incoming")` on those top-API members to find the integration sites the new feature will plug into → `code_callhierarchy(direction="outgoing")` to find shared helpers the new feature will need → `wf_graph_report(sections=["fan_in", "chokepoints"])` for shared infrastructure to route through.
- **Impact analysis** (operator asks "what breaks if I change X?"): run `code_impact(symbol=X, max_hops=3, include_tests=true)` AND `code_impact(symbol=X, max_hops=3, include_tests=false)`. The difference between the two counts shows how much "breakage" is in test infrastructure vs production callers. Chain with `code_callhierarchy` per affected node for line-number evidence (`code_impact` returns no per-edge lines). Also run `code_keyword(queries=[<X_name>], glob="**/*")` to catch non-code references (comments, doc citations, log strings) the graph doesn't model — important for renames.
- **Edge case discovery** (writing tests or doing security review on a function with branching logic): `code_outline(<file>)` → `code_read(<file>, start_line=N, end_line=M)` for the function body → `code_callhierarchy(direction="outgoing")` to find guard helpers (input validation, throttles, retry helpers) → recurse on each guard helper. For language-specific early-exit patterns scoped to the file, use `code_keyword(queries=<project.code_navigation_hints.guard_tokens>, glob="<file>")` if the project has declared `code_navigation_hints` in `docs/workflow-config.json` (matches the existing `design_review_triggers`/`architecture_triggers` schema; project owners tune tokens to local convention).
- **Cross-cutting / architectural questions** ("where do we handle X?" or "what's the relationship between A and B?"): `wavefoundry://graph/communities` resource read for community ownership → `wf_graph_report(sections=["chokepoints", "orphan_docs"])` for hotspot chokepoints and unlinked design docs → `code_graph_path(direction="either")` to confirm coupling between modules (`either` surfaces backward edges through shared state — required for event-driven / reactive / AOP codebases where data flows backward through shared mutable state rather than forward through method calls).

Anti-patterns (these come up in real usage):

- `code_callhierarchy.outgoing` mixes function calls, init/constructor calls, and method calls. Don't report "X calls Y" when Y is a struct initializer (`kind: "class"` in the entry).
- `code_callhierarchy.incoming` is per-caller-function, not per-call-site. If A calls B three times, incoming shows ONE entry for A. For per-call-site counts use `code_references`.
- `code_graph_path` traverses `defines` and `imports` edges as well as `calls`. A `found: true` result can route through `Module --defines--> method` without a real call. Inspect each `path_edges[i].relation` before treating the path as a call chain.
- `code_callgraph` at `depth > 1` on chokepoint symbols (top of `wf_graph_report.fan_out`) often blows the 25K token cap. Default to `depth=1` for chokepoints, or restrict by file glob.
- `code_impact path=` heuristic mode only supports Python/JS/TS/Go/Rust. For Swift/Java/Kotlin/C/C++/C#/etc. it returns `unsupported_language: true` in ~1 ms — use `code_impact(symbol=...)` graph mode instead.
- Empty `code_callhierarchy` / `code_impact` on a symbol you can verify exists is a signal that graph extraction for this language is incomplete, not that the symbol has no callers — fall back to `code_references`. EXCEPT for Java AOP/advice methods (see the AOP exception on tool 5 above).
- `code_keyword` without a `glob` in a multi-language repo returns doc, code, comment, and string-literal hits indiscriminately. Always `glob="**/*.<lang>"` when scoped to one language.
- A direct known-symbol lookup belongs in `code_definition` + `code_callhierarchy`, which is more precise than `code_ask`'s broader citation workflow. Do not start with `code_ask` and bail on `rerank_ms`; use `code_ask` when the question genuinely needs synthesis, where its defensive declaration-first correction keeps a confirmed owner ahead of usages without removing context.
...
Use the best available model and reasoning effort for each specific task throughout Prepare, plan review, implementation, verification, delivery review and Close. The operator may choose a different agent host for each wave or hand review to another host. Coordinator, worker and reviewer are responsibilities, not fixed model tiers; no provider, model, reasoning level or native team UI is mandatory.

- **Choose for the task.** Respect operator choices and use only model controls actually available in the host. Match capability and effort to ambiguity, domain depth, consequences of error, context needs and observed performance. Use the most capable available model when its judgment is needed; use a lighter model only when it meets the same quality bar for that task. Optimize total verified effort, including retries, rework and integration, not just per-call cost. Reassess at task boundaries or when results reveal a mismatch; do not default to either premium models for everything or cheap workers by role. If model selection is unavailable, use the current model and state the limitation. Delegation-time selection is primary: use per-invocation model and effort controls when the host exposes them, honoring operator constraints. Otherwise use available defaults and state the limitation; inheritance alone is not a task-fit decision. Do not bake provider/model names or capability rankings into the policy.
- **Record the choice briefly.** Use the existing work-allocation or review narrative for one concise model/effort rationale per materially distinct assignment; similar routine tasks may share one rationale. Distinguish requested settings from observed runtime identity. When the host does not expose the actual model or effort, record unknown. This adds no telemetry store, model enumeration, benchmark requirement or approval gate.
- **Delegate useful independent work.** Check actual delegation tools, capacity and the receiving worker's tool access. Before delegating, weigh briefing, waiting and integration overhead against the expected benefit; keep small, tightly coupled work with the coordinator when delegation would cost more than it helps. Give each assignment its relevant ACs, dependencies, owned and forbidden paths, interface constraints, MCP-first retrieval direction, verification expectations and bounded deliverable. Serialize overlapping writes; continue independent coordinator work while workers run. Do not fill slots for their own sake or copy the whole conversation by default. Reuse useful worker context for follow-up; rebrief or replace stale context. Set an early, task-sized checkpoint for a useful draft, finding or executed test, without creating another artifact or approval gate. If that checkpoint yields no useful progress, diagnose the blocker and promptly rebrief, change the model when available, or reclaim the task; do not keep waiting merely because the worker is still running.
- **Use honest fallbacks.** If delegation is unavailable or inappropriate, perform implementation tasks sequentially and state that choice. Parent MCP access does not prove worker MCP access. Use MCP-first where attached; otherwise use the existing documented retrieval fallback and record the same `Gapfill:` explanation. Never invent a tool, model switch, host feature or quota. Missing capability does not authorize installing or reconfiguring a host outside the task's authority.
- **Accept integrated evidence.** Workers return evidence using seed 209's **Briefing Packet**, including available optional implementation evidence, and identify AC coverage and blockers. The coordinator rereads the actual changes, verifies integration, and maintains the existing Agent Execution Graph and Progress Log. Worker completion is not wave completion. Keep elapsed-time, usage and rework observations in existing evidence when available; say when measurements are unavailable and make no unmeasured savings claim.
- **Preserve independent review.** Required review uses a fresh independent context, on the same host or another. A different vendor alone is not independence, and relabeling the implementer is not independent review. When no independent context is available, authorized implementation may continue but required review remains pending. Model fit never waives stage gates, required lanes, write ownership or operator authority over closure, commits and releases.
- **Hand off through existing artifacts.** For review or coordinator handoff, update the existing wave/change record using seed 209's **Briefing Packet**, including the reviewed revision or tree fingerprint, and record the next action. The receiver checks the current tree and evidence independently; a moved tree requires affected evidence to be refreshed. No shared chat transcript, extra Markdown packet, host/session registry or cross-host scheduler is required.
...
- keep the wave artifact rooted at `docs/waves/<wave-id>/`, where the `wave-id` uses the shared base36 lifecycle prefix plus the summary slug that best reflects the admitted changes
...
- all participants should leave enough state in wave artifacts (Progress Logs, Decision Logs, the session handoff) for another agent to continue safely
...
10. Execute the implement loop per the **Implement loop execution model** above (Thought → Action → Observe → Reflect, with Level 1/2/3 escalation). Defaults apply as specified there; do not restate them inline. Run `wf_run_sensors()` when computational sensors are configured and fix sensor failures.
11. Coordinate execution until the wave objective is satisfied or the wave must be reconsidered.
12. Reconcile partial completions, blocked changes, retries, deferred work, or moved work as execution proceeds. At an explicit validation or handoff boundary, verify the full docs gate with `wf_validate_docs` when MCP is attached, using the repository CLI fallback otherwise.
...
14. Update wave artifacts, progress state, reviewer status, change state, dependency state, and lifecycle timestamps. When a durable operating-memory signal appears — operator directive, high-salience risk, bug caught in review, hard-to-find constraint, reversed decision, invalidated assumption, or confidence-shift — capture it as a typed memory candidate (`memory_add(status='candidate', ...)`) with evidence references while it is fresh; the close-time validation checkpoint triages every candidate. Do not record tasks that completed normally on the first attempt.

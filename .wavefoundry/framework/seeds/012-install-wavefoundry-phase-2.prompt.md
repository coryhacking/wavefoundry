...
**Optional session checkpoint:** After row 2.8 is verified, save a concise handoff and stop if context is low. Leave row 2.9 unchecked until all its outputs pass audit; no extra install-log rows are needed.

**Action:** Read `seed-100`. Generate `docs/prompts/*.prompt.md` for every public framework prompt and `docs/prompts/prompt-surface-manifest.json`. Include the public-prompt entries for seeds 175 (`review-plan`) and 176 (`evaluate-decision`) — these are easy to miss; verify they're present. **Review plan** is the primary optional plan stress-test phrase and `wf-review-plan` is its only skill; **Interrogate this plan** and **Stress-test this plan** remain natural-language aliases only. Keep this no-signoff workflow distinct from **Review wave** / `wf-review-wave`, which runs required delivery-review lanes and records typed evidence. Do NOT generate a migrate-journals prompt at install time (seed 210 is upgrade-only: fresh installs have no journals to migrate).

On resuming partial 2.9 work, inventory actual prompt files and manifest entries against seed-100's current public-prompt catalog and conditional rules. Preserve project-authored content, create or reconcile only missing or stale required surfaces, and avoid a fixed prompt-file count. Once required prompt and role sources exist, call `wf_sync_surfaces(mode='run')` when MCP is attached; CLI fallback: `wf render-surfaces`. Its renderer populates owned upgrade-policy regions, including `wavefoundry:review-policy-upgrade` when `wave_review.delivery_mode` is explicit. Empty marker pairs alone are incomplete; do not hand-copy `UPGRADE_POLICY_BLOCK` or edit authored prose outside the owned region to fake completion. Run `wf_audit_install` after rendering and mark 2.9 complete only when the actual catalog, manifest, owned regions and audit pass. Do not suppress validation errors to advance the log.

...
**A visible caller does NOT mean live, and a guard does NOT mean dead: two reachability classes, two probes.** The rule above covers **node reachability** (is there any caller the graph cannot see?), answered by `code_references` + `code_callhierarchy(direction="incoming")`. A conditionally-guarded branch — a fallback, degradation, retry, cold-cache, or compatibility path behind a sentinel or predicate — is a **condition-reachability** question: it HAS a caller, and the removal argument is "the guard is never true." No call graph answers that; a `calls` edge carries no path predicate. Before recommending removal of such a branch:

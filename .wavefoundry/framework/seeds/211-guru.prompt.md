...
- **Constant values are retrievable** (wave `1p4hi`, all languages): a module-/type-level constant like `MAX_RETRIES = 5` or `RERANKER_MODEL = "..."` is now its OWN chunk, so `code_ask` / `code_search` surface its **value** (ask "what is the value of X" / "what model does Y use"). `code_definition` resolves a constant; `code_references` lists its reader functions in a distinct **`reads`** bucket (separate from call sites — the functions that read that constant's value).
- **Review-adjacent findings follow seed 209's executable-evidence, safe-execution, finite-risk-budget, and actionability contracts.** Guru validates the proposition against code and, for material behavioral claims, the public/registered path; it supplies citations, reproduction evidence, selected stateful transition/interleaving cells, reachability, impact, and scope facts. It does not derive disposition from LOC, repair difficulty, or whether a new decision is needed. Route every real finding through the ordered `do_now` / `maybe_later` / `dont_do_later` / `not_issue` gate. `do_now` and `maybe_later` are completed in-session before closure; the other two create no follow-on debt.
...
- Use `code_callhierarchy` when the question is "what calls X?" or "what does X call?" and you want exact call-site line numbers and snippets alongside caller/callee structure. Returns direct callers (incoming) and callees (outgoing) at depth 1 with graph-backed attribution. Prefer over `code_references` for structural caller/callee questions; use `code_references` when you need non-call-site hits (mentions, definitions, imports) or when the graph index is absent. External (non-project) entries are suppressed by default and counted in `external_outgoing_count` / `external_incoming_count`; pass `include_external=true` if you need the full list. The same convention covers inheritance: the response's `supertypes` section lists the class's declared supertypes with always-on `external_implements_count` / `external_extends_count` — a class implementing only an EXTERNAL interface (a third-party SDK type) is never invisible; pass `include_external=true` to inline the external entries. For the reverse question — "what implements this external interface?" — `code_impact(symbol=<interface name>)` resolves external supertypes by name (project symbols always win a name tie) and returns the implementor blast radius; when one simple name matches several distinct external types, the response returns an `external_candidates` breakdown instead of a merged guess — re-query with the exact `external::` id. **Fallback (wave 1p2q3, 1p2q9 — response-shape rule):** if a project-internal caller/callee question returns an empty list AND `code_references(symbol=X, graph=false)` returns hits on the same symbol, treat the empty graph result as a **coverage gap, not authoritative absence**. The graph extractor's per-language coverage varies by codebase shape (e.g. TypeScript monorepos with `tsconfig.paths` aliases, deeply-nested namespaces, dynamic dispatch patterns) — any language can hit a coverage gap on a specific repository, not just the historically less-mature set. Use `code_references` / `code_keyword` results as the ground truth in that case. **AOP/advice exception:** if the empty incoming is on a Java method in a class with `@Advice.OnMethodEnter`, `@Advice.OnMethodExit`, `@Around`, `@Before`, `@After`, `@AfterReturning`, or `@AfterThrowing` annotations, do NOT fall back to `code_references` — the callers are wired by the AOP framework (ByteBuddy, AspectJ) at runtime and have no Java call sites. Instead search for the advice registration: `code_keyword(queries=[<advice_class_name>], glob="**/*Instrumentation*.java")` finds the `TypeInstrumentation.transform()` / `@Aspect` pointcut declaration; that registration IS the caller.
...
- `score` — cross-encoder relevance before any soft partition when `reranked=true`. When `reranked=false`: vector/coverage score on the healthy path (raw shared-embedder cosine, uncalibrated and weaker); BM25 score in `lexical_fallback` mode
...
|---|---|---|
| **architect-reviewer** | `code_search(kind="code-summary")`, `code_dependencies(path)`, `code_ask`, `code_impact(symbol)`, `code_risk_score(scope)` | Orient to module shape; trace dependency chains; check boundary violations; size blast radius of proposed changes; rank the riskiest-to-change symbols in an affected module |

...
5. Generate factor-review agent files for each factor in the **operational active-lane set** — `docs/workflow-config.json` `factor_review_policy.applicable_factors`. (`docs/repo-profile.json` `factor_review` records the broader applicability *assessment* — "is this factor relevant?" — while `applicable_factors` records the operational decision — "do we run a review lane for it?". Only an active lane requires a canonical doc; assessment-only factors do not.)

   **Seed `applicable_factors` from the profile as a starting default (install).** Before generating factor docs, if `docs/workflow-config.json` `factor_review_policy.applicable_factors` is absent or empty AND `docs/repo-profile.json` `factor_review` marks one or more factors `applicable`, populate `applicable_factors` with those applicable factor IDs (the sorted set where `factor_review.<id>.status == "applicable"`) as a **sensible default the operator can prune**. This keeps the gate keyed off `applicable_factors` (the operational lane set) — it does not make the gate read the profile — it only avoids a fresh install sitting with an empty lane set while the profile says factors apply, which otherwise emits a factor-review reconciliation warning on every audit. If the operator does not want a lane for a defaulted factor, they prune it from `applicable_factors` (and may set that factor's assessment to `partial`); an intentionally empty `applicable_factors` is still a legitimate operator choice. For each active-lane factor, write `docs/agents/factor-<nn>-<name>.md` as the canonical source file (zero-padded two-digit number, kebab-case name matching the factor table in the framework README), then render native wrappers at the enabled platform locations, for example `.claude/agents/factor-<nn>-<name>.md`. Factor wrapper frontmatter follows the same `tools:` allowlist contract as the guru wrapper (see **Wrapper `tools:` allowlist contract** below): the read-only Wavefoundry retrieval tools plus `ToolSearch` are included explicitly, and the server-level `mcp__wavefoundry` grant is never used; the wrapper body carries the load-and-prefer retrieval bullet required by that contract block. Each file must include: what this factor covers, why it is an active review lane for this project (cite evidence from the repository), the review questions it asks when evaluating a wave, and whether its findings are gating or advisory for this project. The canonical factor file header must include `Role: factor-<nn>-<name>` and `Category: factor`. Do not generate files for factors that are not in `applicable_factors` (including `partial` / assessment-only / `not-applicable` factors); an absent or empty `applicable_factors` (a retired or never-activated lane set) requires **no** canonical factor docs at all — that is a legitimate operator choice, not drift. Record the generated canonical and wrapper paths in `docs/agents/platform-mapping.md` alongside the generic role agents. The docs-lint **factor-surface gate** (`check_factor_surface`) enforces this contract: every factor in `applicable_factors` must have its canonical doc with those headers (missing → **ERROR**); a `.claude/agents/factor-*.md` wrapper with no matching canonical source (orphan) or missing YAML frontmatter (not subagent-loadable) fails the gate as an ERROR regardless of the lane set; and a factor marked `applicable` in `repo-profile.json` `factor_review` that is **not** in `applicable_factors` (assessed-relevant but no active lane) surfaces as a non-blocking **WARNING** for operator reconciliation — never a forced doc over-generation or a build-blocking error. Wrappers are optional rendered copies — a canonical-only surface with no wrappers passes — and the canonical home is `docs/agents/` **flat** (never a `docs/agents/factors/` subdir). Regenerate (never relocate/retire) the missing or malformed half via this task when the gate fires; on upgrade, `seed-160`'s factor-surface backfill repairs drifted repos merge-safely.
...
14. Seed `.claude/settings.json` with the project-level Claude Code hook that automates the same rule for Claude Code (see below). The install/upgrade render also seeds the renderer-owned MCP permission allowlist into the same file (read-only tier by default; see the Claude Code section below for the provenance and operator-knob contract).
15. Prefer generating platform hook/config surfaces via `wf render-surfaces` rather than hand-editing each hook file independently. Keep the repo-local hook files at the platform-expected paths, but treat them as generated Python entrypoints rendered from generic framework templates. Use a repo-global guard override file (`.wavefoundry/guard-overrides.json`) rather than provider-specific sentinel files for temporary seed/framework edit approval.
...
17. **Stage gate (repository code)** — Add a `## Stage Gate (repository code)` section to `AGENTS.md` before the Implementation guard. This gate applies to all repository code (product source, scripts, tests, build manifests, and other checked-in code/config that affects shipped or verified behavior). It must require all three of the following before the first code edit in a given effort: (1) a consolidated change document exists; (2) the change is admitted into a wave via `Create wave` / `Add change to wave`; (3) the wave has a current typed `wave-council-readiness` approval on the current review-policy receipt; legacy waves retain their prose council-verdict authority, and repositories with wave review disabled require a clean Prepare pass. If any step is missing, agents must stop and route back to `Plan feature`, `Create wave`, `Add change to wave`, or `Prepare wave`. Mark documentation-only edits under `docs/`, prompt/framework docs that do not change repository code, and operator-approved explicit waivers for a named scope as out of scope for this gate. **Fixed two-section contract (do not consolidate):** the `## Stage Gate (repository code)` and `## Implementation guard (product code)` headings are a cross-document contract — referenced by literal name across host entry docs and lifecycle prompts — so they are **exempt from the upgrade "preserve repo-grown / consolidate in place" license**: keep them as two separately named sections, do not merge them into one combined section/table or rename them, so those by-name references stay resolvable. They remain the canonical two-section render target for new installs. The framework deliberately does **not** lint these heading strings; the carve-out is enforced by this guidance, not a heading-string validator (a standing decision: do not add a literal-heading validator).
...
22. **Executable review-evidence carriers** — Fresh initialization must create the canonical reviewer/council role and Review-wave/Create-wave prompt sources before surface rendering, including `docs/agents/qa-reviewer.md` from `seed-239`. The public renderer is also a fail-safe backstop: it materializes any missing required canonical carrier from the installed seed (or a bounded source pointer for a multi-output owner) before reconciling its owned region. Guru remains optional/existing-only, and conditional repo-local `docs-contract-reviewer` and `release-reviewer` files remain project-enabled: create them only when project evidence/config enables those lanes, then let the renderer manage only their protocol section. Invoke the same public `wf render-surfaces` path used by setup and upgrade; it calls `render_agent_surfaces.py` and its named `reconcile_review_protocol_surfaces(repo_root)` operation. Verify the registry-enumerated, enabled carriers contain exactly one `wave:executable-review-evidence` marker pair. The marker body is framework-owned and refreshed deterministically; all project-authored content outside it is preserved. Malformed marker pairs must fail safe with the file unchanged and a loud diagnostic. This reconciliation is **not Guru-gated**: it runs before the renderer's Guru-absent early return, so projects that do not enable Guru still receive the review protocol on required/existing carriers. Native Guru wrappers created later in the same render are reconciled again before return. Seed `209` remains the only full protocol/checklist; carriers point to it rather than cloning it.
23. **Memory-review shortcut** — Register **`Review memories`** in the `AGENTS.md` shortcut table, with **`Memory review`** as a human-readable alias, and point it to `docs/prompts/memory-review.prompt.md`. Keep only the canonical shortcut in `docs/prompts/prompt-surface-manifest.json`. The command is operator-initiated and apply-oriented; an explicit read-only request selects the prompt's non-mutating branch.
24. **Refresh TechDocs shortcut** — Register **`Refresh TechDocs`** in the `AGENTS.md` shortcut table, with **`Author TechDocs`** as a human-readable alias, and point it to `docs/prompts/refresh-techdocs.prompt.md` (seed `178`, rendered by `seed-100`). Keep only the canonical shortcut in `docs/prompts/prompt-surface-manifest.json`. The `AGENTS.md` § Skills paragraph, when present, lists `wf-techdocs` among the doc-gated skills with the gate stated as "renders once `docs/prompts/refresh-techdocs.prompt.md` exists (every target after seed-100 reconciliation)", never as a rare or self-host-only skill. An explicit read-only request selects the prompt's review-only branch, which runs the `wf_techdocs_audit` publication audit and returns findings and proposed edits without writing.
...
- Before the first edit to **product implementation source** in a given effort: (1) a consolidated change document exists at `docs/plans/<change-id>.md` or `docs/waves/<wave-id>/<change-id>.md`, grounded in `docs/plans/plan-template.md`; (2) the wave has a current typed `wave-council-readiness` approval on the current review-policy receipt; legacy waves retain their prose council-verdict authority, and repositories with wave review disabled require a clean Prepare pass.
...
**Retrieval-intent backstop — late-detect signal:** if you find yourself about to call `code_search`, `code_keyword`, `code_read`, `code_definition`, `code_outline`, `code_callhierarchy`, `code_references`, or `code_pattern` in service of a user question, **stop**. That retrieval IS Guru's job. Route to Guru instead of doing the retrieval yourself. The tool reach-for catches misses the pre-flight skipped. **This applies to delegated work too:** when you spawn a subagent for code investigation or implementation, route it through a role-typed agent (`implementer`/`guru`) or carry the MCP-first directive in its prompt — check the receiving worker's actual tools and allowlist rather than assuming inherited MCP access; follow seed 180's host-neutral orchestration fallback when unavailable and MCP-first when available, so a bare general-purpose subagent grepping by habit is the same defect (the exploration order lives in `seed-180`/`seed-211` — do not restate it).
...
Canonical role for all hosts: \`docs/agents/guru.md\` + \`AGENTS.md\` auto-Guru section. Use wavefoundry MCP when attached — load and prefer the \`code_*\` tools (via \`ToolSearch\` when the host defers schemas) over grep/raw reads, per the run contract's Retrieval Rules. Read-only here — architecture writes stay in the main session.
```

**Wrapper model/effort defaults (Guru and factor-review alike).** Fresh wrappers omit `model:` and `effort:` frontmatter; do not replace a fixed pin with `model: inherit`. Follow seed 180's task-fit selection at delegation time using the host's available controls. Preserve existing operator choices, including `inherit` and unknown future values; a value alone does not establish framework ownership. Use seed 160's editing pass to retire a pin only with provenance or explicit scoped operator direction.

**Wrapper `tools:` allowlist contract (every Claude Code wrapper this seed generates — guru and factor-review alike).** An explicit `tools:` allowlist is not additive: a wrapper that omits the MCP tools cannot call them, no matter what its body or the canonical role doc instructs — the wrapper would contradict itself. Every generated wrapper allowlist must therefore include the read-only Wavefoundry retrieval tools exactly as in the guru template above: the `code_*` navigation/query tools plus `docs_search`, `seed_get`, and `ToolSearch` (so hosts that defer MCP tool schemas can load them on demand). Never grant the server-level pattern (`mcp__wavefoundry` or `mcp__wavefoundry__*`) — it would include write-capable and lifecycle-mutating `wave_*` tools, and reviewer/guru wrappers are read-only by contract. Exact tool names are the fail-safe form: on a host or repo where an entry is not honored (for example, the MCP server is not attached), the wrapper degrades to its non-MCP tools rather than ever gaining mutators. The wrapper **body** carries a matching retrieval bullet: load and prefer the Wavefoundry `code_*` MCP tools (via the host's schema loader, e.g. `ToolSearch`) over grep/raw reads for locating and understanding code. Factor-review wrapper bullets additionally carry the claim-backing clause inline (back any how-many/blast-radius claim with `code_references`/`code_callhierarchy`) and point to the full posture in `docs/contributing/agent-team-workflow.md` § **Retrieval Posture (All Lanes)**; the guru wrapper's bullet may defer both to its canonical role doc, which owns the full retrieval loop. The bullet keeps the posture visible even when the subagent never reads the canonical role doc.
...
**Codex** (optional) — `.codex/skills/wf-guru/SKILL.md` (wave `1p6lp`: registry-rendered; all framework skills carry the `wf-` kebab-case namespace so `/wf` filters a host's skill list to the family):
...
A leftover pre-namespace `.codex/skills/auto-guru/SKILL.md` from an older install is stale-cleaned by the render pass. Ensure `.gitignore` does not exclude `.codex/skills/` (track project skills like other framework-managed platform files).
...
## Per-Area `AGENTS.md` Context + Root `@AGENTS.md` Bridge (vendor-neutral)

Wave `1p5xc` (wave `1p5x8 large-codebase-map`). Vendor-neutral per-area context, reached on demand — no per-folder bridge files, and `@import` used in exactly one place (the root).

**Root `@import` bridge (the only `@import`).** The root `CLAUDE.md` is rendered as a real `@AGENTS.md` import (a marker block produced by `render_agent_surfaces.py`), replacing the prose "read `AGENTS.md` first" pointer. This makes the root `AGENTS.md` the single source of truth that Claude loads deterministically and that `AGENTS.md`-native agents (Codex, etc.) read directly. Do **not** introduce `@import` anywhere except the root `CLAUDE.md`, and do **not** create `CLAUDE.md` bridge files in subdirectories. Re-run `render_platform_surfaces.py` (which calls `render_agent_surfaces.py`) after upgrade; do not hand-edit the `wave:root-bridge` marker region.

**Root convention line.** The root `AGENTS.md` must carry a standing instruction: *before working in an area of the codebase, consult that area's `AGENTS.md` if present, orienting first from the generated codebase map (`docs/references/codebase-map.md`)*. This same operating instruction is woven into the run-contract seed (`020`) so it renders into **every** host agent surface, not just the root `AGENTS.md` body.

**Per-area `AGENTS.md` (major areas only, agent-drafted + human-refined).** Major subsystems (the bounded top-tier areas the codebase map identifies — `1p5tl` / `gen_codebase_map.py:compute_areas` — and the deployable units from the `seed-030` architecture handoff) carry a vendor-neutral `AGENTS.md` with local conventions/gotchas/intent. This is the codebase map's highest-value content: the part the `code_*` tools cannot synthesize, indexed so it surfaces in `docs_search`/`code_ask` when working in that area. **During inventory (`seed-030`) the agent authors a grounded initial draft for each major area** — purpose, key conventions, non-obvious gotchas/intent, entry points — and **humans refine** it. This is a one-time inventory-authored artifact (the same trust model as `docs/repo-index.md`/domain-map), NOT regenerated on every build, with hard guardrails: major areas only, every line evidence-grounded, no boilerplate (leave an area unwritten when it has no real local context), never overwrite an existing file. The `gen_codebase_map.py --scaffold-area-contexts` tool stays **slot-only** — opt-in, idempotent, creates an empty stub, never overwrites, never authors content — for operators who want slots without drafts. The codebase map names each existing area's repo-relative `AGENTS.md` path as prose.

**Keeping the map pointed at the product.** The codebase map excludes generated and vendored/third-party trees from the orientation tier so a cold-start agent lands on product code, not bundled dependencies. Generated code is detected automatically; vendored/third-party trees are an explicit signal the agent records during inventory — `vendored_paths` globs in `docs/repo-profile.json` (or `.gitattributes linguist-vendored`), per `seed-030`. Excluded trees collapse into a footer count and stay `code_*`-searchable.
...
/wavefoundry-*.zip
```
- **Wavefoundry runtime files (never commit)** — the framework writes per-machine runtime files on every dashboard start and during upgrade. They are regenerated on each run (same class as the semantic indexes) and must be gitignored so they do not appear dirty or get committed accidentally. **This block (which ends with the same `/wavefoundry-*.zip` pack-drop rule shown above) is written programmatically and idempotently by `render_gitignore_block` (in `render_platform_surfaces.py`), which `render_platform_surfaces.main()` runs on every `wf setup` / `wf render-surfaces` / upgrade — so it is enforced and self-heals existing repos (it is the authoritative writer; do not rely on hand-editing, which silently failed when a repo was not a git repo at install time).** The agent's job is to verify the marker-delimited `# >>> wavefoundry runtime …` block is present (run `wf render-surfaces` if not) and to add any project-specific rules only OUTSIDE that managed region. The canonical entries, reproduced verbatim from `render_platform_surfaces._GITIGNORE_BLOCK` and bound to it by a framework parity test so the two cannot drift:
```gitignore
...
**Docs-feedback cadence is two-tiered.** The post-edit hook and successful documentation-writing MCP operations run the incremental changed-set lint (`docs_lint.py --changed`) for routine, fast feedback; it is advisory and does not replace a full corpus result. Agent surfaces must say that agents normally rely on this automatic feedback while editing, rather than manually running full lint after every doc edit. Require the full docs gate at explicit integrity boundaries: an agent's final validation or handoff, `wf_validate_docs` / `wf_audit`, Prepare, Review, Close, installation, and upgrade. A changed workflow config, prompt-surface manifest, or repo profile must automatically escalate the incremental path to a full scan. `wf_garden_docs` gardens Git-changed markdown by default; agents must use an explicit path mode for new untracked docs when their timestamp needs refresh.
...
- **`docs-lint` hook** — run incremental **`wf docs-lint --changed`** after any Edit/Write to files under `docs/`, failing the hook when changed-file validation fails (subprocess hook path; not MCP). Full corpus validation remains the explicit integrity-boundary gate.
...
| Codex | ❌ instruction-only | ❌ | `AGENTS.md`, `docs/prompts/`, `.codex/skills/wf-guru/SKILL.md` |
...
- Only the **read-only tier** (retrieval, listing, status, search tools) renders by default. The mutating tier renders only when the operator sets the top-level `"wavefoundryAllowWriteTools": true` key in `.claude/settings.json` (an operator-authored knob the renderer only ever reads). It never lives in workflow-config or any rendered block.
- The mutating tier is **all or nothing**: there is no per-tool opt-in, so setting the knob also allowlists `wf_close_wave` and both edit gates (`wf_open_gate`, `wf_close_gate`). Wave closure and seed/framework edits then stop producing a host permission prompt, and their only remaining backstop is prompt policy (the operator-owned wave-close rule and the open-gate-then-close-immediately discipline in `AGENTS.md`). Leave the knob unset unless the team accepts that tradeoff. The tier covers, in full: every lifecycle mutation (`wf_create_wave`, `wf_add_change`, `wf_remove_change`, `wf_prepare_wave`, `wf_implement_wave`, `wf_pause_wave`, `wf_review_wave`, `wf_review_event`, `wf_reopen_wave`, `wf_close_wave`), all ten `wf_new_*` scaffolders, `wf_set_handoff`, both edit gates, the AC/task markers (`wf_mark_ac`, `wf_mark_task`), all `memory_*` mutations (`memory_add`, `memory_propose`, `memory_backfill`, `memory_reconcile`, `memory_validate`, `memory_consolidate`, `memory_purge`), `index_build` and `index_optimize`, both evals (`wf_context_efficiency_eval`, `wf_memory_eval`), both sensor entry points (`wf_run_sensors`, `wf_scan_secrets`), `wf_garden_docs`, `wf_sync_surfaces`, `wf_techdocs_baseline` (the Backstage/TechDocs baseline writer), `wf_upgrade`, `wf_reload_mcp`, and the four dashboard tools (`wf_start_dashboard`, `wf_stop_dashboard`, `wf_restart_dashboard`, `wf_open_dashboard`).
- The renderer records exactly which entries it emitted under the top-level `"wavefoundryManagedAllow"` provenance key and on later renders adds/removes **only** entries it recorded emitting. Ownership is never inferred from the `mcp__wavefoundry__` name prefix: operator-authored rules, including rules that happen to name a wavefoundry tool, survive every render, as do all deny/ask entries and unknown keys (preserved value-identically under the canonical `indent=2` re-serialization).
- Permissions rendering is **operator-approved, not structurally unreachable by an agent**, and the guidance must say so plainly. The renderer emits permissions only when its explicit include-permissions switch is passed, and the agent-invocable `wf_sync_surfaces` render never passes it (it never touches `permissions` content, pinned by test), so no agent can widen its own allow rules through the surface-sync path. Two paths that do render permissions are agent-reachable and are accepted with a stated boundary: (1) `wf_upgrade` is an ordinary agent-callable MCP tool whose first phase runs the upgrade orchestration and therefore renders. Its blast radius is bounded to the **read tier**: the write tier requires the operator knob, and `wf_upgrade` is itself a write-tier tool, so an agent-triggered upgrade can never allowlist the tool that triggered it and the tier cannot self-perpetuate. (2) The renderer's include-permissions switch can be passed through the `wf` dispatcher, which forwards argv verbatim; that is an accepted residual outside the threat model, because an agent holding unrestricted shell access can write `.claude/settings.json` directly and the switch grants it nothing new.
- The knob's protection is a **host guarantee, not a framework one**. `.claude/settings.json` is operator territory because the host prompts before an agent edits it; the framework's own pre-edit guard on that file is the `framework_edit_allowed` gate, which an agent can open through its own gate tool. Writing the knob therefore needs exactly the same capability as writing the rendered rules by hand, so the knob is not a weaker link than the file it lives in, but it is not out of an agent's structural reach either. Document it as operator-owned by host enforcement plus prompt policy, never as framework-enforced isolation.
- The upgrade output names the rendered permissions delta as an explicit operator consent line; read it rather than treating the render as a silent mechanical step.
- **How a rendered rule actually goes away.** The merge is declarative: every roster-desired rule the renderer does not find in `permissions.allow` is appended and claimed on the next render, so deleting a rule from the allow array (whether or not you also delete it from `wavefoundryManagedAllow`) is undone by the next upgrade or install render. Provenance governs only the PRUNING of rules that are no longer desired. The three real levers are: (1) add a host `deny` entry for the rule, which suppresses it permanently because deny wins over allow in Claude Code; (2) for a write-tier rule, clear `wavefoundryAllowWriteTools`, which makes the whole tier no longer desired so its claimed rules are pruned on the next render; (3) remove the rule from `wavefoundryManagedAllow` while LEAVING it in `permissions.allow`, which suppresses nothing but converts the rule to operator-owned so it survives a future roster retirement instead of being pruned. Never describe (3) as a way to drop a rule.
- **When the block first appears.** A fresh install (`wf setup`) and a protocol-bridge upgrade render the permissions block immediately. An ordinary upgrade of an existing target also renders it during that same upgrade, but only because of deliberate backstops in the upgrade's later phases: the in-process upgrade orchestrator is pre-extraction code, so on the transition upgrade the surface-rendering phase runs the OLD renderer, which has no permissions switch. The backstops run only from phases the upgrade executes in a SEPARATE process on the freshly extracted code, which is what closes that old-code window. The cleanup phase carries the site every upgrade path reaches; the index-refresh phase carries a second one for the flows that run it. Without them the block would first appear one full upgrade cycle later. If a target shows no permissions block after upgrading, let the next upgrade or a `wf setup` render it rather than hand-writing the rules.
- **Repos that already hand-maintain these rules.** The merge never claims a desired rule that is already present but unclaimed; that is exactly what lets an operator's wavefoundry-named rule survive a later roster retirement. The consequence for a repo whose committed `.claude/settings.json` already carries the read-tier rules: the render claims nothing, `wavefoundryManagedAllow` stays empty, those rules get NO rename self-heal, and the consent output reports them as already present and left unmanaged so the state is visible rather than silent. To hand them over to the renderer, delete the wavefoundry allow rules from `permissions.allow` and let the next upgrade or `wf setup` re-render them; the renderer then emits and claims them, and self-heal applies from that point on. Until that hand-over they stay operator-owned rules the operator must update by hand on every tool rename.

`.gitignore` tracks `.claude/skills/` and `.claude/hooks/` (for reusable operator skills and generated hook entrypoints). `.claude/settings.json` is a committed project-level file and must not be gitignored; `.claude/settings.local.json` is a personal override and should be gitignored (it stays fully operator-owned; the permissions renderer never touches it).
...
- `seed-211` — guru (codebase/documentation investigation)
- `seed-212` — performance-reviewer
- `seed-213` — security-reviewer
...
- `seed-217` — senior-engineering-challenger
...
- `seed-237` — council-review public prompt
- `seed-239` — qa-reviewer
...
- the gating rules: skills render on `wf setup` and **Upgrade Wavefoundry**; a `requires_doc`-gated skill renders only where its backing doc exists (`wf-guru` on `docs/agents/guru.md`; `wf-package`, `wf-code-cleanup`, and `wf-techdocs` on their prompt docs, `wf-techdocs` therefore in every target once `docs/prompts/refresh-techdocs.prompt.md` has been rendered); skill rendering is independent of `enabled_agent_roles`, which gates agent-role wrappers, not skills; every skill body is a thin pointer to the backing `docs/prompts/*.prompt.md`, so the phrase and the skill run the identical workflow.
...
- **`wave-coordinator.md`** — During scope assessment and readiness review, use `code_ask` and `code_search(topic, kind="code-summary", max_per_file=1)` to answer "what does X currently do?" and "which files are affected?" without launching full file reads. When the relevant symbol is already known, switch to `code_definition(symbol)` or `code_references(symbol)` before broad file reads. If you are validating call-site signal, remember that `exclude_tests=true` is available for quieter reads without losing the broad mode. `code_dependencies(path)` is the fastest path to understanding what a changed file touches. When MCP is available, shell search and broad file reads are fallback tools only — use them when MCP is not attached, results are unavailable, or genuinely insufficient. The MCP-first code navigation obligation applies during coordination as well as implementation; do not default to shell exploration for questions MCP tools are designed to answer. When you spawn a subagent for code investigation or implementation, route it through a role-typed agent (`implementer`, `guru`, a builder specialist) **or** carry the MCP-first code-navigation directive in the subagent's prompt — check the receiving worker's actual tools and allowlist rather than assuming inherited MCP access; follow seed 180's host-neutral orchestration fallback when unavailable and MCP-first when available, so a bare general-purpose subagent grepping by habit is the same shell-by-habit defect; require and surface the same `Gapfill:` note from spawned subagents.

- **Reviewer role docs** (`code-reviewer.md`, `qa-reviewer.md`, `architecture-reviewer.md`, `security-reviewer.md`, `performance-reviewer.md`, `release-reviewer.md`, `docs-contract-reviewer.md`, and factor-review docs) — front-load a short **Tool posture** lead near the top of each doc (before Responsibilities): when the Wavefoundry MCP is attached, prefer its retrieval tools over shell search — `code_ask` to open an investigation when the location is unknown, `code_references`/`code_callhierarchy` to back any how-many/blast-radius claim (never a sampled grep), `code_keyword`/`code_search` for sweeps, `code_read` for targeted ranges — loading deferred schemas once via the host's tool loader; full posture in the run contract's Retrieval Rules (seed-020) carried by `docs/contributing/agent-team-workflow.md`. Keep it to a few lines and point rather than restate. This lead exists because reviewer lanes observed in the field default to grep when the posture is only stated on main-thread surfaces they never read.
...
> - **Token exports (wave `12atj`):** reference the generated `exports/` — `var(--ds-<token>)` (CSS), `exports/ts/tokens.ts` (TypeScript), `exports/tailwind/theme.config.js` (Tailwind) — never hard-code values. Run `docs/design-system/bin/build-tokens` after editing any `tokens/*.json` source; `exports/` is generated and must not be hand-edited.
...
> - **Adopt-in-place / reference mode (wave `1p799`):** when `manifest.json` `sourceStrategy` is `external-reference`, this tree is a **thin reference index**, not an extracted mirror. The real design system lives elsewhere (see `manifest.json.externalReference.tokenSource`); there is no `tokens/`/`exports/` mirror and no `--ds-*` namespace. Consume the project's *own* tokens using the convention in `externalReference` (`varPrefix`, `namingConvention`) and read usage from its `consumptionDoc`. `docs/design-system/AGENTS.md` is retargeted to that source. The "never hard-code; consume tokens" rule still holds — against *their* system.

...
- **If a wave was closed by mistake**, use `wf_reopen_wave(wave_id)` to restore it to `active` status. `wf_reopen_wave` only works on waves with `Status: closed` and removes the `Completed At` stamp. After reopening, verify with `wf_review_wave` before attempting closure again.

...
- Record operator approval by writing `operator-signoff: approved` in the `## Review Evidence` section of `wave.md` before or alongside the `wf_close_wave` call. `wf_review_wave` enforces this as a required lane and `wf_close_wave` blocks if it is absent.

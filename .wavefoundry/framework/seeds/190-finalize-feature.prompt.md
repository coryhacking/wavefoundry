...
8. When `wave_review.enabled` is true, confirm both universal council signoffs are recorded before closure (on a wave declaring `review-evidence-source: events.jsonl` these are typed approval events via `wf_review_event`, projected into `## Review Evidence`; prose signoff lines count only on legacy waves):

...
`universe_closed` requires an instrument capable of finding the sites you claim are absent. A text or identifier search enumerates **name** occurrences, not **value** flows: a site that consumes the value under a different local name matches no query naming the original symbol, so a census of consumers built from identifier search can report closed in good faith and be wrong. For “every site that reads/compares this value”, close the universe with a reference-level instrument (`code_references` / `code_impact` on the symbol) and record it in `tools_and_queries`; identifier search is the complement that catches non-code mentions, not a substitute. A passing suite is not evidence of closure when the instrument could not have found the miss.

...
**Lane-clearing recipe (state-derived).** When a finding's current head carries several `blocking_required_lanes`, clear them with the typed tool one lane at a time: (1) call `wf_review_event(event="list")` for the finding and read the `chain_summary` unresolved required lanes; (2) if the repair cycle is not open yet, the **implementer** records `event="finding"`, `run_kind="repair_start"`, `cycle>=1`, same `finding_id` **before mutating anything**; a readiness-born finding may do this directly from its cycle-0 readiness synthesis, while a delivery-born finding first requires `initial_delivery`; (3) the **blocking reviewer lane** then submits ONE reverification per lane where the acting lane is `actor`, `fresh_context=true` and `independent=true` are set, and `blocking_required_lanes` equals the CURRENT list minus that one actor; (4) repeat from the list step until the head's list is empty, then record the affected readiness or delivery approval after every blocking current head for that phase is terminal. Both `repair_start` and `reverification` are `event="finding"` calls carrying `run_kind` — **not** `event="run"`, which records only an empty readiness/initial_delivery run. The actor that performed the repair must not be the actor that reverifies it: independence is exactly what a cleared lane asserts. Lanes clear in any order, but always re-list first: another reviewer may have acted since your last read. A reverification that repeats the current list unchanged verifies without clearing anything. A separately recorded protocol-valid operator waiver is another terminal state; it is not a lane-reverification shortcut. Never carry a repaired readiness finding into delivery merely to obtain a legal predecessor record.

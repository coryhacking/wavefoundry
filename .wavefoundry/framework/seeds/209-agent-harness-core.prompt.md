...
**Lane-clearing recipe (state-derived).** When a finding's current head carries several `blocking_required_lanes`, clear them with the typed tool one lane at a time: (1) call `wf_review_evidence(event="list")` for the finding and read the `chain_summary` unresolved required lanes; (2) submit ONE reverification per lane where the acting lane is `actor`, `fresh_context=true` and `independent=true` are set, and `blocking_required_lanes` equals the CURRENT list minus that one actor; (3) repeat from the list step until the head's list is empty (the server auto-mints the linked `lane_reassessment` evidence for each cleared lane). Lanes clear in any order, but always re-list first: another reviewer may have acted since your last read. A reverification that repeats the current list unchanged verifies without clearing anything. A separately recorded protocol-valid operator waiver is another terminal state; it is not a lane-reverification shortcut.

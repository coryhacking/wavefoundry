...
The tenet reuses this protocol's `execution_status` vocabulary (`executed`, `inferred`, `unverified`, `not_applicable`) in every phase; no phase invents parallel terms. Three claim shapes carry the highest risk and need the strongest evidence: an "X already does Y" mechanism claim, a "no other caller/site" census, and a rationale that justifies a decision by a benefit (the benefit must be observed, not assumed). Per phase:

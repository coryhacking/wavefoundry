...
- Treat `wf_close_wave(mode="create"|"apply")` as a distinct operator-finalization step. Do not infer close approval from adjacent tasks such as "remove the dead code", "run the tests", "fix the review finding", or "implement wave". Those phrases authorize the named task only. Require the operator to say something like "close the wave" or explicitly confirm when asked. `wf_close_wave(mode="dry_run")` is always safe and does not require operator approval.
...
- Only one wave may be **OPEN** (`active`/`implementing`) per `change-id` at a time — the hard single-OPEN rule, enforced at activation (`wf_implement_wave` / `wf_reopen_wave` / `wf_prepare_wave(mode='create')`). Any number of waves may be planned and **readied** (`wf_prepare_wave(mode='ready')`, `Status: planned`) in parallel; readiness never takes the slot.

...
- **`wave-id`**: uniquely identifies a wave using the format `<prefix> <slug>`:
  - `<prefix>` — 5-character lowercase base36 lifecycle token (6 characters after the distant overflow horizon), time-ordered under the provisioned lifecycle policy in `docs/workflow-config.json` (`lifecycle_id_policy`), generated by the MCP `wave_create_wave`/`wave_new_*` tools or `wf lifecycle-id --kind wave --slug <slug>`

...
- **`wave-id`**: uniquely identifies a wave using the format `<prefix> <slug>`:
  - `<prefix>` — 5-character Crockford Base32 timestamp encoding hours since the workflow-configured lifecycle epoch in `docs/workflow-config.json` (`lifecycle_id_policy`) plus a minute bucket, generated by `wf lifecycle-id --kind wave --slug <slug>`

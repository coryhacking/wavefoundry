...
- ensure `docs/workflow-config.json` is seeded with explicit readiness-review behavior in `wave_execution` and persona gating behavior in `persona_review_policy` so later upgrades can regenerate consistently

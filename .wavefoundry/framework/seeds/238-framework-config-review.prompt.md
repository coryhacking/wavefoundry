...
- **On wrappers-without-sources or frontmatter-less wrappers, direct regenerate — not relocate/retire.** The fix is to **regenerate the canonical+wrapper pair via `seed-050` task 5** (or an `Upgrade Wavefoundry` reconciliation — see `seed-160`). The docs-lint factor-surface gate (`check_factor_surface`) already flags these states as ERRORs; this review should echo the gate's recovery, not contradict it.

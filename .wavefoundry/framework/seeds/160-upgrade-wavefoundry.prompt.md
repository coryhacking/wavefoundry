...
8. Regenerate or normalize:
 ...
 - **`.gitignore` runtime file contract** — verify the `.gitignore` contains the Wavefoundry runtime block from `seed-050` (dashboard-server.json, `*.lock`, `logs/`, `index/` paths). Add it when missing. Then check whether any of those runtime files are still tracked: `git ls-files .wavefoundry/dashboard-server.json .wavefoundry/*.lock .wavefoundry/framework/*.lock`. For each tracked file, run `git rm --cached <file>` to remove it from the index without deleting it from disk — this prevents the live pid/port from churning into git history on every dashboard restart.

...
On native Windows, a newly checked-out already-seeded repository needs the same pre-MCP local Python diagnosis as a first install. Before `wf.cmd`, setup, or MCP can be trusted on that workstation, run `powershell -NoProfile -File ".\.wavefoundry\framework\scripts\diagnose_python.ps1"` from the repository root. The read-only diagnostic works without Python or MCP and does not depend on completed install-log rows. If policy blocks it, use the manual probes and IT handoff in `.wavefoundry/framework/seeds/011-install-wavefoundry-phase-1.prompt.md` **Python prerequisite**; do not bypass execution policy. Preserve committed configs and index data while resolving the local prerequisite. Only a working fresh-host `python3` 3.11 or newer, followed by `python3 .wavefoundry/framework/scripts/server.py --dry-run` and host restart/reconnect, establishes MCP readiness. A working `python.exe` alone never changes the canonical host command. On first installation, a successful supported `python3` check permits setup to provision the tool environment before the server dry-run; do not require MCP bootstrap before its dependencies exist. Existing provisioned checkouts need no forced setup or rebuild merely for command repair.
...
6. Validate the installed repo-local Wave Framework surface and detect drift across:
 - public prompt docs (including `implement-feature`, `implement-wave`, `plan-feature`, `review-plan`, and `index` vs current `seed-100` guard requirements)
 - `docs/prompts/upgrade-wavefoundry.prompt.md` vs seed-160 storage recovery guidance on every upgrade, including unchanged-seed/same-version retries (merge contract in step 8)
...
8. Regenerate or normalize:
 ...
 - **Profile key preservation.** Accept either `code_pattern` or `code_patterns` in an existing operator-authored profile; never rename the key during upgrade. New profiles follow seed 030.
 - **Implement template MCP guidance.** Reconcile the implement prompt's host-conditional MCP block for existing targets: pre-edit memory briefing, full docs validation at completion, and role-typed or MCP-first-briefed code delegation; hosts without MCP use the repository fallback. The packaged template itself is missing-only and reaches new prompts, not existing ones.
 ...
 - repo-local prompt docs
   ...
   - `docs/agents/specialists/wave-council.md` — Wave Council role doc (`seed-215`); regenerate when present and stale. The council protocol evolves across versions: Phase 2 seat instructions may be restructured (numbered steps, pre-primer independence discipline), synthesis may gain new required steps (Recommendations Verdict table with red-team closing reconciliation, pre-primer read quality check, output verbosity guidance). Regenerate whenever `seed-215` changed.
   - `docs/agents/specialists/archetype-council.md` — Archetype Council role doc (`seed-236`); regenerate when present and stale. Same protocol evolution applies: axis-declaration sequencing, null-finding requirements, Recommendations Verdict table, falsification check, output verbosity. Regenerate whenever `seed-236` changed.
 - `docs/prompts/agents/` prompt bodies when the repository keeps checked-in project-context/planning helper prompts; these are optional non-public helpers (per that directory's README), so reconcile each body only **when present** — absence is a valid state, never a backfill obligation:
 - `docs/prompts/agents/performance-reviewer.prompt.md` (`performance-reviewer` lane — `seed-212`)
   ...
   - `docs/prompts/agents/code-reviewer.prompt.md` (`code-review` lane — `seed-221`)
 ...
 - `docs/plans/plan-template.md` `## AC Priority` — when the section still instructs a Prepare-time fill (placeholder text such as `(Populated at Prepare wave.)`), replace that instruction with a plan-time fill direction: one row per AC, populated with the ACs before the prepare council runs. The table is requirement-bearing and stays in the review-policy digest, so filling it after readiness is recorded supersedes the receipt and lapses the approvals just collected. Keep the placeholder priority row and the `ac_priority_unpopulated` Prepare advisory as the backstop; this migration changes the instruction, not the gate.
 - `.wavefoundry/framework/install/plan-template.md` is the shipped default used by `server_impl._default_template()` and the surface renderer. `wf render-surfaces` materializes `docs/plans/plan-template.md` only when the project copy is absent, stamping `Last verified:` on write; never restore or edit an inline `_default_template()` string, and never overwrite an existing operator template.
 ...
 - **`docs/scan-rules.toml` secrets threshold** — **verify/complete** the project-level scan-rules policy. Note (wave 1p44z): the automated upgrade flow now **materializes this file before the first docs gate** (`upgrade_wavefoundry.py` Phase 2b, `materialize_secrets_policy`) when it is ABSENT, so the common "file missing" case is already handled by the upgrade — this editing-pass step is now an audit that completes the rarer "file exists but lacks the policy key" case and confirms the value. Do not double-write a value the preflight already set. See `docs/references/scan-findings-format.md` for the findings schema, status lifecycle, and the `[policy]` confirmation-threshold contract.
   ...
   3. If `docs/scan-rules.toml` does not exist (preflight materialization was skipped or failed): create it with a `[policy]` section containing `false_positive_confirmations_required = N` and the standard header comment (see `seed-012` step 2.4a for the exact template).
 ...
 - other changed upgrade clauses in `docs/prompts/upgrade-wavefoundry.prompt.md` when the seed pack’s upgrade contract changes; storage recovery guidance is checked on every upgrade per the explicit step-8 contract above
 ...
 **Tool-list changes propagate via `notifications/tools/list_changed`** (wave 131bt — 131bu). When `wf_reload_mcp()` detects that a tool was added or removed, or that any tool's docstring changed between the pre-reload and post-reload snapshots, it dispatches the MCP `notifications/tools/list_changed` protocol notification to the connected client. The response reports server-side dispatch separately from client adoption; it cannot prove that the client refreshed its catalog. The response carries:
 ...
 - `tool_list_changed_notification_dispatch` — for the direct `wf_reload_mcp()` response, exactly `not_needed`, `completed`, or `failed`; `completed` means the server-side send coroutine finished before the tool returned, but no value proves client adoption
 - For the synchronous `wf_upgrade` response's nested `data.mcp_reload`, the separate dispatch domain is exactly `not_needed`, `scheduled`, or `failed`; `scheduled` means handed to the active event loop, not completed
 - A structured diagnostic — the retained direct-success `tool_list_changed_notification_sent`, the upgrade-path `tool_list_changed_notification_scheduled`, or `tool_list_changed_notification_failed` — with actionable next steps. The old boolean response field `tool_list_changed_notification_sent`, the `queued` dispatch value, and the `tool_list_changed_notification_queued` diagnostic are retired.

 Internal helper states `deferred` and `no_running_loop`, plus the private `tool_list_changed_notification_required` handoff key, do not serialize through either public caller.
 ...
 **Exception:** if the wave changed `server.py` or `venv_bootstrap.py` (the un-reloadable runner files rather than the `wf_server/` implementation modules or any tool definitions), a full session restart is still required because the runner cannot reload itself in-process. You do not need to guess whether this applies: after the upgrade, call `wf_server_info()` and read `runner_stale`. `false` confirms the captured runner identity matches disk; `true` means the on-disk runner files no longer match the ones the running process launched from, and only a full host restart (quit and relaunch) loads them. `null` means that comparison is unavailable (for example, an older runner did not capture a comparable identity): report it as **unknown**, not as evidence that a restart is unnecessary. An in-process implementation reload can still succeed in that case, but only a full host restart guarantees runner-file changes are loaded. After reload or restart, check whether the dashboard is running and restart it if so:
 ```
...
- migrate the optional plan-review identity to **Review plan** / `wf-review-plan` while preserving **Interrogate this plan** and **Stress-test this plan** as natural-language aliases only; apply seed 175's exact project-owned prompt migration contract before skill rendering or missing-only lifecycle baseline materialization, fail closed on customized-old or dual-path conflicts, and keep **Review wave** / `wf-review-wave` unchanged
...
- run `wf render-surfaces` so a missing `docs/plans/plan-template.md` is materialized from `.wavefoundry/framework/install/plan-template.md`; when an existing operator template still uses the old `## Spec Refs` shape, update it merge-safely to the consolidated format without overwriting unrelated refinements
...
- update `docs/plans/plan-template.md` `## AC Priority` to direct a plan-time fill when it still instructs a Prepare-time fill (placeholder text such as `(Populated at Prepare wave.)`); the table is requirement-bearing and stays in the review-policy digest, so a Prepare-time fill supersedes the receipt and lapses the readiness approvals just collected
...
- reconcile operating-memory expectations during upgrade: durable implementation/review lessons are captured as typed memory candidates while fresh, and closure validates every candidate — journals are retired and never a closure requirement; move existing migrated role-journal blocks verbatim to a role-attributed history document, archived rather than deleted
...
| `.codex/skills/wf-guru/SKILL.md` | 3 |
...
- `.wavefoundry/framework/install/plan-template.md` is the shipped scaffold authority, `wf render-surfaces` materializes `docs/plans/plan-template.md` only when absent with `Last verified:` stamped, and an existing project template remains byte-identical unless an explicit merge-safe repair is required
...
- `docs/plans/plan-template.md` `## AC Priority` directs a plan-time fill (one row per AC, populated before the prepare council runs) and no longer carries a Prepare-time-fill placeholder such as `(Populated at Prepare wave.)`; the `ac_priority_unpopulated` Prepare advisory is still present as the backstop
- `docs/plans/plan-template.md` has a `## Serialization Points` section, and that section states that Prepare derives automatic review lanes from its explicit repo-relative paths only, names both declaration forms (a bullet that is entirely paths, or a `**Review targets (repo-relative paths):**` block), states the token grammar (a declared path token has at least one `/`, so a root-level file is never a token in either form, and a bullet declares all or nothing in either form; in a bullet a `*` disqualifies the token; inside the explicit block a span is kept only when its last segment carries an extension or the span ends in `/`, so there a `*` span is accepted as a phantom that recruits a lane only through a trigger token it happens to carry (a directory prefix, an extension, or a trigger basename), and any other `*` span makes its bullet prose; put a root-level file in its own prose bullet, and declare the directory that holds globbed files), states that an undeclared change doc keeps whole-document fallback scoring, and states that any lane may additionally be requested by judgment in `Requested review lanes`. Verify separately that the template itself declares **zero** targets: any example it carries must be fenced, or every change doc scaffolded from it is born in declared mode
- `docs/prompts/prepare-wave.prompt.md` tells the coordinator that automatic lane derivation is a floor and that they may add lanes by judgment, rather than only "select lanes from repository evidence", and states the token grammar (a declared path token has at least one `/`, so a root-level file is never a token in either form, and a bullet declares all or nothing in either form; in a bullet a `*` disqualifies the token; inside the explicit block a span is kept only when its last segment carries an extension or the span ends in `/`, so there a `*` span is accepted as a phantom that recruits a lane only through a trigger token it happens to carry (a directory prefix, an extension, or a trigger basename), and any other `*` span makes its bullet prose; put a root-level file in its own prose bullet, and declare the directory that holds globbed files)
...
- `docs/agents/specialists/wave-council.md` is regenerated from current `seed-215` when present — verify it reflects the current protocol (Phase 2 numbered steps, pre-primer independence discipline, Recommendations Verdict table with red-team closing reconciliation, pre-primer read quality check, output verbosity guidance); regenerate when stale
- `docs/agents/specialists/archetype-council.md` is regenerated from current `seed-236` when present — verify it reflects the current protocol (axis-declaration Step 1 before reading artifact, null-finding requirements, Recommendations Verdict table, falsification check, output verbosity); regenerate when stale
...
- when `docs/agents/guru.md` exists: `.codex/skills/wf-guru/SKILL.md` present (registry-rendered; a leftover pre-namespace `.codex/skills/auto-guru/` is stale-cleaned by the render pass); `.cursor/rules/auto-guru.mdc` when `.cursor/` exists; `.claude/agents/guru.md` when `.claude/` exists; tier-2 `wave:auto-guru` marker blocks on `CLAUDE.md` and each enabled thin pointer (`project-context.mdc`, `.junie/guidelines.md`, `WARP.md`, `.github/copilot-instructions.md`) with no duplicate unmarked bullets outside markers
- `docs/agents/platform-mapping.md` documents auto-Guru tier 1–3 routing when present in the repo
- when a skill host root exists (`.claude/`, `.codex/`, or `.agents/`): `docs/agents/platform-mapping.md` carries the Skills subsection seed-050 specifies (host skill directories, the rendered set, the gating rules), and its listed set matches the skill directories on disk after this upgrade's render pass; add or refresh it from disk when missing or stale, never from the seed's example list (seed-100 points `docs/prompts/index.md` at that section)
...
- `.claude/hooks/pre-edit.py`, `.claude/hooks/post-edit.py`, `.claude/hooks/session-capture.py`, `.claude/hooks/context-efficiency-project.py`, `.claude/hooks/wf-session-start.py`, and `.claude/hooks/simulate-hooks.py` exist as the single cross-platform hook bodies, and `.claude/settings.json` carries their `Stop` and `SessionStart` (`startup|resume`) entries; add if missing per `seed-050`. Any leftover extensionless, `.sh`, `.cmd`, or `.claude/hooks/pycache-cleanup*` launchers from older installs may be deleted.
- `.claude/skills/wf-upgrade/SKILL.md` exists (standard SKILL.md with `name`/`description` frontmatter, rendered by the skill registry via `wf render-surfaces`) with the verified upgrade gate sequence, including the repo-global `.wavefoundry/guard-overrides.json` approval flow for `framework_edit_allowed` and `seed_edit_allowed`; a leftover flat `.claude/skills/upgrade-wave.md` from an older install is stale-cleaned by the render pass
...
- reviewer role docs (`docs/agents/code-reviewer.md`, `docs/agents/qa-reviewer.md`, `docs/agents/specialists/wave-council.md`) contain a **Review Rubric** or equivalent section referencing the preflight checks from `seed-020` **Prompt Preflight**
...
- `docs/prompts/index.md`, `docs/prompts/prompt-surface-manifest.json`, and `AGENTS.md` shortcut tables remain **triplet-consistent** (no orphan public prompts, no duplicate shortcuts); they identify **Review plan** / `wf-review-plan` as the optional no-signoff plan stress test and **Review wave** / `wf-review-wave` as the required delivery-review path
- `docs/workflow-config.json` retains the docs-gate-required top-level **`wave_implement`**, **`wave_review`**, **`agent_memory`**, **`project_persona_generation`**, **`prompt_generation`**, **`factor_review_policy`**, and **`persona_review_policy`** sections; retain **`design_system_policy`** when applicable and backfill it with `{"governance": "evolvable"}` when absent
...
- `docs/missing-docs.md` (or `docs/gaps/missing-docs.md`) is current — new undocumented code areas are listed, resolved items are removed or moved to the `## Resolved / closed` table per `230-author-spec` §6 (in the canonical `docs/missing-docs.md`, that exact heading is what keeps the reconciliation scan from reporting a resolution note that names a retired surface; the legacy `docs/gaps/` path gets no such exemption, consolidate it per `seed-220`)
...
the shared index database prove convergence. The upgrade lock is audit/recovery
...
| Seed 050 Claude wrapper defaults | **Preferred upgrade cleanup:** inventory `model:` and `effort:` pins in verified Wavefoundry-managed Claude wrappers, including `.claude/agents/guru.md` and framework-generated factor wrappers; filenames alone do not establish ownership. In the framework templates immediately before 1.26.0, the only shipped wrapper pin was `model: sonnet` in Guru; factor wrappers had no model pin and no wrapper had an `effort:` default. This identifies the former template, not current operator intent; the protections below still apply. Remove inherited framework defaults when prior templates/history or scoped operator confirmation establishes their provenance, so host/task-level selection can apply. Preserve deliberate operator choices even when their values match an old template; current operator intent takes precedence over historical origin. Never infer ownership from a model name. For unknown, incomplete or conflicting provenance, retain the pin, recommend removing the specific `model:` or `effort:` line, and ask the operator before editing. Limit this cleanup to the selected fields; preserve other frontmatter, `tools:`, wrapper bodies and project-owned custom agents. Rerender and verify cleaned headers stay unpinned and intentional pins survive. Report removed, retained and unresolved pins in the upgrade summary. Fresh wrappers continue to omit model/effort defaults; this is an authored editing-pass cleanup, not an automatic renderer migration. The renderer also preserves `tools:` verbatim: compare each wrapper against seed 050 and add missing read-only retrieval entries when reconciling stale framework defaults. Preserve deliberate operator restrictions; surface any unresolved mismatch between the tools granted and the refreshed body instead of silently broadening permissions. |

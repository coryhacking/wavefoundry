...
8. Regenerate or normalize:
 ...
 - **`code_navigation_hints`** in `docs/workflow-config.json` — backfill with `{"guard_tokens": ["return", "throw", "raise", "guard", "assert"]}` when missing (wave 1p2q3 / 1p2th); **never overwrite** an existing `guard_tokens` array — operators tune it to local codebase conventions and an upgrade-time clobber would erase that tuning

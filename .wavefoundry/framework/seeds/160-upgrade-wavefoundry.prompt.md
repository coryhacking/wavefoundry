...
8. Regenerate or normalize:
 ...
 - **`docs/scan-rules.toml` secrets threshold** — backfill the project-level scan-rules policy when absent or incomplete:
   ...
   3. If `docs/scan-rules.toml` does not exist: create it with a `[policy]` section containing `false_positive_confirmations_required = N` and the standard header comment (see `seed-012` step 2.3a for the exact template).
   ...
   5. **Never overwrite** an existing `false_positive_confirmations_required` value — it may have been set by the operator to match their actual team policy.

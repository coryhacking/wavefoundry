...
0. **Adopting a distribution zip (automatic when present):** When one or more date-shaped `wavefoundry-YYYY-MM-DDx.zip` files exist at the **repository root** (not in a subdirectory), **Upgrade wave framework** must apply the newest pack **before** step 1 — the operator does not need a separate unzip step. Other archive names and zips outside the root **do not** trigger this step — unpack those manually if used. When **no** matching zip exists at the root, skip this entire step with no error and continue at step 1.
   ...
   - **Retire pack-removed paths** after unpack so orphans from prior packs do not shadow or duplicate current-pack code. `unzip -o` overlays files but never removes paths that vanished from the pack, so rename waves and deletions leave stale copies behind. **General rule:** when a path previously shipped under `.wavefoundry/framework/scripts/` (including `scripts/tests/`) is no longer present in the new pack, delete the stale copy from the target repository. **For packs at `2026-04-22a` and later in this repository** (upstream framework plus renamed script layout), delete these legacy paths when they exist on disk after unpack:
     - `.wavefoundry/framework/scripts/render_hooks.py` — replaced by `render_platform_surfaces.py`
     - `.wavefoundry/framework/scripts/tests/test_render_hooks.py` — replaced by `tests/test_render_platform_surfaces.py`
     - `.wavefoundry/framework/scripts/build_zip.py` — replaced by `build_pack.py`
     - `.wavefoundry/framework/scripts/tests/test_build_zip.py` — replaced by `tests/test_build_pack.py`
     - `.wavefoundry/framework/scripts/docs-lint.py` — replaced by `docs_lint.py`
     - `.wavefoundry/framework/scripts/docs-gardener.py` — replaced by `docs_gardener.py`
     - `.wavefoundry/framework/scripts/docs_lint_lib/` (entire directory) — replaced by `wave_lint_lib/`
     - `.wavefoundry/framework/scripts/tests/` (entire directory) — test suite is development-only; removed from pack as of `2026-05-01a`
     - `.wavefoundry/framework/scripts/run_tests.py` — test runner is development-only; removed from pack as of `2026-05-01a`

     Leaving these orphans risks ambiguous Python imports, duplicate test discovery, and stale references from repo-local wrappers or hook configs. The listed paths are safe to delete unconditionally after a successful unpack of a pack stamped `2026-04-22a` or later (first pack in this repository lineage that ships `render_platform_surfaces.py`, `build_pack.py`, `docs_lint.py`, `docs_gardener.py`, and `wave_lint_lib/`).
   - **Continue automatically in the same run:** step 0 only adopts the pack. After unpack + hook regeneration + orphan retirement, immediately continue with step 1 and complete the full upgrade workflow (`020`, `150`, drift detection, backfill, verification). Do **not** stop after unpacking or treat unzip success alone as a completed upgrade.
...
8. Regenerate or normalize:
   ...
   - **`docs/design/` extraction contract backfill (merge-safe upgrade)** — when `docs/design/` exists in the target repository, run the extraction contract backfill defined in `seed-040` task 14 and `seed-010` step 8. This backfill is **merge-safe**: never delete or overwrite existing files. Steps:
     ...
     2. **Schema-version reconciliation** — read the existing `docs/design/manifest.json`. Compare its `schemaVersion` field against the current framework's expected schema version (`"1.0.0"`). Use semver ordering: a manifest `schemaVersion` is stale when it is lexicographically earlier using semver rules (e.g. `"0.9.0"` < `"1.0.0"`). When the manifest's schema version is stale or when required top-level fields are absent, **add** the missing fields with their null/empty defaults (never remove or overwrite existing field values), then append a `meta`-category entry to `docs/design/gaps.md`:
     ...
     3. **Coexistence rule** — extraction must never rewrite `docs/design/design-language.md` or `docs/design/index.md` body content. Extraction may only: (a) append a cross-link row to `index.md` listing new extraction artifacts with status `generated`, when that row does not already exist; (b) add a "See extracted contract" pointer at the top of `design-language.md` when that pointer does not already exist. Both operations are idempotent.
     4. **Rollback path** (document in gap log, not automatic) — if the operator needs a clean re-extraction: (a) move the existing `docs/design/<subtree>` to a timestamped backup at `docs/design/.backup/<ISO-date>/`; (b) regenerate using the seed contract; (c) diff against the backup for review. Record a `meta`-category `gaps.md` entry when a backup is created. Never auto-delete operator artifacts without the backup step.

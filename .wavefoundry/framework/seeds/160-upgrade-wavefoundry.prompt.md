...
11. Re-run the docs gate (**MCP:** **`wave_garden`** then **`wave_validate`** when attached; **CLI:** **`.wavefoundry/bin/docs-gardener && .wavefoundry/bin/docs-lint`**). **If the docs gate fails on a hardcoded-secrets finding (not an ordinary lint error), see "Docs gate failed on a secrets finding — resolve first" immediately below before retrying.**

#### Docs gate failed on a secrets finding — resolve first

The secrets scan runs as the **first** lint check in the docs gate, so a non-zero secrets result **aborts the upgrade** before the rest of the gate runs. When the gate fails on a `docs/scan-findings.json` finding:

1. **Do NOT** edit the finding away, delete the entry, or hand-edit `docs/scan-findings.json`. Run the **security reviewer Pre-Scope resolution loop** in `213-security-reviewer.prompt.md` (its Pre-Scope Step) as the authoritative procedure to classify each `pending` finding.
2. Each entry moves from `pending` → `false-positive` (operator-confirmed safe — clears once it has the `false_positive_confirmations_required` confirmations or a non-empty `override_reason`), `suspected-secret` (needs analysis — reclassify to `false-positive` or `confirmed-secret`), or `confirmed-secret` (a real secret — remediate/rotate, then re-scan). See `docs/references/scan-findings-format.md` for the full schema, lifecycle, and `[policy]` contract.
3. **Re-run the docs gate after EACH resolution** (`wave_garden` → `wave_validate`, or `bin/docs-gardener && bin/docs-lint`). Because the secrets scan is the first check, the gate keeps aborting until every blocking finding is resolved; re-running surfaces the next finding (or a clean pass).
4. **Resume, don't restart, on the automated path.** When the failure happened inside the `upgrade_wavefoundry.py` / `wave_upgrade` orchestrator, it retains a recoverable lock stamped `failed_phase=docs_gate` rather than a half-migrated stuck state. After resolving the findings, resume **without a destructive re-extract**: `.wavefoundry/bin/upgrade-wavefoundry --resume-after-gate` (or `wave_upgrade(phase="resume_after_gate")`). The resume re-runs only the docs gate and is idempotent on an already-advanced tree — do not re-run the full upgrade.
...
14. **Operating-memory upgrade reconciliation:** When `seed-006`, `seed-050`, `seed-120`, `seed-130`, `seed-140`, `seed-160`, `seed-170`, `seed-180`, `seed-190`, `seed-200`, or `seed-210` changed the journal/role/persona memory contract, upgrade existing projects using this checklist: When `seed-006`, `seed-050`, `seed-120`, `seed-130`, `seed-140`, `seed-160`, `seed-170`, `seed-180`, `seed-190`, `seed-200`, or `seed-210` changed the journal/role/persona memory contract, upgrade existing projects using this checklist:
 ...
 - Run the docs gate after reconciliation (**MCP:** **`wave_garden`** then **`wave_validate`** when attached; otherwise **`.wavefoundry/bin/docs-gardener && .wavefoundry/bin/docs-lint`**). If it fails on a hardcoded-secrets finding, resolve it via *"Docs gate failed on a secrets finding — resolve first"* (under step 11) before retrying.

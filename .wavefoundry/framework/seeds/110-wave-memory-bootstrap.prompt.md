...
- Generate both forms with `wf lifecycle-id --kind <kind> --slug <slug>`; the script uses 4 Crockford Base32 digits for hours since the configured lifecycle epoch in `docs/workflow-config.json` (`lifecycle_id_policy`) plus one Crockford minute-bucket character; all output is lowercase.

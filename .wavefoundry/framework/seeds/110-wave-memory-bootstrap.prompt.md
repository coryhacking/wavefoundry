...
| watchpoints and evidence        | `Watchpoints` (legacy heading `Journal Watchpoints` on old waves), optional `Salience / Impact` where decision-relevant |
...
- writes eligible tool events through to a host-local SQLite authority, using phase-scoped source/version uniqueness and one closed ledger: content + structural + completed-workflow + derived-artifact credit minus request and response debits; saved output or avoided tool loops enter only through quality-equivalent paired evidence;
- publishes an idempotent generated `## Context Efficiency` checkpoint at lifecycle projection boundaries and before MCP reload or upgrade, using canonical `wave:` markers and a single plain table of stage, tool-call count, and **Estimated context avoided**. The source component uses whole eligible file sizes (`ceil(UTF-8 bytes / 4)`), not proof of full-file reads avoided or measured model usage savings. Phase/version uniqueness does not prove repeated avoided reads; any paired residual remains separately identified in machine state;
- excludes known binary/runtime source paths and applies a bounded 4096-byte UTF-8/NUL prefix check to other candidate source files through the shared source-proof measurement path. This conservative heuristic does not establish that an entire file is text; unreadable or uncertain eligibility earns no credit while request/response debits remain;
- accepts exact legacy checkpoint wording with numeric/state consistency checks, preserving historical records and persisted keys without rewriting them for a label change;
...
- Use `docs/waves/00000 wave-zero-plans-and-specs/wave.md` as the single baseline-wave record. Keep all content inline — corpus inventory, normalization notes, review checkpoints — rather than splitting across subdirectories. A flat single-file wave is the preferred layout.
...
- Use the `<prefix> <slug>` form for normal waves (from `wf lifecycle-id --kind wave --slug <slug>`), such as `00057 routine-behavior-contract`; do not insert a literal `-wave` token — that is not what the generator emits.
...
- Generate both forms with `wf lifecycle-id --kind <kind> --slug <slug>`; the script emits a 5-character lowercase base36 time-ordered prefix (6 characters after the distant overflow horizon) derived from the provisioned lifecycle policy in `docs/workflow-config.json` (`lifecycle_id_policy`).
...
- Keep personas, workflow memory, and shared core docs in their canonical homes; the baseline wave record should reference those promoted outputs rather than duplicating them inside the legacy baseline folder.
...
- Do not wait for closure to preserve compaction-sensitive operating memory. Critical or high-salience observations become typed memory candidates during planning, implementation, review, handoff, or closure; the close checkpoint validates them.

...
- **One lock timeout no longer blanks Context Efficiency for good.** A context-efficiency write that met a database lock held by another attached MCP server for more than 50 ms wrote the durable accounting gap at once. From then on every wave's `## Context Efficiency` showed `accounting_gap`, with no recorded cause and no supported way out. Busy and locked writes are now retried for up to 10 seconds before the gap is written. The gap file records when and why it began, and `read_store_health` and its diagnostic report that. `python3 -B .wavefoundry/framework/scripts/context_efficiency.py --root <repo> --clear-gap` clears it and keeps every wave that lived through the gap marked `accounting_gap`, so no undercount is published as complete. Wave `1z2m4`.
